...
| **28** | **Schedule 8812 ACTC wiring** | **CONFORMS — audited 2026-07-08** | Line 28 (additional child tax credit) NULL-or-POSITIVE — set only when Schedule 8812 line 27 > 0, so a childless return (or one whose CTC is fully nonrefundable / earned income too low) leaves it null. No ZERO branch. Breadcrumb at the wiring; lock-in `line28AdditionalChildTaxCreditNullWhenNone`. |
...
| **34/37** | **`computeLine31ThroughLine38`** | **CONFORMS — audited 2026-07-08** | Line 34 (overpayment/refund) and line 37 (amount owed) MUTUALLY EXCLUSIVE via strict `>`: overpaid → Refund (positive), no AmountOwed; owed → AmountOwed (positive), no Refund; break-even → both null. No shown-ZERO. Breadcrumb at the branch; lock-in `line34RefundWhenOverpaidAmountOwedNull` + `line37AmountOwedWhenUnderpaidRefundNull`. |
| **38** | **Form 2210 penalty wiring** | **CONFORMS — audited 2026-07-08** | Line 38 (estimated-tax penalty) NULL-or-POSITIVE — set only when Form 2210 penalty > 0 and not WAIVED. No ZERO branch. Breadcrumb at the wiring. |
