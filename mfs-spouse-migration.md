...
25. ☐ **Lump-sum distributions** — `lump-sum-distribution-taxpayer` ↔ `-spouse` (generic; Form 4972). Bucket A — ★ CHECK BOTH prefixed-form gap variants ([[feedback_mfs_spouse_prefixed_form_scoper_unprefix]]): scoper un-prefix AND whether the spouse populate path reads bare keys (MFJ latent bug). **← CURRENT**
