...
**Other affected children's preferential income (Form 8615 line 7) — ✅ DONE 2026-07-11 (auto-derived).** This was a real correctness gap, not just polish: on the `dependent_own` path line 7 was **0** (siblings ignored entirely), understating multi-child kiddie tax. `KiddieTaxParentReader.getSiblingKiddieInfo(uid, currentDependentId)` now enumerates the other `dependent_own` returns, computes each (with a `ThreadLocal` recursion guard so sibling→sibling reads don't loop — a sibling's line 5 is independent of its own line 7), and sums their net unearned income (line 5) + the preferential portion `min(line5, sibPref)`. `DependentOwnFormScoper` seeds `otherChildrenNetUnearnedLine7` + `otherChildrenPreferentialLine7`; `computeForm8615` folds them into line 8 and the line-9 preferential income. Unit tests: scoper line-7 seed (present/omitted) + `getSiblingKiddieInfo` shape; full suite 1463/1463. **E2E deferred** (documented in `dependent-own-kiddie-tax.spec.ts`): a multi-child dependent_own e2e is blocked by a **pre-existing** dependent_own scoping interaction — the child's scoped compute still auto-builds Form 8814 for the household's children, so gross income ≥ $13,500 hits the non-overrideable `FORM8814_CHILD_GROSS_INCOME_TOO_HIGH` blocker, and with the full standard deduction no income level yields both a positive sibling line 5 and stays under that ceiling. Orthogonal to the sibling reader (which is unit-tested; the reader→scoper→compute chain is proven by the parent-preferential e2e). Item is compute-only (no persisted fields → no mapper/serialization e2e-parity risk).
...
- **Pre-existing dependent_own scoping cleanups (separate items, not kiddie-preferential):** the child's scoped compute should scope out the household's Form 8814 child-income logic, and should apply the dependent-limited standard deduction — surfaced by the multi-child e2e above.
