...
- Prevent FE/BE statement-form drift: centralize/generate statement form catalog shared by UI and backend. **Investigated + real drift FIXED 2026-07-08.** Compared the backend `StatementFormCatalog` (42 forms) against the FE `statement-selection.service.ts` list and found genuine one-directional drift: **Forms 4684 (Casualties/Thefts), 4797 (Sales of Business Property), 6781 (§1256 Contracts/Straddles), and 8824 (Like-Kind Exchanges)** were in the backend catalog AND had complete `CAPITAL_STATEMENT_CONFIGS` entries AND were read by `form-capital-gain-loss-{taxpayer,spouse}` compute — but were missing from all THREE FE registration points, so users could not add them (a classic "register in N places" incomplete-wiring, like the four backend hazards). Fixed by adding them to (1) `statement-selection.service.ts`, (2) the shell template `<form-capital-statement>` `*ngIf` blocks, and (3) the `capitalStatementFormIds` Set — mirroring the proven 8853/8889 generic-render path. FE and BE catalogs now match exactly (42 = 42; verified by diff). ng build green. **Drift-guard tests added 2026-07-08 (both sides):** symmetric pin tests now lock each list to the canonical 42-form set — FE `statement-selection.service.spec.ts` (`registers exactly the canonical statement-form ID set`) and BE `StatementFormCatalogTest.registersExactlyTheCanonicalStatementFormIdSet`. A form added/removed on either side fails that side's pin, forcing a deliberate change + a reminder to sync the other side (catches accidental drift like the 4 forms above). **Still open (the item's ORIGINAL intent):** the two lists remain SEPARATE hardcoded sources — a true shared-catalog/codegen (expose the backend catalog via an endpoint the FE consumes, or generate one from the other) that would make divergence IMPOSSIBLE is not built; the pins are a speed-bump, not a single source of truth. E2E coverage for the 4 forms is tracked separately under "[Capital forms E2E coverage]".
