...
- **Targeted FE tests for the multi-return/optimizer UI — the 3 flagship components now covered 2026-07-14; a few minor ones remain.** Added Karma/Jasmine component specs (22 tests, all green) for the three highest-value previously-untested components: `optimizer-comparison-panel` (8 — table totals, MFJ/MFS/TIE recommendation + savings display, running/error states, `runComparison` delegation), `persistent-mfs-toggle-panel` (6 — enable/disable render by `isMfsEnabled`, toggling/error states, method delegation), `dependent-own-toggle-panel` (8 — per-dependent `isEnabled`, multi-dependent disambiguation, string→number id coercion, disabled-without-id, delegation). Run headless via `ng test --watch=false --browsers=ChromeHeadless` (system Chrome). **Still untested (minor, deferred):** `eic-comparison-panel`, `form-filing-status` (parent container), `form-filing-status-spouse`. ★ Also noted: the pre-existing `form-filing-status-taxpayer.component.spec.ts` has 2 failing tests (async `onSubmit` timeout) — a separate pre-existing issue in that unmaintained spec, not touched here.
