...
- **F-D — Pub 590-A coordination discards a beneficial lump-sum election but leaves line 6c checked (LOW, deferred).** When the coordinator reduces the IRA deduction it rebuilds `SocialSecurityComputation` with `newTaxableSs` (normal method) at ~2919-2932, overwriting line 6b and BOTH the normal/lump-sum method fields, while PRESERVING `line6cLumpSumElection()`. If 6c was a beneficial §86(e) election (lower line 6b), that value is thrown away (line 6b rises → taxpayer over-taxed) yet 6c stays checked — an inconsistent return. A correct fix must re-run the lump-sum election INSIDE the coordination loop, because worksheet line 6 (and thus the current-year base) changes with the allowed IRA deduction; a naive "keep the old lump-sum value" would under-tax (it was computed at the pre-coordination lower MAGI). Extremely narrow (needs lump-sum back payment + IRA-coordination case + phaseout-driven deduction reduction simultaneously); LOW severity. Deferred to a coordination+lump-sum joint-recompute pass.
