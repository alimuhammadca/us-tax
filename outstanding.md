...
- **Combined 50%-of-AGI overall ceiling + ordering rule** when a return has BOTH large cash and large capital-gain-property gifts in the same year — the buckets interact (30% capital-gain is further limited so total ≤ 50%). Implemented as independent per-bucket limits; the interaction is deferred in our app. **★ SQA conformance scenario authored 2026-07-12: `sc_00253` (cash $30k + property $40k at AGI $100k → property limited to 50%AGI−cash = $20k, $20k carryover)** — a gap-finder; our app's combined ceiling/ordering compute is still deferred.
...
- **Multi-event Form 4684:** the current fields model ONE casualty event. A return with multiple separate casualties (each its own $100 floor) would need a repeatable per-event structure. **★ SQA conformance scenario authored 2026-07-12: `sc_00254` (two disasters → $100 per event, then one 10%-AGI floor → $26,800)** — a gap-finder; our app still models one event.
