...
- **Duplicate `prepare()` runs (multi-return) — REAL, deferred (LOW, performance only).** `OptimizerService.compareJointVsSeparate` computes MFJ + both MFS legs each run, and the legacy `/compute` recomputes the primary — so the primary return's `prepare()` runs twice across a compute+optimize cycle (4 prepare calls total, primary counted twice). No cross-endpoint caching/memoization. Results are correct; only wasteful. Fix would cache/reuse a leg's computed result within a request or share it across the two endpoints.
