...
> - **§162(l) SE-health + Marketplace/APTC circular (Pub. 974)** — the §162(l) earned-income ceiling IS applied, but when the self-employed person has a Marketplace plan WITH advance PTC, the SE-health deduction and the PTC are mutually dependent and Pub. 974 requires an iterative solve. The app deducts the entered premiums (subject to the ceiling) and raises the `marketplaceAdvisory` — it does NOT run the iterative Pub. 974 reconciliation. Deferred (iterative solver is a large dedicated feature).
>
