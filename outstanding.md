...
> - **Form 8829 home-office carryforward** — the disallowed excess (home-office deduction beyond the gross-income limit) is surfaced via the `HOME_OFFICE_LIMITED_BY_GROSS_INCOME` advisory but is NOT stored/carried into a future year, and no prior-year home-office carryforward is read into current-year expenses. **NOTE (updated 2026-07-27):** the app is no longer app-wide no-carryforward — three carryforwards now bridge across years (Form 172 NOL, Form 8990 §163(j), and Schedule A charitable — each persisted + auto-imported on the primary path). Form 8829 home-office and §461(l) disallowed loss remain **single-year** (current-year math IRS-correct; only the cross-year plumbing is out — a candidate to bridge the same way if wanted). **§465 RENTAL at-risk now bridges (V193, 2026-07-29):** `rentalAtRiskCarryforward` persisted (out_schedule_1) + auto-imported into next year's `rental-income.priorYearAtRiskCarryforward`, released against the current-year amount-at-risk headroom (`rental-at-risk-carryforward-bridge.spec.ts`). Schedule C / Schedule F business at-risk (Form 6198) is still single-year.
