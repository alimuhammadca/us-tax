...
- **[Form 1040 / Cleanup / LOW / ~~OPEN~~ RESOLVED 2026-07-07]** ~~Delete `us-tax-ui/public/irs/f1040_field_mapping_semantic.csv`.~~ **DONE 2026-07-07.** Deleted the served CSV. Verification corrected the entry's premise: the file WAS still referenced at runtime by `pdf-readonly-preview.component.ts` (`getCsvUrl()` f1040 special-case) — but the whole `PdfReadonlyPreviewComponent` is now **unmounted dead code** (nothing imports/mounts it; only past-tense comments in migrated components mention it), so the reference was dead too. Removed the dangling f1040 branch in `getCsvUrl()` alongside the delete. `tsc --noEmit` clean. The canonical `C:\us-tax\pdfs\f1040_field_mapping_semantic.csv` is retained. **Follow-ups left open:** (a) the entire `PdfReadonlyPreviewComponent` is dead and could be removed wholesale (bigger cleanup, out of this item's scope); (b) `f1040_semantic_labels.pdf` + `irs-images/f1040_semantic_labels/` PNGs were NOT deleted — other live components still fetch their own `*_semantic_labels.pdf`, so those need a per-asset check first.
