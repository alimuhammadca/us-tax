...
**QBI / §199A adversarial audit 2026-07-30 (4 agents) — deferred findings (fixed: #1 SSTB Part III, #2 line-12 net-cap-gain, #3 REIT/PTP bridge V207, #4 safe-harbor rental loss, #5 QBI gap-year — commits 839f16d + 12ae17a):**
- **#6 Above-threshold SSTB dilutes loss apportionment (LOW edge).** The loss-apportionment loop sums an above-upper-threshold SSTB's positive QBI into totalPositiveQbi before the SSTB-zero check, so it absorbs a share of a co-existing loss that then evaporates → deduction slightly overstated. Only when an above-threshold SSTB + a QBI-loss activity coexist. Fix: exclude above-threshold SSTBs from the netting pool.
- **#7 §1.199A-5(c)(1) de-minimis SSTB safe harbor not implemented (LOW).** A business with ≤10%/≤5% gross receipts from a specified-service activity can avoid SSTB status; the app only takes a user SSTB boolean (no auto-classification, no de-minimis relief). Acceptable for self-file, but the relief is silently unavailable.
- **#8 Elective aggregation (Form 8995-A Schedule B) not offered (LOW feature gap).** Activities are always treated separately (the correct conservative default); a taxpayer who could benefit from combining a high-wage + no-wage business into one aggregation cannot elect it.
- **#9 §461(l)-disallowed loss fully in QBI this year (LOW/UNCERTAIN).** buildScheduleCQbi uses raw net profit (before the §461(l) excess-business-loss cap); whether the disallowed portion belongs in QBI is genuinely gray (treated as an NOL carryforward). Flagged uncertain, not a confirmed bug.

**QBI / §199A adversarial audit 2026-08-01 (3 read-only Explore agents) — FIXED F-A + F-B (see history.md); deferred F-C/F-D below; F-E is a duplicate of #6 above:**
- **F-C — Above-threshold null W-2 wages → silent $0, no flag (MEDIUM, documented not fixed).** For a non-SSTB activity above the threshold, a null `section199AW2WagesAmount` is coerced to $0 (`firstNonNull(W2, ZERO)`), so the wage/UBIA limit and the deduction floor at $0. This is IRS-correct AND taxpayer-conservative (lower deduction) when wages are genuinely absent — a no-employee sole-proprietor legitimately has $0 W-2 wages. A blocking flag would FALSE-POSITIVE on those legitimate returns; the only real gap is a UX signal ("above threshold + $0 wages limits your QBI deduction"). Deliberately NOT flagged — a candidate advisory-only UX enhancement, not a compute defect. Complements the canonical null/zero rule (here null is treated as zero, the conservative choice).
- **F-D — MFS: a TIN-less §199A statement defaults to the taxpayer leg (LOW, deferred).** On an MFS return the statement lists are passed to `belongsToPerson` unshadowed; a statement with no `recipientTIN` falls to the taxpayer via the `!spouseHadInputs || taxpayerHadInputs` fallback (spouseHadInputs forced false by the MFS guard). Real 1099-DIV/K-1 imports carry a recipient TIN, so reachability is low. `belongsToPerson` is shared by ALL statement attribution — a fix belongs in a dedicated MFS-attribution pass (same class as the known MFS statement catch-all leak: exclude by `filing-status.mfsOtherSpouseSsn` first), not a QBI-local patch. The spouse's TIN-TAGGED statements are already correctly excluded; only the blank-TIN edge leaks.
- **F-E — duplicate of #6 (above-threshold SSTB dilutes loss apportionment).** The 2026-08-01 core-math agent independently re-surfaced #6 as a LOW/second-order observation (both reductions proportional; trigger largely blocked by `LINE13A_NEGATIVE_K1_QBI_THRESHOLD_UNSUPPORTED`). No new action — tracked as #6.

**Form 8863 (education credits §25A) adversarial audit 2026-08-02 (3 read-only Explore agents) — FIXED tax-free-assistance expense reduction (HIGH, see history.md); deferred below; AOTC math/LLC/phaseout/CLW/gates CLEAN:**
- **Schedule R credit-ordering (LOW-MED, pervasive — deferred to a dedicated pass).** `computeScheduleR`/`applyScheduleRToSchedule3` (Schedule 3 line 6d) runs at ~4526, AFTER `computeForm8863` (~4432) AND ~10 other credit Credit-Limit-Worksheets that subtract the elderly/disabled credit (grep `getElderlyDisabledCredit` → 33794/34316/34611/34965/35148/35455/35720/36449/37128/37144). All of them read line 6d as $0 because Schedule R hasn't run, so each individual nonrefundable credit can over-allow. The IRS Form 8863 Credit Limit Worksheet line 5 explicitly lists line 6d, so Schedule R must precede these credits. The aggregate Form 1040 line-22 floor (max(0, tax − credits)) usually absorbs the over-allocation → total tax typically unchanged, but the FILED Schedule 3 line-3 (and other credit) values can be wrong, with a rare ripple to refundable ACTC (Schedule 8812 CLW-A). A correct fix is a dedicated credit-ordering pass: move Schedule R before the first credit whose CLW subtracts 6d, verifying each of the ~10 affected CLWs against its IRS worksheet (moving it blindly could wrongly reduce a credit whose CLW should NOT subtract 6d). Too broad/risky for a single-form audit. (Precedent: Form 8880 was already reordered after Form 8863 for the same class of CLW-ordering fix, comment at ~4437.)
- **LOW (documented, not fixed):** null "at least half-time" (line 24) defaults to AOTC-denied (conservative, §-correct); no LLC fallback for a student the user tagged AOTC who fails an AOTC gate (relies on the per-student `creditTypeRequested` election — advisory/UX); the MAGI manual-fallback add-back fields (Puerto Rico / Form 4563 / Form 2555 manual) are read only from the taxpayer education form, not the spouse (household-total intake contract); §529/Coverdell tax-free distributions (Form 1099-Q, mapped but never coordinated) and the same-expense-two-benefits bar are trusted-input; `lines/8863.md` has stale entries (line 13 = "$180k/$90k threshold" not "line 9 + line 12"; the line-30 "= line 29 + 2,000" simplification predates the G-NEW-1 ≤$2k branch); the student-view `adjustedQualifiedEducationExpenses` still echoes the GROSS input (the tax now uses net; a form-fidelity display tweak, deferred under the preview visual-change protocol).

**Form 8959 (Additional Medicare Tax) adversarial audit 2026-08-02 (3 read-only Explore agents) — essentially CLEAN; FIXED Part II MFS spouse-SE guard (latent, see history.md); deferred LOW/cosmetic below:**
- **Whole-dollar rounding of the 0.9% products (LOW, ambiguous — deferred).** Lines 7/13/17 (Part I/II/III tax) are computed unrounded and summed into line 18, which is rounded once at the Schedule 2 hand-off (3338). The IRS whole-dollar method rounds each form line then adds; a filer with excesses in 2-3 Parts simultaneously can differ by ≤$1-2 (e.g. three $105.30 parts → per-line $315 vs code $316). But Pub 17's "include cents when adding and round off the total" rule arguably supports the current sum-then-round, so it is genuinely ambiguous; left as-is to avoid substituting a different small discrepancy. Would only be worth changing alongside a per-line whole-dollar pass.
- **Output model stores lines 7/13/17/18 unrounded (LOW, presentation).** Unlike line 21 (rounded on set, 33474), the Part I/II/III tax fields keep sub-dollar precision; a renderer emitting the actual Form 8959 PDF would print fractional dollars on whole-dollar lines. The TAX is unaffected (line 18 → Sch 2 is rounded). Round on set for form fidelity.
- **Line 22 subtracts the unrounded line 21 (Info/Low).** Line 22 = box-6 − raw(1.45%×box-5); the displayed line 21 is rounded. Differs by ≤$1 only at a half-dollar boundary on line 25c. Cosmetic.
- **Blank-primary-SSN cross-spouse W-2 leak (LOW, pre-existing).** `sumW2...ForSsn` helpers fall back to summing ALL household W-2s when `you.ssn` is blank, so an MFS leg with a missing primary SSN could pull the other spouse's Medicare wages/RRTA/box-6 against the $125k threshold. Requires a missing filed SSN (unusual); defensive fallback pre-dates this audit.
- **Legacy `computeAdditionalMedicareTax` (33482-33528) — confirm dead code.** A Part-I-only duplicate that the G3 supersede (3332) overwrites; appears unused for the Schedule 2 figure but sets a Part-I-only estimate. Confirm no live caller depends on it, then remove.

**Line 16 cap-gains adversarial audit 2026-08-02 (3 read-only Explore agents) — FIXED Schedule D Tax Worksheet §1250/28% net-cap-gain cap + `computeTaxBracket` JavaDoc (see history.md); deferred below; QDCG/routing/brackets/Tax-Table CLEAN:**
- **§1250-cap fix full-path e2e — DEFERRED (fix unit-tested + reachability confirmed).** The Schedule D Tax Worksheet §1250/28% cap is validated by `capGainsAudit4_scheduleDTaxWorksheetCapsSpecialGainsAtNetCapitalGain` (through the real caller `computeScheduleDTaxWorksheet` with a realistic ScheduleD, hand-verified $75,797.25) and reachability is code-confirmed (line 19 set at 17381 with no net-cap-gain cap; the only other cap fires solely in the §1231(c)-lookback path). A full-path e2e requires assembling a §1250 Form 4797 recapture (→ Sch D line 19) PLUS a separate LT capital loss (1099-B) shrinking net LTCG below line 19 — an intricate multi-form seed whose exact Sch D line-15/16/19 outcome depends on the loss-netting; deferred to a scoped e2e (the Sch D assembly itself is already covered by `form4797-section1250-recapture.spec.ts` + the capital-loss specs).
- **App Sch D line 19 not reduced by unrelated capital losses (display note).** The IRS Unrecaptured §1250 Gain Worksheet reduces line 19 by net capital losses; the app sets line 19 from the raw §1250 (17381) without that reduction, so the *displayed* Sch D line 19 can overstate the unrecaptured §1250 when losses exist. The TAX is now correct regardless (the Schedule D Tax Worksheet line-35 cap — the fix above — is the authoritative limiter), but the printed line 19 could be reduced to match the IRS worksheet for form fidelity.
- **`16.md` stale bits + LOW notes:** MFS QDCG 20%-floor typo $300,025 → $300,000 (FIXED in 16.md; code was already correct); a stale Branch-3 kiddie-fallback comment (7532-7538) describing a bracket fallback that the current guard no longer takes; sub-$25 Tax Table rows use a uniform $25 width (≤~$2, practically unreachable post-deduction); the local `docs/books/i1040gi_2025.txt` actually contains 2024 figures (methodology-only, not a 2025 dollar source).

**PTC §36B (Form 8962) adversarial audit 2026-08-02 (3 read-only Explore agents) — FIXED MFS cap-column + <100% FPL gate (see history.md); deferred below; FPL/MAGI/caps/Pub-974 CLEAN:**
...
- **LOW-nominal:** QSEHRA permitted-benefit reduction of col E not implemented; app model labels FPL/percentage as line 5/6 vs the IRS form's line 4/5 (value correct, label shifted); stale Schedule-2 comment corrected.

**EIC §32 (line 27a) adversarial audit 2026-08-02 (3 read-only Explore agents) — FIXED F1/F2/F3 (see history.md); deferred below; all 72 table constants + snap + continuity CLEAN:**
- **§32(i) royalty-vs-passive combined flooring (MEDIUM, wrongly-allows — deferred).** `computeInvestmentIncomeForEic` adds `netPassiveRentalRoyaltyForEic` (built upstream at ~22429 by pooling royalty net + passive rental + passive K-1 and flooring ONCE at >0). §32(i)(2) floors the rents/royalties tier (C) and the passive-activity tier (E) SEPARATELY (each `max(0, ·)`), and `max(0,C)+max(0,E) ≥ max(0,C+E)` — so a passive rental/K-1 LOSS masking royalty income UNDER-states the disqualifier and can admit an ineligible filer (e.g. royalty +$15k, passive rental −$10k → code $5k ≤ $11,950 allowed; correct $15k > ceiling → disqualified). A correct fix must split the pooled figure so royalty is floored independently of passive income (and royalty arguably shouldn't be in the §469 passive pool at all — it's portfolio income). Invasive (touches the shared rental/§469 computation); deferred to a scoped pass with the exact Pub 596 Worksheet 1 tier structure verified.
- **EIC-table FLOOR-vs-round (LOW — needs published-table confirmation).** `eicTableLookup` truncates (`setScale(0, FLOOR)`) the midpoint×rate. If the published 2025 EIC Table rounds-to-nearest at the bracket midpoint, FLOOR trails by $1 at brackets where the fractional part ≥ 0.50 (e.g. 1-child $10,000–$10,050: 10,025×0.34 = 3,408.50 → FLOOR 3,408 vs round 3,409). `27abc.md` asserts FLOOR "matches the IRS table convention"; the local docs contain the table rows but they didn't cleanly map to verify. Confirm against one published bracket before flipping (flipping on uncertain grounds risks a $1 error the other way).
- **Attestation-vs-ground-truth Form 2555 coupling (MEDIUM, robustness).** The EIC Form 2555 disqualifier reads the self-attested `hasForm2555`/`spouseHasForm2555` EIC-form booleans, not the actual computed `form2555Taxpayer`/`form2555Spouse` objects (which exist but aren't in scope at the EIC call site). A filer who genuinely files Form 2555 but leaves the EIC checkbox unchecked keeps EIC. Thread the real Form 2555 presence into `computeLine27aEIC` for a ground-truth gate.
- **LOW edges (documented, not fixed):** relationship set missing great-grandchild / grandniece-nephew / literal "adopted child" (drops a legit child only if the UI enum emits those strings); Medicaid-waiver (Notice 2014-7) not auto-added to EIC earned income unless the SE/otherEarnedIncome path carries it (possible intake gap); child born/died mid-year not treated as "lived all year"; younger-than-filer skipped when the filer DOB is missing; §1231/Form-4797 refinement to the investment-income capital-gain tier not applied (conservative/over-disqualifying).

**Social Security §86 (lines 6a–6b) adversarial audit 2026-08-01 (3 read-only Explore agents) — FIXED F-A/F-B/F-C (see history.md); deferred F-D + F-C test-note below; core tier math CLEAN:**
- **F-D — Pub 590-A coordination re-runs a beneficial lump-sum election (FIXED 2026-08-01).** Previously, when the coordinator reduced the IRA deduction it rebuilt `SocialSecurityComputation` with the normal-method `newTaxableSs` (~2919-2932) while PRESERVING `line6cLumpSumElection()`, discarding a beneficial §86(e) election (line 6b rose → over-taxed) yet leaving 6c checked. Fixed with the joint recompute: because reducing the IRA deduction lowers worksheet line 6 and raises provisional income (changing the lump-sum current-year base), the coordinator now re-runs `computeTaxableSocialSecurityLumpSum` with the coordinated worksheet line 6 and re-applies the same lesser-of gate as `computeSocialSecurityBenefits`, keeping line 6b and 6c consistent. Unit `iraCoordinationRerunsBeneficialLumpSumElectionAfterDeductionReduced`. See history.md.
- **F-C dedicated regression test — DEFERRED (fix applied; verified by inspection + existing tests).** The Form 8815 double-count fix in the Pub 590-A with-SS MAGI is verified by code inspection (the with-SS branch now strips Form 8815 once, exactly mirroring the no-SS branch's documented approach) and guarded by the six existing `iraCoordination*` tests (all green; the strip is a no-op when Form 8815 is absent). A dedicated numeric unit test needs five simultaneous conditions (taxable SS + covered-by-plan + traditional IRA contribution + Form 8815 exclusion + MAGI on the covered-phaseout edge), and pinning the exact post-fix allowed-deduction requires threading the SS worksheet through the coordination loop by hand — high arithmetic-error risk, so per the exact-value-pins rule it was not written loosely. Candidate for a follow-up when the phaseout-edge oracle is hand-verified.
- **Documentation drift (`lines/6abcd.md`) — NOTED.** §5.2/§9.3 still describe IRA↔SS coordination as advisory-flag/manual-review only (it is now a full iterative computation), the §6.2 adoption note still says "line 1f" (now Form 8839 line 30), and the line-number citations (orchestrator "14560", etc.) predate the current source (orchestrator 24339, lump-sum 25621, coordinator 26037). Spec understates current behavior; not a tax defect.

**§86 re-audit 2026-08-05 (3 agents) — RECONFIRMED CLEAN; the 2026-08-01 fixes hold. New deferred item F-E; the lump-sum JavaDoc + read-site comments were corrected (no compute change):**
- **F-E — foreign treaty-exempt Social Security has no intake (DEFERRED, under-tax scope gap).** Under the U.S.–Canada and U.S.–Germany income-tax treaties, social security paid by those countries is treated as U.S. SS and included on Worksheet 1 line 1 (Pub 915). The aggregator reads only SSA-1099 `netBenefitsAmount` + RRB-1099 `netSSEBAmount`; there is no Canadian/German-SS intake field, so such benefits are omitted → understated provisional income and line 6b. Needs a new intake field (statement/personal-form field-add → user sign-off) + an advisory; not built. No advisory today.
- **F1 "MFS spouse-leg residence flag not scoped" — FALSE POSITIVE (verified 2026-08-05).** BOTH SS forms collect `livedWithSpouseAnyTimeDuringTaxYear`/`livedApartFromSpouseEntireTaxYear` as bare-key answers (spouse component lines 356-357); the mapper round-trips them per owner_role and the MfsFormScoper generic rename carries the spouse's own answer onto her leg. `computeSocialSecurityBenefits` reads the correct per-leg fact — no scoper case needed. (The V15 schema comment "taxpayer form only" is stale; clarifying note added at the read site.)

**Form 8880 (§25B Saver's Credit) adversarial audit 2026-08-05 (run directly, no subagents) — CLEAN core; FIXED the distribution-double-count UI label (see history.md); deferred below:**
- **1099-R distribution auto-sum excludes only rollover codes G/H (line 4 over-count → under-credit).** `sum1099RDistributions` filters out direct-rollover codes G and H, but Form 8880 line 4 also excludes in-plan Roth rollovers, Roth conversions, loans treated as distributions, and returned excess contributions/deferrals — none of which are code-G/H, so a 1099-R for any of those over-counts the line-4 offset and under-states the eligible contribution. Direction: under-credit (conservative). The manual `distributions2023to2025` field lets the user override, and precise auto-classification needs per-code logic across many 1099-R codes; deferred. (Everything else — AGI ceilings, the 2025 rate table, the $2k cap, the age test, the Credit Limit Worksheet composition, the MFJ cross-column rule, and the Schedule 3 line-4 wiring — verified correct vs f8880.pdf.)

**Form 5695 (§25C/§25D Residential Energy) adversarial audit 2026-08-05 — FIXED battery line-5b per-property eligibility + fuel-cell half-kW flooring (see history.md); deferred below. Part II §25C, both Credit Limit Worksheets, the §25D carryforward, and the Schedule 3 wiring all CONFIRMED correct:**
- **Fuel-cell joint-occupancy proration not auto-computed (§25D, over-credit, intake-dependent).** For non-MFJ co-owners of a home with fuel-cell property, i5695 caps each occupant's qualifying cost at the lesser of (a) what they paid or (b) $1,667 × their payment fraction, per half-kW (the Ava/Bruce example). The app exposes only a `line7cJointOccupants` checkbox and relies on the user entering their pre-allocated share; two co-owners who each enter the full cost would double-claim. Auto-proration needs each co-owner's payment amounts (not captured). Deferred (trust-the-user allocation-input, like the Form 2441 disqualified-provider gap). Same class: the door "other" bucket isn't per-door $250-capped (only the $500 total binds) — minor, list items are capped.

**Form 2441 (§21/§129 Child & Dependent Care) adversarial audit 2026-08-05 (3 agents) — FIXED line-16 gross fallback + name-only-count (see history.md); deferred below. Two headline findings were FALSE POSITIVES (line-30 net field is correct; single-filer deemed income is IRS-allowed):**
- **Disqualified providers (§21(e)(6)) not checked — trust-the-user (over-credit gap).** Payments to the taxpayer's own dependent, their child under 19, or the qualifying child's parent don't qualify, but the provider ingest reads name/EIN/amount only with no cross-check against the return's dependents (contrast: qualifying *persons* get a family-SSN/age check). No advisory. Auto-detection needs provider-vs-dependent SSN matching; deferred (trust-the-user posture, like other provider-side gaps).
- **MFS §129(b) spouse-earned-income falls back to the taxpayer's OWN income when blank (over-exclusion / under-tax).** For an MFS filer not considered unmarried, when `mfsSpouseEarnedIncome` is $0/blank, line 19 = the taxpayer's own earned income instead of the smaller-of-both → the $2,500 benefit exclusion is under-limited. A non-blocking advisory `DEPENDENT_CARE_MFS_SPOUSE_EARNED_INCOME_LIMIT` already documents it ("the amount excluded may be too high"). Candidate to PROMOTE to a §17 blocking flag (per the MFS-disallowed-credit playbook) so the wrong line 1e can't reach a filed return — deferred (behavior change; the exclusion can't be computed without the spouse's income).
- **Per-month vs annual deemed §21(d)(2) income — minor under-credit approximation.** Deemed income uses `max(annualActual, monthly×months)` rather than the per-month `max($250/$500, actual)` the i2441 requires; understates earned income for a mixed work/student year (taxpayer-unfavorable). Common all-year-student case is exact. Deferred (low audit risk; needs per-month calendar tracking).
- **Considered-unmarried MFS test uses a different field than the rest of the app.** Dependent care reads `childcareData.generalQuestions.marriedFilingSeparatelyException`; other MFS gates read `filing-status.mfsOrHohLivedApartOrLegallySeparated`. A filer who set one but not the other gets divergent MFS treatment across forms. Candidate to unify onto the filing-status field. Deferred.

**NIIT / §1411 (Form 8960) adversarial audit 2026-08-01 (3 read-only Explore agents) — FIXED line-9a auto-flow + MFS FEIE guard + line-4b comment (see history.md); deferred below:**
- **Line-5b sign convention vs the printed-form "combine" (documented; tax correct, display needs a design call).** `computeForm8960` computes `line5d = line5a − line5bOverride + line5cOverride`, i.e. line 5b is SUBTRACTED as a positive magnitude while 5c is ADDED (signed). The 2025 Form 8960 line 5d is "combine lines 5a–5c" (algebraic add), with 5b entered as a NEGATIVE to back out a gain. Under the app's documented positive-magnitude convention (comment ~32840) the TAX is correct, but (a) the rendered `form-tax-return-8960` preview shows 5b positive while 5d subtracts it → the printed lines don't reconcile with "combine", and (b) a user who mirrors the IRS form and enters 5b negative gets a sign-flip (NIIT overstated). Resolving it (standardize on the signed IRS "combine" convention across compute + UI intake + preview, or render 5b negative) spans three layers and touches a verified preview form → needs the visual-change protocol + sign-off. NOT changed blind.
- **Active-business / non-passive disposition gains in line 5a are override-only (MEDIUM).** Gains from disposing property held in a non-passive trade/business or an actively-participated S-corp/partnership interest are NII-excluded (§1411-7 / line 5b/5c) but flow into line 5a and are removed only via a manual override — asymmetric with line 4's auto-exclusion → NIIT overstated absent user action. Auto-backing-out needs a taxpayer DESIGNATION of which disposition is an active interest (the app can't infer it from Schedule D), same class as the taxpayer-designated cap-gains character rule. Needs a designation field + sign-off.
- **RE-professional flag over-excludes non-materially-participated rentals from NIIT (MEDIUM, §469 subsystem).** `nonpassive = reProfessional || materialParticipation(p)` (~22241) forces EVERY rental non-passive once RE-pro, dropping a genuinely-passive (e.g. triple-net-lease, no material participation) rental from line 4c → NIIT UNDER-collected. §469(c)(7) requires per-rental material-participation testing; the correct predicate is closer to `reProfessional && materialParticipation(p)`. This classification also drives §469 rental LOSS limitation, so changing it has broad blast radius and needs its own §469-scoped audit + tests. NOT changed in a NIIT pass.
- **No self-rental §469(f)(6) recharacterization (MEDIUM).** Self-rental to a materially-participated business recharacterizes net INCOME (only) as non-passive → excluded from NII (Reg. §1.1411-4(g)(6)); a self-rental LOSS stays passive. The data model has no self-rental input, so income can't be excluded and the only workaround (`materialParticipation`) wrongly flips a loss too. Needs a new self-rental field + sign-off.
- **Passive-K-1 default = non-passive (LOW-MED, §469 default).** `isPassiveK1` treats an unanswered participation question as non-passive → a passive K-1's income is dropped from line 4c → NIIT under-collected. Safe for §469 (no loss cap) but wrong-direction for §1411. Changing the default is a cross-subsystem §469 decision.
- **Line-1/2 no non-passive-business carve-out (LOW) + line-3 annuity default-0 (LOW-MED, conservative).** Business-derived interest/dividends are on Sch C/E not 2b/3b (low exposure; line-7 override available). Line 3 defaults to 0 because 1040 line 5b is overwhelmingly §1411(c)(5)-excluded qualified-plan income; auto-pulling it would wrongly tax retirement income. Both defensible.
- **Sibling Form-2555-spouse FEIE leaks (Roth MAGI ~3390, MAGI consumer ~4367) — same root as the fixed NIIT F-B, but OUTSIDE NIIT scope.** Both add the other spouse's FEIE/housing exclusion unguarded on MFS (inflating the Roth-contribution MAGI and another MAGI base). Fix pattern is identical (`isMfsReturn ? null`), but changing Roth MAGI affects Form 5329 excise on MFS → needs Roth-scoped validation. Deferred to a dedicated MFS-MAGI pass.
- **Known MFS null-TIN statement catch-all leak (`belongsToEitherCapitalPersonByKey` ~19074) — NIIT is a downstream victim.** A 1099-INT/DIV/B with a blank `recipientTIN` leaks onto the taxpayer's MFS leg, inflating NIIT lines 1/2/5d. Same known class as `feedback_mfs_statement_catchall_leak`; root fix is upstream of `computeForm8960`.
...
>
> The entire SE program (C1–C7) plus all follow-ups are implemented, e2e-verified, and deployed to prod. Repeated "100% complete" claims were each disproven — **three adversarial audits, each a different lens, found & fixed 9 real gaps.** (Honest posture per `rules.md`: "no known gaps in the areas audited," NOT "provably complete" — each new lens found real bugs.)
> - **Audit 1 (routing):** Gap 1 — K-1 (Form 1065) box 14A → SE tax (`270ae57`); Gap 2 — §465 at-risk on Schedule C/F LOSSES / Form 6198 (V156, `5a6d1af`); Gap 3 — Schedule SE line 1b CRP-payments exclusion (V157, `735845c`).
> - **Audit 2 (captured-but-never-read):** Schedule SE **line 8b/8c real OASDI-overpayment bug** — line 8d was W-2 SS wages only; now + Form 4137 line 10 (unreported tips) + Form 8919 line 10 (wages) (`b3fb2ee`); plus 3 dead se-tax-options fields wired — `priorYearUnreportedTips` + `groupTermLifeAdditions` → Schedule SE line 2, `medicaidWaiverIncludeInSe` (Notice 2014-7) → SE base.
> - **Audit 3 (computation-correctness):** SEP/SIMPLE line-16 **over-deduction bug** — per-plan caps were never applied; now SEP ≤ 20% of net earnings, SIMPLE ≤ $16,500 + $3,500 catch-up (50+) + 3% match (`b5f69d0`); **§179(b)(5) heavy-SUV cap** — the $31,300 constant existed but was unenforced; now V158 `is_heavy_suv` caps §179, remainder → bonus/MACRS (`8a2f0fc`).
>
...
> - ~~**§199A negative-QBI netting across businesses**~~ **✅ RESOLVED 2026-07-27:** loss trades are now included in the QBI components (Schedule C main path, QJV per-spouse split, Schedule F), so a LOSS business's negative QBI **nets** against other businesses' positive QBI (Form 8995 line 1 aggregation). The per-person ½SE/line-16/line-17 deductions are allocated over positive-profit trades only, so all-profit returns are byte-identical (no regression). e2e `qbi-negative-netting.spec.ts` (profit $100k + loss $40k → aggregate QBI $55,761 not $92,935; pure loss → −$40k floored to $0). **Both follow-ups also DONE 2026-07-27:** (a) Form 8995-A above-threshold per-activity **Part IV loss apportionment** — a loss activity is apportioned across the positive activities before their wage/UBIA limit (§1.199A-1(d)(2)(iii)); `qbi-negative-netting.spec.ts` 8995-A test (profit-only $37,281 → +$50k loss $27,881). (b) **QBI-loss carryforward bridge** — a negative net QBI auto-imports as next year's prior-year QBI-loss carryforward (Form 8995/8995-A `loadByTaxReturnId` + `importedPriorYearQbiLossCarryforward`); `qbi-loss-carryforward-bridge.spec.ts`. Now four cross-year carryforward bridges (Form 172 NOL, Form 8990 §163(j), Schedule A charitable, §199A QBI loss).
>
...
> - **Form 4797 depreciation/§179 RECAPTURE on disposition** — §1245/§1250 sale recapture and §179/§280F business-use-drop recapture are NOT computed (Part III/IV). Form 4797 is otherwise a manual pass-through: the `OTHER_INCOME_FORM_4797_REQUIRED_TO_BE_FILED_SEPARATELY` advisory tells the filer to complete the form externally (including any recapture) and hand-enter the result. A taxpayer who sells a fully-§179'd asset must compute the ordinary recapture themselves. Building the full disposition-tracking + Part III recapture engine is a large dedicated feature (`C:\us-tax\form-4797-plan.md`), phased. **★ Phase 1 DONE (2026-07-27): the §1231(c) 5-year lookback** — a net §1231 GAIN is recharacterized as ordinary (line 8) up to the prior 5 years' non-recaptured net §1231 losses; the residue (line 9) stays capital. Multi-year persistence (out_section_1231_recapture, V185) + cross-year replay; net §1231 gain routes line 8 → Schedule 1 line 4 (ordinary), line 9 → Schedule D. Form4797Mapper activated (formIds()→"4797") so 4797 §1231 dispositions are enterable via the statement REST API (still OUT of the FE picker until its capital-statement-config migrates off raw PDF-slot keys). e2e `section1231-lookback-bridge.spec.ts`. **★ Phase 2 DONE (2026-07-28): §1245 recapture ENGINE** — computes §1245 ordinary recapture (min(gain, depreciation)) + the §1231 excess/loss directly from the existing se_form_4797 Part III raw inputs (line 20/21/22) — no new form. Recapture → Schedule 1 line 4; §1231 excess → the lookback → Schedule D. Form4797Recapture output + V186. e2e `form4797-section1245-recapture.spec.ts` (3). **★ Phase 3 DONE (2026-07-28): §1250 REAL-property recapture + unrecaptured §1250 @ 25%** — line 26a "additional depreciation" presence discriminates §1250 (no new field); §1250 ordinary recapture = min(gain, line 26a) → Sch 1 L4; unrecaptured §1250 = min(gain, line 22) − ordinary → Schedule D line 19 @ 25%; residual → §1231. **★ ADVISORY ADDED (2026-08-05):** the line-26a discriminator means post-1986 STRAIGHT-LINE real property (line 26a legitimately blank) routes through the §1245 branch → ALL depreciation recaptured as ordinary (over-tax vs. 25% unrecaptured §1250 + §1231 capital). Non-blocking `FORM_4797_POSSIBLE_SECTION_1250_REAL_PROPERTY` now fires when a §1245-branch long-term gain has a real-estate-looking description (`looksLikeRealProperty` heuristic), steering the filer to the capital-gain-loss form's §1250 designation field (the correct path). Description-only heuristic → advisory, no compute change; unit-pinned. A property-type intake field (to auto-route) remains the only way to make this automatic — deferred (statement field-add needs sign-off). Also fixed two pre-existing gaps that blocked ANY 4797 §1231 gain from reaching Schedule D (scheduleDRequired + computeCapitalForPerson early-return now account for a 4797 line-9 gain). V187. e2e `form4797-section1250-recapture.spec.ts` (2). **★ Phase 4 DONE (2026-07-28): §179/§280F(b)(2) Part IV business-use-drop recapture** — line 35 = max(0, line 33 prior accelerated deduction − line 34 recomputed SL/ADS), pure ordinary → Schedule 1 line 4; computed from existing 4797 Part IV lines 33/34 (no new field). V188. e2e `form4797-part4-recapture.spec.ts` (3). Phases 1-4 of the recapture engine complete. **★ Phase 5 QBI/SE ATTRIBUTION DONE (2026-07-28):** recapture attributed to the person's single Schedule C — Part IV §179/§280F → Schedule C net (SE tax + QBI, off line 4); Part II/III §1245/§1250 → line 4 (AGI) + QBI base only (not SE, §1402(a)(3)); single-Schedule-C heuristic with conservative line-4 fallback. e2e `form4797-recapture-qbi-se.spec.ts` (3). **★ Phase 5 §1231 NETTING DONE (2026-07-28):** per-owner `consolidateSection1231` — a net §1231 LOSS (§1231(a)(2)) → Schedule 1 line 4 ordinary + QBI (was dropped); multi-entry §1231 nets to the true Schedule D amount. e2e updated §1245 scenario C + new multi-entry netting test. **★ §1250-LOOKBACK cap DONE (2026-07-28):** `capUnrecapturedSection1250` caps the unrecaptured §1250 (Sch D line 19, 25%) at the residual §1231 capital gain after the §1231(c) recharacterization; two-year e2e `form4797-section1250-lookback-cap.spec.ts`. **★ Multi-business advisory DONE (2026-07-28):** non-blocking `FORM_4797_RECAPTURE_MULTI_BUSINESS` when the owner has >1 Schedule C (or QJV) → recapture falls back to line 4 (not in QBI/SE); e2e in `form4797-recapture-qbi-se.spec.ts`. **★ Precedence DONE (2026-07-28):** computed Part III supersedes a transcribed line 7 on the same entry; `FORM_4797_MIXED_COMPUTED_TRANSCRIBED` advisory when computed + transcribed §1231 coexist (net at lookback, not per-entry for Schedule D); e2e `form4797-precedence.spec.ts`. **★ 6252/8824 interplay DONE (2026-07-28):** `FORM_4797_RECAPTURE_INSTALLMENT_LIKEKIND` advisory when a computed recapture coexists with a Form 6252/8824 entry (§453(i)/§1031); Form8824Mapper activated (backend/API); e2e `form4797-installment-likekind.spec.ts`. **★ AMT DONE (analyzed, no code): recapture/§1231/unrecaptured-§1250 are identical for regular tax and AMT — no adjustment; the disposition basis-difference (Form 6251 line 2k) needs AMT-depreciation tracking → separate deferred feature. ★★ PHASE 5 COMPLETE — the Form 4797 recapture engine (Phases 1–5) is done.** Separate deferred features (NOT Phase 5): AMT disposition basis-difference (needs AMT-depreciation tracking); 6252 statement-mapper activation (blocked by its raw-slot FE config). NOTE: a pre-existing, UNRELATED `StatementFormCatalogTest` failure exists (stale pin — ~42 expected vs ~56 actual forms in the catalog; needs a deliberate pin refresh).
> - ~~**§162(l) SE-health + Marketplace/APTC circular (Pub. 974)**~~ **✅ RESOLVED 2026-07-28.** The Pub. 974 (2025) Iterative Calculation Method between the §162(l) SE-health deduction (Schedule 1 line 17) and the §36B Premium Tax Credit (Form 8962) is now modeled. After Form 8962 is computed, `solvePub974SeHealthPtc` bisects the deduction fixed point `D = min(specifiedPremiums − PTC(MAGI@D), earned-income ceiling)` — the deduction→PTC map is monotone non-increasing so the fixed point is unique and bisection is robust (Pub. 974 itself falls back when naive iteration oscillates ≥$1) — then `prepare()` re-runs ONCE with the converged per-side line-17 in `SE_HEALTH_PTC_DEDUCTION_OVERRIDE` (same two-pass ThreadLocal pattern as the §172 NOL cap) so AGI/MAGI/PTC/QBI/downstream are all self-consistent. The result satisfies the Pub. 974 invariant (deduction + allocable PTC ≤ premiums; deduction ≤ ceiling). Specified premiums = 1095-A col A gross enrollment (Worksheet W line 1); nonspecified (LTC/Medicare) deducted first against the ceiling. Advisory `SE_HEALTH_MARKETPLACE_COORDINATION_ADVISORY` replaced by a positive `SE_HEALTH_MARKETPLACE_COORDINATION_APPLIED` note when coordinated; the advisory remains only on the fallback paths. **Both-spouses / >1-business now ALSO coordinated (V192, 2026-07-29):** two SE-Marketplace spouses no longer fall back — the PTC is a household credit depending only on total MAGI, so `solvePub974SeHealthPtc` solves the fixed point once on the TOTAL specified premiums (`g(D)=min(totalSpecified − PTC(M0−D), Σ remaining §162(l) ceilings)`) and splits `dStar` across the two ceilings by a new per-side `marketplacePremiumSharePercent` intake field (single side → 100%; both-null → even; one-null → complement; both → normalized). One side reduces exactly to the former result. A conservative discontinuity clamp (`dStar = min(dStar, totalSpecified − PTC(M0−dStar))`) keeps the invariant *deduction + allocable PTC ≤ premiums* and makes the pick deterministic across the PTC step (no-op on clean fixed points). e2e `pub974-both-spouses-se-health-ptc.spec.ts` (A 30k/30k 50/50 → 1,320+4,668=5,988; B1 ceiling-cap 5,576; B2 full 6,000). **Remaining simplifications:** MFS excluded (PTC disallowed → no circularity); months-ratio = 1 (no per-month self-employment input); the per-business split is the declared percentage, not a per-policy 1095-A attribution (Form 8962 is one household form); the Worksheet X/Y repayment-limitation only sets the IRS method's *starting* deduction, irrelevant to the bisected fixed point. e2e `pub974-se-health-ptc-coordination.spec.ts` (net profit 60k + 1095-A 500/600/400×12 → line 17 = 2,676, PTC = 3,324, excess-APTC repayment = 1,476; invariant 2,676+3,324 = 6,000; + fallback test). The §162(l) ceiling itself was already applied.
>
> **Remaining SE-1040 compute deferrals (OOS-by-design):** ~~Form 2210-F (farmer/fisherman ⅔ estimated-tax safe harbor)~~ **NOW BUILT (2026-07-28):** farmer/fisher (auto-detected: current-year gross farm income ≥ ⅔ of Form 1040 total income) uses the 66⅔%-current / 100%-prior single-period safe harbor → penalty to Form 1040 line 38, and a persisted `Form2210F` output slice (out_form_2210f, V190) populates the existing `form-tax-return-2210f` preview. e2e `form2210f-farmer-fisher.spec.ts`. See the dedicated section below. ~~multi-year NOL carryforward tracking (Form 172)~~ **NOW BUILT (2026-07-27):** §172 80% limit + persisted `nolCarryforwardToNextYear` (out_form_172, V175) + cross-year auto-import into next year's line 8a (bridge; two-year e2e `nol-carryforward-bridge.spec.ts`). Everything else in the SE plan below is DONE. (The broader "never in SE-1040 scope" items — entity/partnership return preparation, payroll filings, Schedule J farm-income averaging, Schedule H, etc. — are unchanged; see §I below.) See `history.md` (2026-07-20 → 2026-07-21) and memory `project_se_income_implementation.md` for the authoritative detail.
...
2. **Depreciation = FULL MACRS engine up front** (5/7-yr 200/150DB, half-year + mid-quarter
   ...
   **C4c DONE 2026-07-20** (vehicle standard-mileage $0.70; listed-property ≤50% → ADS SL; luxury-auto
   §280F caps via new isPassengerAutomobile field). **C4d DONE 2026-07-20** (Form 8829 actual home
   office → Schedule C line 30, gross-income limit + carryforward). **★ C4 COMPLETE** — full depreciation
   engine + Form 8829 delivered.
...
| Sch 1 L17 | SE health (Form 7206) | ✅ DONE 2026-07-20 (C3a) — blocker retired | §162(l): min(premiums, net profit−½SE−L16) → L17; Marketplace = advisory |
| 1040 L13a | QBI from C (F=C5) | ✅ DONE 2026-07-20 (C3b) — blocker retired | Schedule C net (−½SE−L16−L17) → 8995/8995-A (SSTB flag + line-26 W-2 wage limit); verified below-threshold 15,437 / SSTB disallowed / wage-limit path |
...
**Form 8863 per-institution address decomposition — ✅ RESOLVED 2026-07-13 (Gap 8863-3, V127).** Signed off by
the user (full 7-piece scope; delete the old combined field, no data migration). Replaced the single
`institution{N}AddressLine22` blob on each of the two institution slots with seven structured fields —
street/city/state/ZIP + foreign country/province/postal-code — each mapping 1:1 to its IRS Part III PDF box.
Full stack: `PfEducationStudent` + `OutForm8863Student` entities, `EducationCreditsMapper` +
`Form8863OutputMapper`, `Form8863Student` output model, `computeForm8863`, the 8863 preview `buildSemanticValues`,
and the education-credits intake UI (street/city/state/ZIP always shown; a non-persisted "outside the U.S."
toggle reveals the 3 foreign inputs, auto-shown when foreign data exists). V127 drops the 2 old columns + adds
...
2. **Combined $7,000 cap / traditional-reduces-Roth — ✅ DONE 2026-07-28.** Per §408A(c)(2) the Roth limit
   ...
   $5k traditional + $5k Roth (combined limit $7k) → $3k Roth excess = $180 (was $0 pre-fix — computed
   independently against the full $7k).
3. **No year-end-value cap default — unchanged (reasonable).** The excise is 6% of the smaller of the
   excess or the account's year-end FMV; when the (optional) value field is PRESENT the code already caps
   at min(excess, value), and when absent uses the full excess — a correct conservative default. No change.
...
**Can the app produce separate returns for dependents? YES** — the same multi-return architecture that powers the MFS split also produces a standalone Single Form 1040 for a dependent who files their own return. It is live at runtime with no feature flag. Full stack present: `tax_return_v2` (`return_kind='dependent_own'`) + `tax_return_v2_person` filer link + `person.files_own_return`; `TaxReturnV2LifecycleService.enable/disableDependentOwn`; REST (`POST /api/tax-return-v2/dependent-own/{enable,disable}/{dependentId}`, `GET /api/tax-return-v2`, `POST /api/tax-return/compute/{taxReturnId}`); compute scoping via `scopeForDependentOwn` → `DependentOwnFormScoper.scope()` (drops household forms, injects synthetic identity as filer, forces Single, statements route by SSN); frontend `dependent-own-toggle-panel` + per-dependent Tax Return section in `shell.component.ts`. Tests: `dependent-own-basic`/`-eic-ineligible`/`-kiddie-tax` e2e + `Phase8a/8b` unit.

**Correctness that IS in place:** kiddie tax (Form 8615 — parent taxable-income/status auto-pulled, preferential QDCG + 28%/§1250 rates, multi-child sibling pooling); dependent-limited standard deduction (fixed 2026-07-11); EIC structurally unavailable; Form 8814 no longer mis-fires on the child's own return (fixed 2026-07-11).

### Limitations (open)

1. **Income-attribution / dependent-statements — ✅ IMPLEMENTED 2026-07-11 (curated picker, mirrors the spouse mechanism).** A dependent's income is pulled from the household statement pool by SSN match (exactly the spouse mechanism — statements are stored per household, routed to a tab by the `recipientTIN`/SSN filter `statementFormBelongsHere`; the frontend never stamps `owner_role`, so it defaults to `taxpayer` and is NOT the routing key). The dependent tab renders the full Statements section, and the **"Select statement" picker now offers the dependent-applicable subset** (`StatementSelectionService.formSectionsFor('dependent')` filters out the out-of-scope + parent-claimed types — see `DEPENDENT_INAPPLICABLE_FORM_IDS`), passed from the shell via `[personKind]`. The prior hard-coded auto-offer of `1099-int`/`1099-div` was removed for a consistent spouse-like picker flow. No `dependent_id` on statement rows — multi-dependent disambiguation is by SSN, per the spouse approach (owner). Verified: `dependent-statement-picker.spec.ts` e2e (dependent picker offers w-2/1099-int/div/b/r/2439, excludes child-interest-dividends/1099-nec/1099-k/1095-a/1099-sa/1098-t; Family Head picker is full) + `statement-selection.service.spec.ts` unit pin. **Residual — ✅ DONE 2026-07-11 (recipient-SSN auto-fill).** A newly-created statement entry now pre-fills its recipient SSN with the active person tab's SSN, so the statement routes to that tab automatically (no manual SSN typing). New `PersonScopeService` holds the active tab's SSN (the shell syncs it on tab change + after identification SSNs load); `StatementEntryStateService.newEntrySeed()` supplies the SSN under every recipient-field key (recipientTIN / employeeSSN / beneficiarySSN / shareholderTIN — each mapper reads only its own, extras ignored) on both create paths. Benefits taxpayer + spouse + dependent. Verified: `statement-recipient-ssn-autofill.spec.ts` e2e (dependent 1099-INT → 333-33-3333; taxpayer 1099-DIV → 111-11-1111).
2. **EIC / §152 self-claim disallowance — ✅ EXPLICIT 2026-07-11.** Previously EIC was correct by-omission (never seeded) with no user-facing reason. Now `prepare()` emits a non-blocking advisory `DEPENDENT_OWN_RETURN_EIC_AND_SELF_CLAIM_DISALLOWED` on the `dependent_own` path (gated on the `filing-status.dependentOwnReturn` marker) stating that because the filer can be claimed as a dependent, the return excludes the Earned Income Credit (IRC §32(c)(1)(A)(ii)(III)) and they can't claim their own personal exemption/dependents (IRC §151(d)(2)/§152(b)(1)); education credits for their tuition go to the claiming taxpayer. The math was already correct (EIC absent, dependent-limited standard deduction); this just makes the reason visible. Verified: `dependent-own-eic-ineligible.spec.ts` asserts the advisory + that line 27a stays absent.
4. **Freshly hardened** — this path had two live correctness bugs (full-vs-limited standard deduction; Form 8814 blocker leak) fixed 2026-07-11; it has not had heavy real-world exercise.

### Dependent-statements analysis — are ALL statement types applicable to a dependent?

**No.** A dependent's own return reports only the dependent's own income; several statement types are out-of-scope (self-employment) or N/A for a person who is claimed as someone else's dependent. Curated applicability of the ~40 statement types:

| Bucket | Statement types | Rationale |
|---|---|---|
| **Applicable — core (implement first)** | `w-2` (teen job), `1099-int`, `1099-div`, `1099-b`, `1099-oid`, `2439` (undistributed cap gains), `1099-r` (inherited/beneficiary), `1099-da` (crypto) | Ordinary earned + investment income a minor/student commonly has; drives wages, interest, dividends, capital gains, kiddie tax. `1099-int`/`div` already surfaced. |
| **Applicable — less common but valid** | `1099-g` (unemployment; state refund rare), `1099-misc` (prizes/awards/rents), `ssa-1099` / `rrb-1099` / `rrb-1099-r` (survivor/disability benefits to a child), `1099-q` (529/Coverdell in the student's name — taxable earnings portion), `1099-qa` (ABLE), `1099-ptr`, `schedule-k1-1041` (trust beneficiary), `schedule-k1-1065`/`-1120s` (gifted family-business units), `5498`/`8606` (dependent IRA), `1099-c`/`-a`/`-s`/`-cap`, `w-2g` (gambling), `1099-ltc`, `6252`, `6781`, `4684`, `8824`, `3921` | Legitimate but rarer; include for completeness, likely behind the "Select statement" picker rather than auto-offered. |
| **Out of scope (self-employment — app-wide OOS)** | `1099-nec`, `1099-k`, `4797` | A dependent's gig/SE income routes to Schedule C/SE/F, which the app does not support (same OOS as everywhere). Should NOT be offered on a dependent tab (or must show the existing SE out-of-scope blocker). |
| **N/A for a claimed dependent** | `1099-e` (1098-E student-loan interest — a dependent can't take the deduction), `1095-a` PTC + `1098-t` education CREDIT (claimed by the parent who claims the dependent — but the taxable-scholarship portion of 1098-T IS the dependent's income, so 1098-T is a partial case), `1095-b`/`-c` (informational), `1099-sa` (a dependent generally can't have an HSA), `child-interest-dividends` (this is the Form 8814 PARENT-election form — the ALTERNATIVE to the dependent filing, so it must never appear on the dependent's own tab) | Credits/deductions that flow to the parent, or forms that contradict the dependent filing their own return. |

**Recommended implementation:** curate the statement picker + `statementFormBelongsHere` per person-kind so a dependent tab offers the *core applicable* set by default and the *rarer applicable* set via the picker, while excluding the OOS and N/A types (especially `child-interest-dividends`, which is mutually exclusive with a dependent-own return). Mirror the spouse mechanism: stamp `owner_role='dependent'` + default the entered statement's `recipientTIN`/`employeeSSN` to the dependent's SSN so compute attributes it correctly, and confirm the picker's per-person offering. Consider whether statement rows need a `dependent_id` for clean multi-dependent UI filtering (today only the SSN filter disambiguates).

**★ MOSTLY DONE (2026-07-28) — the mechanism was already wired; only two gaps closed.** The per-person-kind curation already exists (`statement-selection.service.ts` `DEPENDENT_INAPPLICABLE_FORM_IDS` + `formSectionsFor(personKind)`), the dependent SSN is already defaulted into a new entry's `recipientTIN`/`employeeSSN` (`statement-entry-state.service.ts` + `person-scope.service.ts`, person-kind-agnostic), and compute attributes a dependent's statements purely by SSN via `DependentOwnFormScoper.scopedFilerSsn` — so `owner_role='dependent'` is NOT needed (and the `se_*` CHECK constraints reject it anyway; no migration). `4797`/`child-interest-dividends` were already absent from the picker. Gaps closed: (1) added `5498-sa` (HSA contributions) to the dependent-inapplicable set for consistency with the already-excluded `1099-sa` — someone claimable as a dependent can't have an HSA (IRC §223(b)(6)); (2) refreshed the stale FE drift-guard pin (`statement-selection.service.spec.ts`, 35→49 IDs — it had gone red as forms were added: w-2-gu/vi/as, 1097-btc, 1099-ls/sb, 5498-sa/esa/qa, 1098-q, 3922, 1098, 1098-c, 1042-s). Dependent-subset test extended. NOT done (deliberate/deferred): the *core-vs-rarer auto-offer split* — an earlier hard-coded 1099-int/div auto-offer for dependents was deliberately removed, so re-adding an auto-offer is a separate UX decision, not curation; `owner_role='dependent'` stamping (unneeded). Karma spec GREEN 5/5.
...
3. **Schedule 1 line-7 unemployment-repayment not in preview store — ✅ RESOLVED 2026-07-08 (V93).** Root cause: `Schedule1OutputMapper` persisted the output to Postgres via the `out_schedule_1` entity, but that entity had **no column** for `unemploymentRepaymentAmount` and the mapper never mapped it (compute→row or row→output) — so compute set it in memory (POST response = 400) but `toRow()` dropped it and GET's `fromRow()` left it null. The sibling `unemploymentCompensation` was mapped, which is why f1_12 filled but the repayment disclosure didn't. Same class of bug as V91 (combat-zone checkbox). Fix: added the `add_inc_unemployment_repayment_amount` column (`OutSchedule1` + migration `V93__out_schedule_1_unemployment_repayment.sql`, registered in `db.changelog-master.xml`) and the 3 missing `Schedule1OutputMapper` lines (write/read/reset). Applied via a full backend restart (Liquibase ran V93 on boot); **GAP-PDF-LINE7 now un-skipped and passing** (checkbox uses the field's real on-state value "/1", amount round-trips). Guarded by that e2e (the mapping lives inside Panache-backed `save()`/`load()`, so a pure unit test isn't feasible without `@QuarkusTest`, which the project avoids).
...
**What the test enforces:** every return-level ("Pattern C") mapper in `PATTERN_C_MAPPERS` must override the default `loadByTaxReturnId` (the multi-return read-scoping hook — the return-scoped analogue of `loadByPersonId` for the person-scoped Pattern A/B mappers). The check is a source-contains string match on `"loadByTaxReturnId"` in each mapper file.

**Root cause / origin:** the three offending mappers were added by the recent MFS Spouse-Forms migration owner_role-split work. Each was added to `PATTERN_C_MAPPERS` (so the invariant applies to it) but its `loadByTaxReturnId` override was not written. Consistent with the "register in N places" hazards and the per-artifact-visibility convention (each Pattern-C mapper declares its own return-scoped reader).

**The three mappers** (all in `src/main/java/com/ustax/microservices/personal/`):

| Mapper | `formIds()` | Table(s) | Origin | Notes |
|---|---|---|---|---|
| `Form8862Mapper` | `form8862`, `form8862-spouse` | parent `pf_form8862` (one row/return, owner_role split); children `pf_form8862_ctc_child` (Part III), `pf_form8862_aotc_student` (Part IV) | MFS #48 / V87 | Parent + 2 child tables — the `loadByTaxReturnId` override must re-assemble the children, mirroring its `load(...)`. |
| `OtherPayments31Mapper` | `31-other-payments`, `31-other-payments-spouse` | parent `pf_other_payments_31` (PK = uid, **no** owner_role column); child `pf_other_payment_item` | MFS #49 / V88 | Header comment says "one per return; taxpayer tab only," yet `roleFor` maps `-spouse` → SPOUSE — verify the intended scope while adding the override. |
| `CtcActcScreeningMapper` | `ctc-actc-screening-taxpayer`, `ctc-actc-screening-spouse` | owner_role-split screening table | MFS #50 / V89 | Already overrides **`loadByPersonId`** (person-scoped) but is listed in `PATTERN_C_MAPPERS` (return-scoped). Confirm the correct pattern bucket before adding — it may belong in `PATTERN_AB_MAPPERS` instead, or need both readers. |

Because all three carry owner_role (taxpayer/spouse) splits — and `CtcActcScreeningMapper` even implements `loadByPersonId` — part of the fix is confirming each is in the correct classification bucket (person-scoped Pattern A/B vs return-scoped Pattern C) rather than reflexively adding a `loadByTaxReturnId` method.

**Impact:** none at runtime today — the multi-return feature is gated off (`multi_return.feature.*=false` by default), so the default `loadByTaxReturnId` fallback is never exercised in production. The cost is (a) a red build on the full suite, and (b) when multi-return is eventually enabled, these three return-level forms would use the default reader instead of their intended scoped override.

**Fix (when picked up):** add an explicit `loadByTaxReturnId` override to `Form8862Mapper`, `OtherPayments31Mapper`, and `CtcActcScreeningMapper` following the existing Pattern-C mapper convention (mirror a sibling Pattern-C mapper already in the list). Then re-run `-Dtest=Phase3InfrastructureTest`.
...
- ✅ Lines 13a/13b (QBI + additional deductions) — audited 2026-07-08 (CONFORMS: line 13a null when no QBI workflow (record null), ZERO when a QBI workflow's deduction computes to 0 e.g. net-loss year; line 13b null when no Schedule 1-A (guarded setter), ZERO if fully phased out; both non-negative; breadcrumbs at both null-exits + lock-in tests `line13aQbiNullWhenNoQbiWorkflow` / `line13bAdditionalDeductionsNullWhenNoSchedule1A`).
- ✅ Lines 14/15 (total deductions, taxable income) — audited 2026-07-08 (CONFORMS: line 14 null only when no deduction at all (no filing status), positive floor otherwise; line 15 null when no income, ZERO by the SPEC-MANDATED floor when deductions ≥ AGI (common low-income case, `subtractNonNegative`); breadcrumb at the line-14/15 computation + lock-in tests `line15TaxableIncomeNullWhenNoIncome` / `line15TaxableIncomeZeroWhenDeductionExceedsAgi`).
- ✅ Line 16 (tax) — audited 2026-07-08 (CONFORMS: null when line 15 null — computeLine16 returns before TaxAndCredits/setTax exists; ZERO when line 15 ≤ 0 via the ZERO decision-tree branch; can be > 0 even at line 15 = 0 if a Form 8814/4972/box-3 add-on applies; breadcrumb at the line-15 null gate + lock-in tests `line16TaxNullWhenNoTaxableIncome` / `line16TaxZeroWhenTaxableIncomeZero`).
- ✅ Line 17 (AMT / Schedule 2 line 3) — audited 2026-07-08 (CONFORMS: NULL-or-POSITIVE by design — `wireLine17ToOutputs` coalesces a computed $0 to null (`amt > 0 ? amt : null`), matching the IRS "leave blank if no additional tax" convention; no ZERO branch; breadcrumb at the wiring + lock-in `line17AmtNullWhenNoAmt`).
- ✅ Line 18 (total tax before credits) — audited 2026-07-08 (CONFORMS: null when no tax computed (taxAndCredits null, line 15 was null); else = line16 + line17 (each coalesced to 0), so ZERO when taxable income is 0, positive otherwise; breadcrumb at the taxAndCredits-null guard + lock-in tests `line18TotalTaxBeforeCreditsNullWhenNoTaxableIncome` / `line18TotalTaxBeforeCreditsZeroWhenTaxableIncomeZero`).
...
## ~~Form 1040 Top-of-Form `combat_zone` Checkbox — Filing Deadline Extension Marker — Deferred 2026-05-10~~ **RESOLVED 2026-07-07**

**RESOLVED 2026-07-07 — built end-to-end.** New per-person input `servedInCombatZone2025` on the identification personal form (Personal section — always available, so it reaches users who served but have no line-1i code-Q pay, per the item's rationale). Backend: `Form1040.topCombatZone` (Boolean) set in `TaxReturnComputeService` right after `buildForm1040` — TRUE when the taxpayer OR (on a joint return) the spouse served; read via the plain `servedInCombatZone2025` key from both identification maps (MFS scopes out the other spouse → only the filing spouse counts). Administrative marker only — NO tax/credit/AGI/refund effect (Pub. 3 §7 filing-deadline-extension). Persistence: `pf_identification.served_in_combat_zone_2025` (V90 migration + changelog include), `IdentificationMapper` save/load (plain key for both taxpayer and spouse — no spouse-prefix, with a new `coalesceBoolean` helper), `PfIdentification` column. Frontend: Rule-7-compliant Yes/No radio group on both identification components (taxpayer + spouse) mirroring the address-form pattern, `TaxpayerIdentificationPayload`/`SpouseIdentificationPayload` typed field, help-modal entry, and the 1040 view wires `values['top_combat_zone'] = form.topCombatZone === true` (semantic field already present in `f1040_fields.json`, renders the checkbox + Save-as-PDF). YAML specs added to both identification forms. **Validation:** 4 backend unit tests + **5 e2e tests** (POST round-trip, GET-reload round-trip, none-served, spouse-MFJ plain-key persistence, UI-preview-render) — full backend suite **1336/1336**; frontend AOT build green; backend boots clean with V90+V91; screenshot confirms the "⊠ Combat zone" box renders checked. **★ Reload-path fix (V91):** the 1040 preview loads via `GET /api/tax-return`, which reconstructs `Form1040` from `out_form_1040` via `Form1040OutputMapper` — a distinct path from the compute POST that the first 3 e2e tests didn't exercise; without persisting the field the box rendered unchecked after reload (same class as the Form 8862 reload gotcha). Added `out_form_1040.top_combat_zone` (V91) + `OutForm1040` column + mapper save/load, plus GET-reload + UI-render e2e lock-ins.

## ~~Form 1040 Top-of-Form `combat_zone` Checkbox — Filing Deadline Extension Marker — Deferred 2026-05-10~~ (original entry below)
...
### ✅ Admin/Support UI — support-dashboard route + role guard BUILT 2026-07-14 (message-compose folded in)

~~Per user request, the **frontend admin surface is parked**~~ **BUILT 2026-07-14.** The role-gated support-staff route now exists end-to-end:
- **`/admin/support` route** (`SupportDashboardComponent`) — status-filter chips (open / in_progress / resolved / all → `GET /api/admin/support-requests?status=`), a request table, a detail panel (subject / full message / contact snapshot), a **triage action** (`PATCH` status + internal response note), and a **reply-to-inbox** compose (subject + body → `POST /api/admin/messages` to the requester's uid — this folds in the old "message-compose UI" item). Live triage moves the row out of a now-mismatched filter.
- **`supportRoleGuard`** (`auth/support-role.guard.ts`) — requires auth + the `support` role (via `RoleService` → `GET /api/roles/me`); a signed-in non-support user is bounced to `/app`, unauthenticated to `/auth/sign-in`; **fails closed** on any role-read error. Paired with `authGuard` on the route so the role round-trip only runs on admin routes.
- **Angular `RoleService`** — caches `GET /api/roles/me` per session (`isSupport`/`roles`/`loaded` signals, `ensureLoaded`/`refresh`/`reset`); `SupportAdminService` wraps list/triage/reply.
- **Topbar nav** — a support-only "Support dashboard" menu item (rebuilt once roles resolve; hidden for regular filers, cleared on sign-out).
- **Tests:** `support-role.guard.spec.ts` (3) + `support-dashboard.component.spec.ts` (9) Karma + `admin-support-dashboard-ui.spec.ts` (3 e2e: non-support redirect, support triage-to-resolved flow, nav-item gating). UI build green.
- **Still deferred (minor):** a dedicated re-usable "compose to an arbitrary uid" admin screen beyond the per-request reply (support staff can hit `POST /api/admin/messages` directly); the payment card-processor + header-dialog test residuals tracked in their own sections.
...
**Display/persistence — DONE 2026-07-10.** `Form8615` is now threaded into the `TaxReturnComputation` record (66th field, before `flags`; all 5 positional construction sites updated — production assembly, `TaxReturnDataService` load path, `MicroserviceModelsTest`, 3× `TaxReturnResourceTest`) and persisted via `OutForm8615` (`out_form_8615` table, V107) + `Form8615OutputMapper` (save/load all 18 lines + `manualOverrideUsed`), wired into `TaxReturnDataService` save + load. `computeForm8615` moved to `prepare()` (before `computeLine16`, which now takes it as a param). Healthy boot ran V107 and passed schema validation; `TaxReturnComputeServiceTest` 965/965 + `MicroserviceModelsTest`/`TaxReturnResourceTest` green. The computed worksheet now survives a GET reload and is available to a preview. **No UI change** (per direction — the presentation layer is handled separately; note there is not yet a `form-tax-return-8615` preview component, though the f8615 semantic assets exist in `pdfs/` + `public/irs/`).

**Preferential-rate paths — ✅ DONE 2026-07-11.** Lines 9/15/17 now use the Qualified Dividends and Capital Gain Tax Worksheet (0%/15%/20%) when the child has qualified dividends or net long-term capital gain — the usual kiddie-tax trigger — otherwise the ordinary bracket path (unchanged). The child's preferential income is derived from the child's own computed return (`childPreferentialIncome` = qualified dividends line 3a + net LTCG from Schedule D / line 7a); no new input fields. The 0%/15%/20% stacking was extracted from `computeQDCGWorksheet` into a shared `computePreferentialRateTax(taxableIncome, filingStatus, preferentialIncome)` (QDCG delegates to it — behavior-preserving, full suite 1451/1451). Form 8615 layers the right slice on each line: **line 9** taxes the preferential portion pulled into net unearned income (`min(line5, childPref)`) at the parent's preferential rate, with the parent's line-6 income treated as ordinary (cancels in line 11 = line 9 − line 10 when the parent has no preferential income); **line 15** taxes the child's remaining preferential income (`childPref − prefInLine5`); **line 17** taxes all of `childPref`. Unit test `line16Form8615PreferentialRateOnQualifiedDividends` (child $20k wages + $10k qualified dividends, parent $200k Single → line 13 = $1,095 at the parent's 15% rate, line 18 = $1,523 vs ~$3,034 ordinary).

**Parent's own preferential income — ✅ DONE 2026-07-11 (auto-derived, no manual field).** The parent's own qualified dividends + net capital gain now feed Form 8615 line 9 and line 10 (line 10 uses the QDCG worksheet on the parent's income instead of the ordinary bracket), so line 11 is exact when the parent has preferential income and the child's net unearned income has an ordinary component. Because the filer is never themselves the kiddie-tax child (kiddie tax only arises on the `dependent_own` path, where the parent IS the household's computed return), the value is **auto-derived** — no input field. `KiddieTaxParentReader` now returns the parent's preferential income (`ParentReturnInfo.preferentialIncome`, via `preferentialIncomeOf(Form1040, ScheduleD)` mirroring the child-side logic) alongside taxable income + filing status; `DependentOwnFormScoper` seeds it onto the synthetic kiddie form as `parentQualifiedDivCapGainLine6`; `computeForm8615` reads it (`parentPref`, capped at line 6). Tests: `line16Form8615PreferentialRateParentOwnCapitalGains` (compute math → line 13 $3,432 vs $4,439 ordinary), `preferentialIncomeOf` + scoper-seed unit tests, and e2e `dependent-own-kiddie-tax.spec.ts` (parent $200k wages + $40k qualified dividends → Form 8615 line 6 = $224,250, line 10 = $43,067 QDCG vs $48,823 bracket). Full suite 1458/1458.

**28%/§1250 Schedule D Tax Worksheet — ✅ DONE 2026-07-11 (child side).** `computePreferentialRateTax` was generalized to a 5-arg form taking explicit 28%-rate-gain and unrecaptured-§1250 buckets (0/15/20 regular preferential stacked at bottom, §1250 at min(25%, marginal), 28% at min(28%, marginal) — the Schedule D Tax Worksheet rate order; the 3-arg overload = plain QDCG, behavior-preserving). `computeForm8615` derives the child's Schedule D line 18/19 amounts and apportions them (proportionally to `prefInLine5/childPref`) across lines 9/15/17. Unit test `line16Form8615PreferentialRate28PercentCollectibleGain` (child $10k collectible gain → line 13 = $2,044 at the parent's rate capped at 28%, vs $1,095 at the regular 15%; line 18 = $3,843). **Deferred (rare-rare):** the parent's own 28%/§1250 (cancels in line 11 when line 9/10 treat it identically) and the *siblings'* 28%/§1250 (line 7 preferential is regular-only).

**Other affected children's preferential income (Form 8615 line 7) — ✅ DONE 2026-07-11 (auto-derived).** This was a real correctness gap, not just polish: on the `dependent_own` path line 7 was **0** (siblings ignored entirely), understating multi-child kiddie tax. `KiddieTaxParentReader.getSiblingKiddieInfo(uid, currentDependentId)` now enumerates the other `dependent_own` returns, computes each (with a `ThreadLocal` recursion guard so sibling→sibling reads don't loop — a sibling's line 5 is independent of its own line 7), and sums their net unearned income (line 5) + the preferential portion `min(line5, sibPref)`. `DependentOwnFormScoper` seeds `otherChildrenNetUnearnedLine7` + `otherChildrenPreferentialLine7`; `computeForm8615` folds them into line 8 and the line-9 preferential income. Unit tests: scoper line-7 seed (present/omitted) + `getSiblingKiddieInfo` shape; full suite 1464/1464. **E2E now GREEN** (`dependent-own-kiddie-tax.spec.ts`): a 2-child household → child A's Form 8615 line 7 equals child B's line 5 (sibling pool auto-derived, not 0). This became testable once the two pre-existing dependent_own scoping bugs below were fixed the same day.

**Pre-existing dependent_own scoping bugs — ✅ FIXED 2026-07-11 (surfaced while building the multi-child e2e).**
- **Dependent standard deduction.** A dependent filing their own return was getting the FULL standard deduction ($15,750) instead of the dependent-limited one (`max($1,350, earned income + $450)`, capped) — a systematic under-taxation of every dependent_own return (a child with $10k interest showed $0 taxable income instead of ~$8,650). Root cause: `DependentOwnFormScoper` never seeded `someoneCanClaimYou`, which gates the dependent worksheet (a dependent-own filer is by definition claimable). Fix: the scoper now seeds `standard-deductions-taxpayer` with `someoneCanClaimYou = true` (earned income auto-imports from the computed line 1z wages).
- **Form 8814 leak.** The child's scoped compute still auto-built Form 8814 (parent's election to report a child's income) from the household's 1099-INT/DIV + dependents list — firing the non-overrideable `FORM8814_CHILD_GROSS_INCOME_TOO_HIGH` blocker on the child's OWN return when a child's gross income ≥ $13,500. Fix: the scoper marks `filing-status.dependentOwnReturn = true` and `prepare()` skips `computeChildInterestDividends` on that path. (Form 8814 remains available on primary/MFS returns.)
- Verified: full suite 1464/1464 + `dependent-own-basic`/`-eic-ineligible`/`-kiddie-tax` e2e all green.

**Remaining follow-ups (deferred):**
- **Cross-return auto-fill (taxable income + filing status)** — ✅ already done: `DependentOwnFormScoper` auto-seeds `parentTaxableIncomeLine6` + `parentFilingStatus` from `KiddieTaxParentReader` on the `dependent_own` path.
...
| Schedule 8812 sidebar always visible | `shell.component.ts` (`buildTaxReturnItems`, ~line 1414) | **WON'T-FIX-IN-ISOLATION — premise corrected 2026-07-08.** The item said "move to conditional like Form 8880", but Form 8880 is ALSO always-visible: it and Schedule 8812, Form 4868, Form 8959, Form 8862, Form 2210 (+ Form 1116 via an `else` placeholder) are ALL deliberately kept always-visible via the same TEMP-toggle convention `if (true /* <conditional> */)`. Each form's conditional-hide condition is preserved in the inline comment. This is an intentional, section-wide "show every Tax Return form in the sidebar regardless of compute data" choice — NOT an incomplete Schedule 8812 migration. Flipping only Schedule 8812 to conditional would make it inconsistent with the rest of the section. The correct scope is a single section-wide decision (flip all the `if (true)` toggles to their conditions at once) — a deliberate UI-behavior change to make, not a per-form bug. No e2e depends on any of these sidebar items. |
...
- ~~[Form 8863 / MAGI adjustments]~~ **RESOLVED / premise corrected 2026-07-08.** Form 8863 MAGI is NOT "AGI directly" — `computeForm8863()` computes `magi = AGI + Form 2555 exclusion (line 45+50) + Puerto Rico excluded income + Form 4563 (American Samoa) excluded income`, exactly the three add-backs the 2025 Form 8863 line-3 MAGI worksheet lists. The original item's claim that **student loan interest** should adjust MAGI is IRS-incorrect: education-credit MAGI never adds back or subtracts student loan interest (AGI already reflects the deduction; the worksheet lists only the three foreign exclusions). No code change; item was based on a wrong premise. See `[[computeForm8863]]` + tests below.
- ~~[Form 8863 / MAGI auto-read from Form 2555 output]~~ **RESOLVED — stale (verified 2026-07-08).** The Form 2555 → Form 8863 MAGI bridge IS implemented: `computedForm2555AddBack = computeForm2555ExclusionForSsWorksheet(taxpayer) + (MFJ ? spouse : null)` auto-reads the computed exclusion and is authoritative over the manual `magiAddBackForm2555ExcludedIncome`/`magiAddBackForm2555HousingExclusion` fields (which remain a fallback only when no Form 2555 is computed). Because the exclusion is netted out of AGI at Schedule 1 line 8d, the add-back restores exactly the computed exclusion (undoing the netting). Covered by unit tests `computeForm8863_form2555AddBack_autoFilledFromForeignEarnedIncomeWhenManualBlank` (add-back pushes MAGI over the $90k ceiling) and `computeForm8863_form2555AddBack_computedExclusionWinsWhenForm2555Present` (computed $20k beats manual $5k). This row predates that implementation.
...
- ~~[Line 12e / Dependent worksheet earned income not auto-imported]~~ **FULLY RESOLVED — the full formula is implemented + tested (12e.md Gap 1 closure 2026-06-08; verified/doc-synced 2026-07-08).** The earlier "partially resolved 2026-04-17" note (line-1z-only proxy) is superseded: `computeDependentWorksheetEarnedIncome(income, otherIncome, incomeAdjustments)` now sums the full per-spec earned income = line 1z (wages) + Schedule 1 line 3 (business) + line 6 (farm) + line 8r (scholarship/fellowship grants not on a W-2) + line 8t (nonqualified deferred comp) + line 8u (wages while incarcerated) − line 15 (deductible SE tax), floored at 0, and injects it into the worksheet when the manual `dependentStandardDeductionEarnedIncome` override is null. Matches the 2025 Form 1040 Dependent Standard Deduction Worksheet earned-income definition (Pub 501). Tests: `computesLine12DependentWorksheetAutoImportsWagesWhenEarnedIncomeNotEntered` (wages) + `dependentWorksheetEarnedIncomeIncludesScholarshipGrantsLine8r` (W-2 $500 + 8r $3,000 → std ded $3,950, not the $1,350 minimum).
...
- ~~[Form 1116 / high-tax kickout] Passive income with an effective foreign tax rate above the highest US bracket should be reclassified to general category. Not implemented.~~ **DONE 2026-07-13.** `applyPassiveHighTaxKickout` runs per-country before the category merge: a passive country whose effective foreign tax rate (foreign taxes ÷ net foreign taxable income) exceeds the highest US individual rate (37%, `HIGHEST_US_INDIVIDUAL_RATE_2025`) is split out into a general-category entry; low-taxed countries stay passive (§904(d)(2)(F) / Reg. §1.904-4(c)). Unit `form1116HighTaxKickoutReclassifiesPassiveToGeneral` + e2e `form1116-foreign-tax-credit.spec.ts` Test 4.
...
- **Form 1116 adversarial audit 2026-08-05 — FIXED (4) + confirmed-DEFERRED (below).** FIXED (see history.md): line-18 senior-deduction add-back (§151(d)(5)(C) Schedule 1-A line 37 → §904 denominator); §901(j) sanctioned-country credit forced to $0 + no carryover; >3-countries-per-basket truncation removed (display-only limit); category checkbox letters e/f/g corrected (e=§901(j), f=treaty, g=lump-sum). Still DEFERRED (all confirmed by the audit, verified vs i1116.pdf):
  - **[§904(b)(2)(B) QDCG rate adjustment]** — foreign qualified dividends / net capital gain are NOT scaled by the 0.4054/0.5405 (line 1a) factors and the *Worksheet for Line 18* denominator adjustment is not run; the &lt;$20,000 adjustment-exception gate is never evaluated. Over-credits filers with material foreign QDCG above the exception. Self-documented in code (`buildFullForm1116` line-17/18 comment).
  - **[Line 3a–3g deduction apportionment]** — the standard deduction (or itemized) and other not-definitely-related deductions are not ratably apportioned to foreign income by the gross-income ratio (line 3g omitted). Over-credits nearly every full-path standard-deduction filer. (Exclude the senior deduction from line 3b per i1116; car-loan interest → line 4b.)
  - **[§911 FEIE coordination]** — no automatic reduction of line-1a foreign income for §911-excluded wages, and no §911(d)(6) disallowance of foreign taxes allocable to excluded income. Relies on the user entering post-exclusion figures. Over-credits a FEIE + foreign-wage filer.
  - **[De-minimis gate hardening]** — the ≤$300/$600 simplified exception enforces only the dollar threshold + user checkbox; it does not enforce the passive-only / qualified-payee-statement / non-§901(j) / non-Form-4563 conditions, and hardcodes category "passive". The collected `confirms1099DivUploaded`/`confirms1099IntUploaded` attestations are unread.
  - **[Imported-1099 on the full path]** — 1099-DIV box-7 / 1099-INT box-6 foreign taxes feed ONLY the simplified branch; a filer over the threshold (required to use the full form) whose tax comes from 1099 imports gets $0 unless they re-key it into `foreignIncomeSources`. Silent under-credit (common passive case). NOTE: also an opt-in-gating question — the whole FTC is behind `hasForeignTaxCredit`, so a small 1099 foreign tax is dropped unless the user opens the FTC form (a product/UX decision, not auto-fixed).
  - **[Line 20 − Form 4972]** — line 20 (limitation base) does not subtract lump-sum tax from Form 4972 included in line 16; category-g line 20 is not special-cased. Edge (Form 4972 filers), over-credit.
  - **[MFS-leg 1099 scoping]** — PLAUSIBLE: the simplified-path 1099 aggregation may not be SSN-scoped per MFS leg (the per-spouse `simplifiedForeignTaxesOverride` avoids it when populated). Verify before relying on the simplified fallback on an MFS leg.
- ~~[Form 1116 / MFS spouse simplified-exception path]~~ **RESOLVED 2026-06-23 (full mirror, same-day #42 follow-up).** The spouse Form 1116 component/model now mirror the taxpayer form — added `spouseClaimsSimplifiedException`, `spouseSimplifiedForeignTaxesOverride`, `spouseConfirms1099DivUploaded`, `spouseConfirms1099IntUploaded`. The mapper persists them on the spouse `owner_role` row's (otherwise-unused) bare scalar columns — NO migration. The existing `unPrefixSpouseKeys` scoper case already maps them onto the bare keys the compute reads, so on her MFS leg the spouse can elect the $300 simplified exception (skip Form 1116) on her own separate return. Verified: `mfs-spouse-foreign-tax-credit.spec.ts` adds a test (spouse simplified $250 → credit $250, `simplifiedExceptionUsed=true`); `form1116-foreign-tax-credit` 3/3 unchanged.
...
- ~~[Form 8962 / MFS standalone spouse-leg repayment] **Added 2026-06-23 (MFS migration Form #39).**~~ **DONE 2026-07-13 (#17, full mirror).** The `premium-tax-credit-spouse` form is now a full standalone mirror (no migration — the spouse row already had all taxpayer-scoped columns): the mapper stores her full PTC set from `spousePtc*` keys + shared-policy child rows; `MfsFormScoper.normalizePremiumTaxCreditSpouse` un-prefixes it onto the taxpayer slot on her `mfs_spouse` leg (dropping only when she filled nothing standalone); the compute filters 1095-A statements by `recipientSSN` per leg so her Form 8962 uses her own policy. UI: a "Separate return with her own Marketplace coverage (MFS)" section on the shared PTC form. Scoper unit test + e2e `mfs-spouse-premium-tax-credit.spec.ts` (head $15k vs spouse $8.4k → her leg uses $8.4k). **Remaining refinements (rarer sub-edges):** her own Form 2555 MAGI add-backs, alternative-calculation-for-year-of-marriage, and dependent-MAGI rows are not yet mirrored on the standalone spouse leg (the core gate + FPL + MFS exception + 1095-A + shared-policy are).
...
- ~~[Form 8859 / MFS spouse Worksheet B override field] **Added 2026-06-23 (MFS migration Form #38).**~~ **DONE 2026-07-13.** Added `spouseUseSchedule8812CreditLimitWorksheetBOverride` + `spouseSchedule8812CreditLimitWorksheetBLine14Override` (V126) + entity + `CarryforwardHomebuyerCreditMapper` + `MfsFormScoper.normalizeCarryforwardHomebuyerCreditSpouse` un-prefix onto the bare override keys + two MFS-only override fields on `form-carryforward-homebuyer-credit`. Scoper unit test `mfsSpouseUnPrefixesCarryforwardHomebuyerWorksheetBOverride`. (Original note below.) The spouse form (`carryforward-homebuyer-credit-spouse`) collects only the gate + carryforward amount; it has no equivalent of the taxpayer form's optional `useSchedule8812CreditLimitWorksheetBOverride` / `schedule8812CreditLimitWorksheetBLine14Override` line-2 override. On the spouse's MFS leg her Form 8859 now computes correctly (scoper remaps her gate → master claim + her carryforward → the bare line-1 key), and line 2 is auto-computed from her own leg's tax-before-credits − CTC/ODC − other credits. The override matters only in the rare case where Schedule 8812 directs Credit Limit Worksheet B (CTC/ACTC interplay) on her separate return; absent the field her line-2 limit uses the default CTC instead of the override. Cosmetic for the common case (no dependents / line-2 capacity ≫ carryforward). Fix when polishing: surface the two override fields on the spouse form MFS-only + map them in `MfsFormScoper.normalizeCarryforwardHomebuyerCreditSpouse` (mirrors the #37 Form 8396 MFS-only-core approach). Low priority.
...
- ~~[Line 16 / Box 3 / 1291TAX] Form 8621 line 16e PFIC section 1291 tax — requires Form 8621 implementation.~~ **DONE 2026-07-13 (#18 regime 2).** `computeForm8621Section1291Tax`: the excess distribution allocated to prior PFIC years × 37% (flat highest ordinary rate, accurate for 2018+ holding periods) → Form 8621 line 16e → line 16 Box 3 code "1291TAX"; the manual line-16e amount stays as a fallback. V125 (1 column) + entity + mapper + compute + UI (§8621 section on the Line 16 tax form). Unit + e2e ($50k × 37% = $18.5k). **Simplifications:** the user does the ratable per-day holding-period allocation; the flat 37% approximates each prior year's highest rate (exact for 2018+); the §6621 interest charge (line 16f) stays user-entered on Schedule 2 line 17p (compounded quarterly interest back-years needs a historical §6621 rate table).
- ~~[Line 16 / Box 3 / Form 8978] Form 8978 line 14 BBA partnership push-out adjustment tax — requires Form 8978 implementation.~~ **WON'T-IMPLEMENT (correct as user-entered) 2026-07-13 (#18 regime 3).** Line 14 is the aggregate change in tax across the reviewed years, which requires reconstructing each prior year's return WITH and WITHOUT the pushed-out partnership adjustments. The app does not persist prior-year returns, so the delta cannot be recomputed — same root cause as Form 8801 line 12. The user computes line 14 on their Form 8978 worksheet; the app routes it (+ → line 16 Box 3, − → Schedule 3 line 6l). Documented with a code breadcrumb.
- ~~[Line 16 / Box 3 / 965INC] Section 965(i) triggered deferred net tax liability — requires Form 965-A implementation.~~ **WON'T-IMPLEMENT (correct as user-entered) 2026-07-13 (#18 regime 4).** The triggered amount is the net §965 transition-tax liability computed in the 2017/2018 transition year and deferred under §965(i) for an S-corp shareholder; a triggering event makes that fixed historical figure due. §965 is a one-time transition tax — not a current-year computation — so recomputing it in 2025 would be non-conforming. The user enters the triggered net liability from their §965(i) records. Documented with a code breadcrumb.
...
- ~~[Form 2555 / housing cap] Country-specific housing exclusion caps are approximated at 30% of the annual exclusion. Implement the IRS location-specific ceiling table for accurate country-by-country housing caps.~~ **DONE 2026-07-13 (curated subset).** V123 adds `foreign_housing_high_cost_locality` + a high-cost-locality dropdown; `FORM2555_HIGH_COST_HOUSING_LIMITS_2025` (30 major localities from IRS Notice 2025-16, e.g. Hong Kong $114,300 / Geneva $102,600 / Moscow $108,000) replaces the $39,000 base cap when a listed locality is selected (prorated by qualifying days). Unlisted localities fall back to the base cap (conservative under-claim, unchanged behavior). Unit `form2555HousingExclusionUsesHighCostLocalityLimit` / `...FallsBackToBaseCapForUnlistedLocality` + e2e.
  - **⚠️ ANNUAL REVISION REQUIRED.** The IRS republishes the high-cost-locality housing limits every year in a new Notice (2025 = Notice 2025-16; find the next year's via an irs.gov search for "Determination of Housing Cost Amounts Eligible for Exclusion"). Each tax year, update BOTH `FORM2555_HIGH_COST_HOUSING_LIMITS_2025` in `TaxReturnComputeService` AND `HIGH_COST_LOCALITY_OPTIONS` in `form-foreign-earned-income.component.ts` (same "Country - Locality" keys + refreshed dollar amounts + the year suffix), and the base cap ($39,000 = 30% × the new FEIE limit). The current list is a curated subset of the ~400-locality table — consider expanding coverage when revising.
...
- ~~[Line 8 / Line 8d Form 2555 exclusion not auto-wired] `computeOtherIncomes()` reads `otherIncomeForeignEarnedIncomeExclusion8d` from manual user entry only...~~ **RESOLVED — stale duplicate of the "[Line 8d / Form 2555 exclusion not auto-wired] Fixed 2026-04-16" entry below (verified 2026-07-07).** The proposed fix is implemented: `computeOtherIncomes()` already reads the manual `otherIncomeForeignEarnedIncomeExclusion8d` AND calls `computeForm2555ExclusionForSsWorksheet(foreignEarnedIncomeTaxpayer/Spouse)` as the auto-fallback (no pipeline reordering was needed — the exclusion is computed inline, not read from `populateForm2555Taxpayer`'s later output). This bullet was the unstruck twin of the same gap.
...
- [Form 2441 / earned-income inputs] **Partially resolved 2026-07-08** — Form 2441 lines 18/19 earned income now includes computed per-person tip income (line 1c) per IRC §21(d)(1), in addition to W-2 box-1 wages, the manual `additionalEarnedIncome*` field, and student/disabled deemed income. Per-person line-1c is surfaced on `TipComputation` and added to `taxpayerEarned`/`spouseEarned` in `computeDependentCareBenefits` (MFS spouse excluded). 2 unit tests + 1 e2e. **Remaining:** Form 8919 uncollected wages (line 1g) and other earned income (line 1h) are still captured only via the manual `additionalEarnedIncome*` field — automatic per-person inclusion is deferred because `OtherEarnedIncomeComputation.line1h` exposes only a return-level total (no per-person split) and `computeOtherEarnedIncome` runs after the Form 2441 call in compute order.
...
- ~~Fix Auth user state mutation path: call `firebaseAuth.updateUser(...)` in `disableUser`/`enableUser`, then add assertions in `AuthServiceTest`.~~ **MOOT — resolved by removal (verified 2026-07-08).** `AuthService.java` (with `disableUser`/`enableUser`) and `AuthServiceTest` were deleted on 2026-04-28 (see `us-tax-be/src/main/java/com/ustax/microservices/context.md`) — orphan email/password admin code with zero callers, removed during the phone-OTP migration. Nothing left to fix.
- ~~Protect admin-like auth endpoints: require authenticated/authorized access for `/auth/users/disable` and `/auth/users/enable`.~~ **MOOT — resolved by removal (verified 2026-07-08).** `AuthResource.java` (`POST /auth/users`, `/auth/users/disable`, `/auth/users/enable`) was deleted on 2026-04-28 (context.md). The unprotected endpoints no longer exist, so the security concern is closed by deletion. Both auth items predate the 2026-04-28 removal and were never re-introduced (no `AuthResource`/`AuthService` in the codebase today).
- Remove duplicate compute work: consolidate flags+compute flow to avoid running `prepare(uid)` twice per user action. **Investigated 2026-07-08 — confirmed real, deferred (marginal ROI + core-flow risk).** The Compute button (`app.topbar.ts::onComputeReturn`) calls `taxReturnService.loadFlags()` → `GET /tax-return/flags` → `prepare(uid)` (pass 1, to preview blocking flags for the confirm dialog), then `computeAllAndOptimize()` → `POST /tax-return/compute` → `prepare(uid)` (pass 2, computes + persists). So `prepare()` runs ≥2× per click. **Fix**: drop the separate `/flags` preview and drive the blocking-flag dialog from the `/compute` 409 response instead (the compute endpoint already returns the flags in its 409 body / 200 `computation.flags()`), then retry `/compute?overrideFlags=true` on "compute anyway". That saves exactly one `prepare()` on the happy path. **Why deferred**: (1) `computeAllAndOptimize` already runs the optimizer, which itself calls `prepare()` once per filing-status scenario — so the extra `/flags` prepare is ~1 of N+2 per click, a marginal fraction; (2) the change moves blocking-flag handling from a pre-compute preview to a 409-catch flow, altering a core user-facing UX path and its error handling, which needs new e2e coverage for the compute-button + blocking-flag + override-retry sequence. Low ROI relative to that risk; revisit if the optimizer is made lazy or the compute path is otherwise reworked.
...
- Prevent FE/BE statement-form drift: centralize/generate statement form catalog shared by UI and backend. **Investigated 2026-07-08 — the apparent "drift" is INTENTIONAL; drift-guard pins added.** Diffing the backend `StatementFormCatalog` (42 forms) against the FE `statement-selection.service.ts` (38) showed 4 BE-only forms: **4684 (Casualties/Thefts), 4797 (Sales of Business Property), 6781 (§1256 Contracts/Straddles), 8824 (Like-Kind Exchanges)**. ★ I first (wrongly) treated this as accidental drift and exposed them in the FE — but an API-level e2e smoke then FAILED at statement-entry *creation* ("no StatementMapper registered"): these forms' `Form{4684,4797,6781,8824}Mapper.supports()` return `Set.of()` — **INERT** (persistence disabled per the mappers' own comments, "flip back … when the UI ships a semantic-key translation layer"). So they can be *read* (generic compute read) but not *created/saved* — offering them would let users pick forms that 500 on save. The FE omission was CORRECT; my exposure was reverted (us-tax-ui). ★ The e2e verification caught this before it shipped a broken UX. **Also flagged:** Form 6252 is offered in the FE picker yet its `Form6252Mapper` is likewise inert — a pre-existing latent issue (offered-but-unsaveable) to confirm separately. **Drift-guard pins added (both sides, kept):** FE `statement-selection.service.spec.ts` pins the 38-form picker set; BE `StatementFormCatalogTest` pins the 42-form catalog — the two are intentionally NOT equal (the FE is the persistable subset); each pin makes its own list's edits deliberate. **The item's ORIGINAL intent (a shared/generated single-source catalog) remains open**, and would now also need to encode which forms are actually persistable (mapper-active) vs catalog-only. **Real follow-up to make these 4 usable:** activate their StatementMappers (semantic-key translation), then re-expose in the FE + add e2e (tracked with "[Capital forms E2E coverage]"). **Deferred by design (2026-07-08, per project owner): the Statements section is still under active development.** The capital-statement FE configs in `us-tax-ui/src/app/forms/capital-statement-configs.ts` for `2555/4684/4797/6252/6781/8824/8853/8889` still use raw PDF-acroform slot keys (`page1_0_f1_1_0`), and the typed mappers for `4684/4797/6252/6781/8824` (semantic keys) + the three K-1 mappers are inert — all STATEMENT-side, to be finished as part of the ongoing statements work. Leave as-is for now. NOTE: this raw-slot situation is confined to the Statements section — the Tax Return forms are fully migrated to semantic names (source of truth: the pure-HTML/CSS `form-tax-return-*` components + the `C:\us-tax\pdfs\*_field_map_semantic.csv` assets).
...
- ~~**[Line 23 / G4 / MEDIUM] Net investment income tax (Form 8960 → Schedule 2 line 12) not implemented.**~~ **RESOLVED — stale (verified 2026-07-07).** Form 8960 IS implemented: `computeForm8960(...)` + `applyForm8960ToSchedule2(...)` (TaxReturnComputeService), a `Form8960` output model, wired into `TaxReturnComputation`, with ~28 test references (e.g. `form8960HonorsTaxpayerEnteredLine9bStateLocalTaxOverride`) covering the 3.8% NIIT threshold/MAGI logic. This audit row predates that implementation.
...
- **[Form 1040 / Cleanup / LOW / ~~OPEN~~ RESOLVED 2026-07-07]** ~~Delete `us-tax-ui/public/irs/f1040_field_mapping_semantic.csv`.~~ **DONE 2026-07-07.** Deleted the served CSV. Verification corrected the entry's premise: the file WAS still referenced at runtime by `pdf-readonly-preview.component.ts` (`getCsvUrl()` f1040 special-case) — but the whole `PdfReadonlyPreviewComponent` is now **unmounted dead code** (nothing imports/mounts it; only past-tense comments in migrated components mention it), so the reference was dead too. Removed the dangling f1040 branch in `getCsvUrl()` alongside the delete. `tsc --noEmit` clean. The canonical `C:\us-tax\pdfs\f1040_field_mapping_semantic.csv` is retained. **Follow-ups:** (a) ~~the entire `PdfReadonlyPreviewComponent` is dead and could be removed wholesale~~ **DONE 2026-07-07** — deleted the component (no template mounts its selector, nothing imports the class or its `ExtraTextOverlay` type, not in any module/imports array; `tsc --noEmit` clean after removal). (b) ~~`f1040_semantic_labels.pdf` + `irs-images/f1040_semantic_labels/` PNGs were NOT deleted~~ **DONE 2026-07-07** — deleted both (407 KB PDF + two ~470 KB PNGs, ~1.3 MB total). Confirmed orphaned: no `f1040_semantic_labels` or `irs-images` reference anywhere in `src/`; the only dynamic `*_semantic_labels.pdf` consumer was the (now-deleted) `PdfReadonlyPreviewComponent`; live components hardcode their own form's PDF (none is f1040); the 1040 view renders from JSON vector assets (`f1040_vector_pages.json` + `f1040_fields.json`), not these. This entire L2860 cleanup item is now fully closed.
...
- **Increment 2a — intake persistence — DONE 2026-07-11.** New `PfForm8889Hsa` entity (`pf_form_8889_hsa`, V108) + `Form8889HsaMapper` (formIds `hsa-taxpayer`/`hsa-spouse`, flat per-person) + registered in `PersonalResource.PERSONAL_FORMS` + `UserDataBulkDelete.PARENT_TABLES_UID_CASCADE` + the changelog master. ~14 HSA input fields (coverage type, contributions L2, Archer L4, months-eligible/last-month-rule for L3, family allocation L6, employer L9, funding L10, rollovers L14b, qualified medical L15, 20%-exception, Part III failure incomes L18/L19). Healthy boot ran V108 + passed schema validation (entity ↔ table match; mapper CDI-discovered; allow-list registered).
- **Increment 2b — service wrapper + Schedule wiring — DONE 2026-07-11.** `computeForm8889(hsaForm, 1099-SA entries, personSsn, dob, uid)` resolves the form inputs, sums line 14a from the person's HSA-flagged 1099-SA (read-only, `sum1099SaHsaDistributionsForSsn`), derives the age-55 catch-up from DOB (via `computeAgeAtYearEnd ≥ 55`). Computed for taxpayer + spouse in `prepare()`; the results are injected into the existing input maps **only when the manual field is absent** (`injectHsaDeduction` → line 13, `injectHsaOtherIncomeLine8f` → line 8f = line 16 + line 20, `injectHsaSchedule2Taxes` → return-level 17c = Σ line 17b / 17d = Σ line 21). Null input maps are created; MFS spouse-shadowing is handled by the downstream methods' own guards. Unit test `form8889_endToEnd_wiresToSchedule1And2` (HSA form + 1099-SA → Sch 1 line 13 $3,000, line 8f $500, Sch 2 17c $100). `TaxReturnComputeServiceTest` 969/969.
- **Increment 2c — Angular HSA form — DONE 2026-07-11.** New standalone `form-hsa.component.ts` (single component, `formId` input for `hsa-taxpayer` / `hsa-spouse`) with a screening gate + Part I/II/III sections (~14 fields, Part III behind a collapsed `<details>`), loading/saving via `PersonalDataService.getForm`/`saveForm`. Wired into `shell.component.ts` (import + `imports:` array + `*ngIf` routes + two sidebar items "HSA (Form 8889)" next to Income adjustments; per-person filtering via the `-taxpayer`/`-spouse` suffix). NOTE: a `form-tax-return-8889` **preview** already exists (pure-HTML) and was left untouched per the no-tax-return-form-modification constraint; this is the separate data-entry intake form.
- **Increment 3 — output persistence + reload — DONE 2026-07-11.** `Form8889` threaded into the `TaxReturnComputation` record (2 fields, taxpayer + spouse; all 5 positional construction sites updated) + persisted via `OutForm8889` (`out_form_8889`, one row per (uid, owner_role), V109) with an `AbstractForm8889OutputMapper` + `Form8889Taxpayer/SpouseOutputMapper` concrete pair (mirrors the Form 8606 per-person pattern) wired into `TaxReturnDataService` save + load. Healthy boot ran V109 + passed schema validation (no mapper-conflict warning). `TaxReturnComputeServiceTest` 969/969. The computed worksheet now survives a GET reload (the existing `form-tax-return-8889` preview can render it).
- **Increment 4 (next) — e2e.** A Playwright spec seeding the HSA form + a 1099-SA and asserting the computed Schedule 1 line 13 / line 8f + Schedule 2 line 17c on the return.
...
**Form 8853 (Archer MSA + LTC) — ✅ COMPLETE 2026-07-11 (Full Sections A+B+C + dedicated form).** Same 4-increment shape as Form 8889.
- **Increment 4 — e2e — DONE.** `e2e/tests/line8853-msa-ltc.spec.ts` seeds the `msa-ltc-taxpayer` form + a 1099-SA (Archer) + a 1099-LTC (per-diem) and asserts the full stack: Schedule 1 line 23 (Archer deduction) = $1,000, line 8e (taxable MSA + LTC) = $7,600. Passes.
...
- **Increment 2a+2b — intake persistence + wrapper + Schedule wiring — DONE 2026-07-11.** `PfForm8853` entity (`pf_form_8853`, V110) + `Form8853Mapper` (`msa-ltc-taxpayer`/`-spouse`, ~23 fields) registered in PERSONAL_FORMS + UserDataBulkDelete + the changelog. `computeForm8853` wrapper (line 6a from Archer-flagged 1099-SA, line 10 from MA-MSA-flagged 1099-SA, line 17 from per-diem 1099-LTC — all read-only) computed for taxpayer + spouse in `prepare()`, injected put-if-absent: line 5 → Schedule 1 line 23 (`archerMsaDeductionLine23`), lines 8/12/26 → Schedule 1 line 8e (`otherIncomeForm88538e`), line 9b → Schedule 2 line 17e, line 13b → line 17f. End-to-end unit test (msa-ltc form + 1099-SA + 1099-LTC → Sch 1 line 23 $1,000, line 8e $7,600). Healthy boot ran V110 + passed schema validation. `TaxReturnComputeServiceTest` 971/971.
- **Increment 2c — Angular MSA/LTC form — DONE 2026-07-11.** New standalone `form-msa-ltc.component.ts` (`formId` input for `msa-ltc-taxpayer`/`-spouse`) with a screening gate + Section A (Archer MSA) / Section B (MA MSA) / Section C (LTC, behind a collapsed `<details>`) sections, load/save via `PersonalDataService`. Wired into the shell (import + `imports:` array + `*ngIf` routes + two "Archer MSA / LTC (Form 8853)" sidebar items). UI builds clean.
- **Increment 3 — output persistence — DONE 2026-07-11.** `Form8853` threaded into the `TaxReturnComputation` record (2 fields, taxpayer + spouse; all 5 positional construction sites updated) + persisted via `OutForm8853` (`out_form_8853`, one row per (uid, owner_role), V111) with `AbstractForm8853OutputMapper` + `Form8853Taxpayer/SpouseOutputMapper` (mirrors `OutForm8889`), wired into `TaxReturnDataService` save + load. Healthy boot ran V111 + passed schema validation (no mapper conflict). `TaxReturnComputeServiceTest` 971/971.
- **Increment 4 (next) — e2e.** A Playwright spec seeding the `msa-ltc-taxpayer` form + a 1099-SA (Archer) + a 1099-LTC (per-diem) and asserting Schedule 1 line 23/8e on the return. **Constraint:** no `form-tax-return-8853` preview or 1099-SA/1099-LTC statement changes.
...
## Prior-year-data foundation — State/Local Refund Taxable-Portion Worksheet — ✅ **Step 1 + Step 2 COMPLETE 2026-07-11; Step 3 (unemployment §1341 repayment) DEFERRED**

Chosen by the project owner as the next foundation after the 8889/8853 forms drained the standalone-compute queue. The `prior-year-tax-taxpayer` form previously served only Form 2210 (prior-year AGI + tax liability + estimated payments); it now also carries the 2024 deduction detail needed to correctly tax a state/local income-tax refund under the tax-benefit rule (Pub. 525), instead of naively taxing the full 1099-G box 2 gross.

- **Step 1 — extend prior-year data storage — DONE 2026-07-11.** 6 new nullable columns on `pf_prior_year_tax` (V112): `prior_year_filing_status`, `prior_year_itemized_deductions_claimed`, `prior_year_total_itemized_deductions`, `prior_year_standard_deduction_available`, `prior_year_state_local_taxes_before_cap`, `prior_year_taxable_income`. `PfPriorYearTax` entity + `PriorYearTaxMapper` save/load extended (no new register-in-N-places sites — the form was already in PERSONAL_FORMS + the mapper already existed). Healthy boot ran V112 + passed schema validation. Angular `form-prior-year-tax.component.ts` gained a "2024 Deduction Detail (state/local refund worksheet)" section: a filing-status `p-select` + an itemized-yes/no radio, with the 4 detail amounts gated behind `priorYearItemizedDeductionsClaimed === true` (a refund is only taxable if you itemized). 6 help entries added. Fields pre-approved as part of the owner-approved "~6 fields" plan. UI builds clean.
- **Step 2 — State/Local Refund Taxable-Portion Worksheet + Schedule 1 line 1 wiring — DONE 2026-07-11.** Pure static `computeStateLocalRefundTaxablePortion(gross, priorItemized, priorTotalItemized, priorStandardDeduction, priorSaltBeforeCap, priorFilingStatus, priorTaxableIncome)`: returns $0 if not itemized (no tax benefit); otherwise taxable = min(gross, SALT-cap benefit, itemized-over-standard excess), further reduced by a negative prior-year taxable income. MFS uses the $5,000 SALT cap, all others $10,000. Injected in `prepare()` **only when prior-year data is present and the user hasn't overridden line 1** — it writes `imported1099GLine1TaxableRefunds` (the field that REPLACES `computeOtherIncomes`'s naive full-gross 1099-G box 2 auto-sum), applying even when it computes $0 (e.g. refund of tax above the SALT cap, or standard-deduction year). 4-case unit test (`stateLocalRefundWorksheet_taxBenefitRule`) + end-to-end unit test (`…_wiresToSchedule1Line1` → $2,000). `TaxReturnComputeServiceTest` 973/973. **e2e `prior-year-state-refund-worksheet.spec.ts` (3 cases, all pass):** itemized/under-cap → full $2,000 taxable; standard-deduction year → $0 (overrides the naive auto-sum); SALT above cap → only the $1,000 benefited portion taxable.
- **Step 3 — unemployment §1341 claim-of-right repayment — DEFERRED (per owner 2026-07-11).** Repayment of unemployment compensation received in a prior year (a claim-of-right / §1341 credit-vs-deduction path). Not started; the two prior-year figures it would need (prior-year tax with and without the repaid income) are separable follow-up work.
...
- **#4 (form simplification) — DONE 2026-07-10.** On `form-kiddie-income-taxpayer.component.ts`: reworded the child-unearned and parent-income sections to say those amounts are now auto-detected / auto-pulled (leave blank unless overriding), and reframed the "Manual kiddie-tax result" `<details>` from "automatic computation is on our roadmap" to "Advanced — override the automatically computed result." Removed the two now-vestigial manual inputs the backend no longer reads (`childTaxableIncomeNotSubjectToKiddieTaxLine14` = line 14, `childTentativeTaxShareLine13` = line 13 — both computed by `computeForm8615`, verified not read anywhere in compute), keeping only `childFinalTaxLine18` as the explicit override. UI builds clean. (Model/mapper still persist the two dropped fields — harmless orphaned columns.)

**Deferred:** ~~extending `dependent-own-kiddie-tax.spec.ts` to pin the now-real kiddie tax value (it currently asserts income flow only); MFS/HOH-split kiddie e2e coverage.~~ **DONE 2026-07-12.** GAP A (parent 28%-rate collectible + unrecaptured §1250 slices): `KiddieTaxParentReader.ParentReturnInfo` extended with `twentyEightPercentRateGain`/`unrecaptured1250Gain` (auto-derived from the parent's computed Schedule D lines 18/19); `DependentOwnFormScoper`/`scopeForDependentOwn` inject `parent28RateGainLine6`/`parent1250GainLine6` onto the kiddie form; `computeForm8615` layers them into lines 9/10 via `computePreferentialRateTax` (min(28%/25%, marginal)). Unit test `line16Form8615PreferentialRateParentOwn28PercentCollectibleGain` (parent line 10 = $49,047 with a $40k collectible slice vs $43,847 at the plain 15% rate). GAP B (parent-selection e2e): 3 new specs in `dependent-own-kiddie-tax.spec.ts` — MFJ joint (line 6 = 228,500), MFS-split higher-income leg (184,250), HOH-split custodial leg regardless of income (36,375). **The HOH-split spec caught a real bug:** `computeInfo` fed `tr.filingStatus` (the raw row enum, still "MFS" on an HOH-split leg) to `selectSplitParent`, so the custodial HoH parent was mis-selected as the higher-income MFS spouse. Fixed by keying selection off the COMPUTED status (`extractFilingStatus(computation, fallback)`).
...
## ~~Schedule 1 Part II Post-AGI Refinement: Line 21 MAGI Phase-Out + Line 24c AGI Ceiling + Lines 24h/24i Income Caps — Deferred 2026-06-07~~ **RESOLVED 2026-07-07 (all 5 sub-items)**

**RESOLVED 2026-07-07 — all five sub-items closed.** #1 line-21 student-loan MAGI phase-out (§221(b)(2)) and #2 line-24c Olympic AGI ceiling (§74(d)(2)) implemented via the post-AGI two-pass (ThreadLocal + single-iteration recursion, same shape as the Form 8815 pass; §221 MAGI and §74(d) pre-exclusion AGI are both income-based → deterministic; the two checks + line-21 + 8815 compose safely, bounded passes). #3/#4 lines 24h/24i attorney-fee income caps (§62(a)(20)/(21)) implemented with new cap-basis inputs (V92). #5 line-21 dependent-disallow verified NOT a defect (the existing `someoneCanClaimYou OR someoneCanClaimSpouse` is IRS-correct per Pub 970). All verified against J.K. Lasser 2025; full backend suite **1347/1347**; e2e round-trips for each. ★ #2 was implemented as an auto-disallow + NON-blocking advisory (not the originally-registered §17 blocker): per §74(d)(2) the return computes with the line 8m income taxable, so a permanent 409 would be un-actionable — the vestigial `SCHEDULE1_LINE24C_...` entry was removed from `NonOverrideableFlags.CODES`.
...
1. ~~**Line 21 student loan interest MAGI phase-out.**~~ **DONE 2026-07-07 (Option A two-pass).** Implemented via a post-AGI second pass (mirrors the Form 8815 two-pass): after `buildForm1040` sets AGI, §221(b)(2)(C) MAGI is computed (AGI + the deduction added back + §911/§931/§933 foreign exclusions — Form 2555 FEIE+housing, Form 4563, Puerto Rico); `computeStudentLoanInterestPhaseout` applies the worksheet reduction (`round((MAGI−start)/range, 3dp)`), and if it reduces the deduction, `prepare()` re-runs once with the phased value in the `LINE21_STUDENT_LOAN_PHASEOUT_CAP` ThreadLocal so it cascades through AGI → line 15 → 16 → all credits. MAGI is pre-deduction (invariant) → deterministic single iteration; ThreadLocal guard prevents a third pass. MFJ uses the $170k–$200k band, Single/HOH/QSS the $85k–$100k band (MFS disallowed upstream). Verified against J.K. Lasser 2025 §33.13. 4 unit tests (below/mid/above/MFJ-band, with AGI-cascade assertions) + 1 e2e ($2,500 → $1,250 at MAGI $92.5k, AGI $91,250); suite 1345/1345. **(Item #2 line-24c ceiling + #5 dependent over-block still open.)** Original detail:
   IRC §221(b)(2)(B). Below MAGI thresholds (`STUDENT_LOAN_PHASEOUT_START_SINGLE_HOH_QSS` = $85,000; `STUDENT_LOAN_PHASEOUT_START_MFJ` = $170,000) the full deduction (up to the $2,500 cap) is allowed. Above the END thresholds ($100,000 / $200,000) the deduction is fully phased out. Between, a linear phase-out applies: `cap * (1 - (MAGI - START) / (END - START))`. MAGI for line 21 = AGI before deducting line 21 + foreign-exclusion add-backs (Form 2555 / Form 4563 / Puerto Rico income). Requires AGI plumbing because `computeIncomeAdjustments` runs before line 9 / line 10 / AGI are computed in `buildForm1040`. Two implementation options:
...
2. ~~**Line 24c Olympic / Paralympic exclusion AGI ceiling.**~~ **DONE 2026-07-07.** IRC §74(d)(2): when the pre-exclusion AGI (current AGI + line 24c) exceeds $1,000,000 ($500,000 MFS), the post-AGI pass re-runs with `LINE24C_OLYMPIC_CEILING_DISALLOWED` set → `computeIncomeAdjustments` forces line 24c to 0 (line 8m income stays taxable) and emits the non-blocking advisory `SCHEDULE1_LINE24C_OLYMPIC_EXCLUSION_AGI_CEILING_EXCEEDED`. Verified against J.K. Lasser 2025 (12.1/13.2): the income is always reported and the return computes with the offset only when AGI ≤ ceiling. ★ Implemented as auto-disallow + advisory, NOT a §17 blocker — the vestigial registry entry was removed. 2 unit tests + 1 e2e; suite 1347/1347.

3. ~~**Line 24h attorney fees for unlawful discrimination claims — income cap.**~~ **DONE 2026-07-07.** IRC §62(a)(20) cap implemented: new `line24hGrossIncomeFromDiscriminationAction` input (`pf_income_adjustments`, V92) + mapper + YAML + both Angular components; compute caps `line24h = min(fees, incomeBasis)` and emits non-blocking advisory `SCHEDULE1_LINE24H_ATTORNEY_FEES_CAPPED_TO_INCOME` (cap only when the basis is provided — omitted basis trusts the entry). Verified against J.K. Lasser 2025 ("only to the extent of the amount included in income as a result of the judgment or settlement"). 3 unit tests + 1 e2e round-trip; suite 1341/1341; backend boots on V92; frontend AOT green.

4. ~~**Line 24i attorney fees for IRS whistleblower awards — income cap.**~~ **DONE 2026-07-07.** IRC §62(a)(21) cap implemented alongside #3: new `line24iWhistleblowerAwardIncludedInIncome` input (V92) + mapper + YAML + both components; compute caps `line24i = min(fees, awardBasis)` and emits `SCHEDULE1_LINE24I_WHISTLEBLOWER_FEES_CAPPED_TO_AWARD`. Verified against J.K. Lasser 2025 ("up to the amount of the award reported as income"). Covered by the same test set as #3. **(Item #2 line-24c ceiling + #1 line-21 phaseout + #5 dependent over-block remain open — they need the AGI post-pass.)**

5. ~~**Line 21 dependent-disallow accuracy.**~~ **RESOLVED — NOT a defect (verified against Pub 970 / IRC §221(c), 2026-07-07).** The current `someoneCanClaimYou OR someoneCanClaimSpouse` is IRS-correct. Pub 970 (J.K. Lasser 2025 §33.13): *"You, **or** your spouse if filing jointly, could be claimed as a dependent by another taxpayer"* → if **either** is claimable, the deduction is disallowed (the OR). The item's proposed "only block when BOTH spouses are claimable on MFJ" would **under**-block — allowing a deduction the IRS disallows — so it is NOT the correct rule. No code change; the existing OR stands. (In practice the scenario is nearly moot anyway: a person filing a joint return generally cannot be claimed as another's dependent.)
...
| `STUDENT_LOAN_PHASEOUT_START/END_SINGLE_HOH_QSS / _MFJ` | same | Landed; **APPLIED 2026-07-07** via `computeStudentLoanInterestPhaseout` + the post-AGI two-pass (item #1 DONE) |
...
| `OLYMPIC_EXCLUSION_AGI_CEILING_DEFAULT / _MFS` | same | Landed; **APPLIED 2026-07-07** via the post-AGI pass + `LINE24C_OLYMPIC_CEILING_DISALLOWED` (item #2 DONE) |
...
### ~~Gap 1 — Schedule 1-A worksheet intermediates not on `Schedule1AView` (LOW priority; cosmetic)~~ ✅ **RESOLVED 2026-07-10**

**Resolved 2026-07-10.** `computeSchedule1A` already computed every worksheet intermediate as method-local values (MAGI build-up, per-channel tips/overtime sources, statutory caps, thresholds, excess-MAGI, whole-thousand buckets, phaseout reductions); they simply were not surfaced. Added 34 fields to `Schedule1A` + compute setters (hoisting the per-channel accumulators and phaseout intermediates out of the inner `if` blocks; new helpers `phaseoutExcess`, `phaseoutBuckets`, `amountOrNull`) + `Schedule1AView` interface + `buildSemanticValues` mappings for all worksheet lines (Part I 1/2a–2e/3; Part II 4a/4c/5/6/7/9/10/11/12/13; Part III 14a/14b/14c/15/17/18/19/20/21; Part IV 23/24/26/27/28/29/30; Part V 32/33/34/35/36/37) + persisted via V106 (`OutSchedule1a` 34 columns + `Schedule1AOutputMapper` save/load). MAGI feeds preview lines 3/8/16/25/31. No filed-return impact (Form 1040 line 13b consumes only line 38). 2 unit tests (`schedule1ATipsWorksheetIntermediatesSurfaced` — $20k tips @ MAGI $160k single → 4a/4c/6/7=20000, 9=150000, 10=10000, 11=10, 12=1000, 13=19000; `schedule1ASeniorWorksheetIntermediatesSurfaced` — MFJ both eligible @ $180k → 32=150000, 33=30000, 34=1800, 35=6000, 36=2, 37=10200). Backend schedule1A suite 21/21; UI builds; healthy boot validated the V106 schema end-to-end. **Known remaining blanks (not part of this gap):** per-vehicle Part IV rows 22a/22b (VIN + lender name) — the VIN is a documented backend non-exposure and the vehicle list is not carried on the flat preview model; line 35 is populated with the $6,000 statutory per-person figure (the IRS field label "after phaseout per person" is a naming quirk — the phaseout is applied to the aggregate at line 34, so line 35 × line 36 − line 34 = line 37).
...
### ~~Gap 2 — Form 6251 sub-adjustments 2c–2t + line 3 not on `Form6251View` (MEDIUM priority; affects AMT filers with non-trivial adjustments)~~ ✅ **RESOLVED (verify-and-close) 2026-07-10 — mis-specified as a surfacing gap; it is a compute gap**

**Resolved 2026-07-10 — no code change.** Investigation of the AMT builder (`TaxReturnComputeService.computeLine17`) shows AMTI is `line4 = line1b + line2a + line2b + line2g`; the Java AMT computation recognizes exactly three preference items and **all three are already surfaced** — `Form6251View` carries `line2aStateLocalTaxes`, `line2bStateLocalRefund`, `line2gPrivateActivityBondInterest` and `buildSemanticValues` maps all three to their PDF slots. So the gap's own acceptance criterion ("when a filer has any preference item recognized by the Java AMT computation, the corresponding line-2x field renders") is **already met** for every item the compute produces. The remaining 18 sub-adjustments (2c–2t) + line 3 are **not a surfacing gap** — they have no backing compute: they are the deferred / out-of-scope AMT preference items documented inline at `TaxReturnComputeService.java:17914-17918` (2c Form 4952 AMT investment-interest recompute — deferred; 2f ATNOLD / AMT NOL carryover — deferred; 2i ISO spread — out of scope; 2l depreciation, 2m passive-activity, 2r R&D, 2t intangible-drilling, etc. — tied to out-of-scope Schedule C/E/F / K-1 / depreciation paths). Surfacing them would require building that AMT-preference compute; adding blank `Form6251View` pass-throughs with no producer would be misleading dead code (implies wired when nothing feeds them), against the Gap 1/Gap 3 convention of surfacing only computed values. **Re-tracked as compute work, not view-shape:** the buildable follow-up is "implement the deferred AMT preference items (2c/2f first — the two marked *deferred* rather than *out of scope*), then surface them" — a compute task gated on Form 4952 AMT path + AMT NOL carryover, both of which are their own deferred items.
...
### ~~Gap 3 — Form 8814 worksheet ratios + Part II branch checkboxes not on `Form8814View` (LOW priority; cosmetic)~~ ✅ **RESOLVED 2026-07-10**

**Resolved 2026-07-10.** `buildForm8814` already computed the line 7 / line 8 allocation ratios, line 11, and line 14 as locals (plus the line-15 branch), just not surfaced. Added `line7RatioQualDiv`, `line8RatioCapGain`, `line11Total`, `line14Base`, `line15YesBranch10Pct`, `line15NoBranch135` to `Form8814` + compute setters + preview mappings (ratios split across the whole/decimal boxes; the line-15 branch drives the two Part II checkboxes) + persisted via V105 (`OutForm8814` + `Form8814ListOutputMapper`). No filed-return impact (the parent's 1040 consumes only line 12 / line 15). 1 unit test (`form8814_worksheetRatiosAndPartIIBranch_surfaced` — child $4k mixed income → line7=0.5/line8=0.25/line11=975/line14=2650/no-branch). Backend 1433/1433; UI builds; healthy boot validated the V105 schema. ★ No bespoke preview e2e: the three render mechanisms (fmtAmount amount boxes, the ratio whole/decimal split, and `=== true` checkboxes) are each already e2e-proven (Form 4972 line-20 ratio split + 8863/4972 checkboxes), and the compute is unit-pinned.
...
### ~~Gap 4 — Shared filer-profile injection for Form 8606 (and any preview form with address / preparer / signature fields) (MEDIUM priority; cross-form pattern)~~ ✅ **RESOLVED 2026-07-10 — preparer/signature OOS by product design; address surfaced; cross-form claim was overstated**

**Resolved 2026-07-10.** Two findings collapsed this gap:

1. **Preparer (5) + signature (2) fields are OUT OF SCOPE by product design.** The application is exclusively for taxpayers preparing and **electronically filing their own** returns — no preparer is ever involved and the PDFs are generated **only for the user's own record-keeping**. So the preparer block (`paid_preparer_*`) and taxpayer-signature block (`signature_taxpayer_if_filing_by_itself`, `signature_date_*`) have nothing to collect, model, or surface. They stay blank permanently (documented, same spirit as the IRS-attachment scope rule). This retires 7 of the 13 fields — the entire MEDIUM-priority driver.

2. **The "cross-form pattern" was overstated — only Form 8606 has the filer mailing-address block.** A field-map scan of the named forms shows: `f6251`, `f4972`, `f8862`, `f8880`, `f8949` have **zero** address fields (attachment schedules inherit the address from the 1040); `f8863`'s address fields are the **educational institution's** address (`part3_line22a/b_institution_address_*`), not the filer's; `f2555`/`f4563`/`f8396` have **form-specific residence** fields (foreign residence for the FEIE, American Samoa occupant relationship, mortgage main-home) that must NOT be auto-filled from the filer profile. **Form 8606 is the only migrated preview with the canonical filer home + foreign mailing-address block.**

**Address surfacing implemented (Form 8606, frontend-only).** The filer `Address` is already computed and serialized (`Form1040.setAddress(buildAddress(you))` → `form1040.address` in the compute response; the Form 1040 preview already renders it). Added a `filerAddress` computed to `form-tax-return-8606.component.ts` that reads `form1040.address`, plus `buildSemanticValues` mappings for the 6 keys (`home_address_street`, `home_address_apt_no`, `home_address_city_state_zip` [city/state/ZIP combined into 8606's single box via a `formatCityStateZip` helper], `foreign_country_name`, `foreign_province_state_county`, `foreign_postal_code`) + a `FilerAddressView` interface. No backend change, no migration (address is stable identity data already in the compute response, and the 1040 preview proves it survives GET). UI builds clean. Elements JSON confirmed to carry all 6 slots. **Product constraint recorded in agent memory** (`feedback_self_filing_no_preparer_scope`) so no future form ever builds preparer/signature fields.
...
#### ~~Gap 4972-1 — Part I eligibility questions 2–6 (10 checkboxes) ⚠️ MEDIUM-HIGH~~ ✅ **FULLY RESOLVED — Q2–Q5 2026-07-09; Q6 2026-07-11**

**Q6 resolved 2026-07-11.** The 2025 Form 4972 Part I has six questions (`part1_question_01`–`06` in the semantic CSV); Q6 is the beneficiary counterpart of Q5 — a prior post-1986 Form 4972 use for the *same deceased participant* (received as a beneficiary) disqualifies, just like Q5. It was **not collected and not checked** by the eligibility gate — a genuine tax-correctness gap (a repeat beneficiary election would wrongly still qualify), not just a blank checkbox. Added end-to-end mirroring Q5: new Yes/No card "Q6" on `form-lump-sum-distribution.component.ts` (Incomes → Lump-sum distributions → Eligibility questions; serves taxpayer + spouse via the `person`/prefix) + `PfLumpSumDistribution.partIQ6NoPriorBeneficiaryElection` + `LumpSumDistributionMapper` (V113.1) + `computeForm4972` folds `q6noPrior` into the `eligible` gate and sets `Form4972.partIQ6PriorBeneficiaryElection` + `OutForm4972` column (V113.2, survives reload) + `form-tax-return-4972.component.ts` maps `part1_question_06_yes/no`. Tests: `form4972PartIQ6BeneficiaryPriorElectionDisqualifies` (Q6=Yes → null form + INELIGIBLE flag) + Q6 assertion in `form4972_partIQuestions2to5_carriedToModel` + extended `form4972-8863-preview-render.spec.ts` e2e (part1_question_06_no renders). Core suite 976/976; UI builds; healthy boot ran V113.1/V113.2. **The 10-checkbox item is now fully closed.**

(original Q2–Q5 note follows)

#### Gap 4972-1 — Part I eligibility questions 2–6 (10 checkboxes) ⚠️ MEDIUM-HIGH — **Q2–Q5 RESOLVED 2026-07-09; Q6 deferred**

**Q2–Q5 resolved 2026-07-09.** ★ The item's premise was partly stale: questions 2–5 were **already collected** as Yes/No radios on the lump-sum-distribution intake form and **already read** by `computeForm4972()` (for the eligibility gate), so no intake change was needed. The gap was only the view-shape handoff: added 4 `Boolean` fields to `Form4972` (`partIQ2RolledOver`, `partIQ3BornBefore1936OrBeneficiary`, `partIQ4FiveYearsParticipation`, `partIQ5PriorElection`), set them from the literal intake answers in `computeForm4972()` (right after `setEligible`), and mapped `part1_question_02..05_yes/no` in `form-tax-return-4972.component.ts`. Each intake radio binds Yes→true / No→false, so the preview shows the taxpayer's literal answer. Q1 was already surfaced via the aggregate `eligible` verdict. 1 unit test (`form4972_partIQuestions2to5_carriedToModel`) + extended `form4972-8863-preview-render.spec.ts` e2e. Compute suite 956/956; UI builds. ★ **Reload-persistence RESOLVED 2026-07-09 (V100):** Q2-Q5 are persisted through `OutForm4972` + `AbstractForm4972OutputMapper` (V100 columns), so the checkboxes survive a page reload; lock-in e2e in `form-checkbox-reload-persistence.spec.ts`.
...
**Resolved 2026-07-09.** ★ The item's premise was partly stale: the four disclosure answers were **already collected** on the education-credits intake form (`institution1Received1098T`, `institution1PriorYear1098TBox7Checked`, and the institution-2 pair) and already persisted by `EducationCreditsMapper` — no intake-form change, no Form 8863 layout change, and no 1098-T/statement work were needed (the line-22 checkboxes are the taxpayer's own attestations, and the 8 checkbox cells already exist on the IRS f8863 template). The gap was purely the backend→preview handoff: added 4 `Boolean` fields to `Form8863Student`, set them in `computeForm8863()` (next to the existing line 23-26 disqualifier answers), added the 4 fields to the frontend `Form8863StudentView`, and mapped the 8 `part3_line22a/22b_question_b/c_1098t_*` checkbox slots in `form-tax-return-8863.component.ts`. Matches the existing line 23-26 pattern (compute-view carried). 1 unit test (`form8863_line22Disclosures_carriedToStudentView`) + 1 e2e (`form4972-8863-preview-render.spec.ts` — mixed Yes/No across both institutions). Compute suite 955/955; UI builds. ★ **Reload-persistence follow-up RESOLVED 2026-07-09 (V100):** the whole Form 8863 Part III boolean set (4 line-22 disclosures + 4 line 23-26 disqualifiers) is now persisted through `OutForm8863Student` + `Form8863OutputMapper` (V100 columns), so the checkboxes survive a page reload (`GET /api/tax-return`), not just the in-memory compute response. Lock-in e2e `form-checkbox-reload-persistence.spec.ts` computes → reloads → asserts the checkboxes still render. (Same V100 persists Form 4972 Part I Q2-Q5.)
...
#### Gap 8863-2 — Per-institution structured address (line 22a/22b address cells + EIN + attendance dates) ⚠️ MEDIUM — **EIN + 2nd-institution identity RESOLVED 2026-07-10; structured address decomposition RESOLVED 2026-07-13 (V127)**

> **Update 2026-07-13 (Gap 8863-3 closed, V127):** the structured city/state/ZIP/foreign decomposition below is now DONE — the single combined address field on each institution slot was replaced with 7 structured fields (street/city/state/ZIP + foreign country/province/postal), each mapping to its own IRS PDF box, across entity/mapper/output/compute/preview/intake. See the dated section at the top of this file. (There is no "attendance dates" field on the 2025 Form 8863 Part III — that part of the original note was a misread; the only line-22 boxes are name/address/EIN.)
...
### Multi-return cross-cutting items — live state (verified 2026-07-14)

- ~~**HoH-split leg stored as "MFS" (`tax_return_v2.filing_status` stale).**~~ **✅ FIXED 2026-07-14.** `enableMfs` stamped both legs "Married filing separately" once and never updated it, so a leg that later elected HoH still stored "MFS". New `syncLegFilingStatus` (called from `prepare(long taxReturnId)` with the scoper-resolved status) keeps the column honest; invisible in the UI but corrected for any consumer that reads it. See history.md 2026-07-14.
- **Duplicate `prepare()` runs (multi-return) — WITHIN-REQUEST redundancy ✅ FIXED 2026-07-14; cross-request remains (LOW).** The biggest waste was within a single optimize request: computing each MFS leg with an AUTO deduction election triggers `resolveSection63c6Election`, which recomputes BOTH legs — so the optimizer (which computes both legs) ran that resolution twice per request (4 inner leg computes). Added a `@RequestScoped ComputeRequestCache` memoizing the resolution per uid (auto-cleared per request → no staleness); the optimizer now resolves once (dev-log-confirmed: "resolutions in optimize request: 1"). Results byte-for-byte identical (optimizer + hoh-split e2e green; new `optimizer-joint-vs-separate.spec.ts`). **Still deferred (genuinely low):** the CROSS-request duplicate — the UI calling `/compute` (primary) then `/optimize` (primary again) recomputes the primary across two separate HTTP requests. Deduping that needs an input-versioned persistent cache with save-invalidation (staleness risk), so it's left as-is.
- ~~**Targeted FE tests for the multi-return/optimizer UI.**~~ **✅ CLOSED 2026-07-14.** All previously-untested multi-return/optimizer components now have Karma/Jasmine component specs — **46 tests, all green** across 6 specs. First wave (22): `optimizer-comparison-panel` (8 — table totals, MFJ/MFS/TIE recommendation + savings display, running/error states, `runComparison` delegation), `persistent-mfs-toggle-panel` (6 — enable/disable render by `isMfsEnabled`, toggling/error states, method delegation), `dependent-own-toggle-panel` (8 — per-dependent `isEnabled`, multi-dependent disambiguation, string→number id coercion, disabled-without-id, delegation). Second wave (24, 2026-07-14): `eic-comparison-panel` (12 — hidden without combat pay, ok-outcome table + winning-cell + with/without/tie recommendation tone, ineligible note, error/blocked outcomes, `compareBothWays` delegation + `comparisonComplete` emit, autoRun-on-init, stale-banner on `personalData.updated()` bump), `form-filing-status` parent (5 — init merge, required-status validity, save payload, no-save-when-invalid, nonresident-spouse conditional field), `form-filing-status-spouse` (7 — MFJ lock / MFS-or-HoH election / neither-info-note gating, considered-unmarried reveal + advisory, HoH-field clearing, MFS vs HoH vs non-electing save payloads). Run headless via `ng test --watch=false --browsers=ChromeHeadless` (system Chrome). ★ Spec-authoring gotcha captured: a test that re-creates the component via a `createComponent()` helper (which calls `ngOnInit()` manually **and** `detectChanges()`, re-triggering an unawaited `ngOnInit`) and then immediately `await onSubmit()` races the second `ngOnInit`'s `loading=true` window and the submit short-circuits — set the field under test directly on the settled component instead of re-creating.
- **line-20/31/32/33 test coverage — essentially CLOSED (verified).** Line 31 excellent (16 e2e + 15 Java, 10 gap closures); lines 32/33 very strong (arithmetic-invariant + multi-source integration); line 20 the thinnest but still ~3 e2e + ~10 Java. No genuine gap remaining; line 20 could take a couple standalone Schedule-3-line-8 tests if desired.

## Schedule A mortgage interest — home-equity tracing (sc_00123) — user-judgment, not computed (2026-07-12)

Gap-closure Phase 1 implemented the Pub 936 $750k/$1M acquisition-debt limitation and points amortization (see history 2026-07-12). One related Chapter-17 case is deliberately **not** auto-computed: **sc_00123 home-equity use-of-proceeds tracing** (§17.3). Whether home-equity-loan interest is deductible depends on whether the proceeds were used to buy/build/substantially improve the securing home — a fact the software cannot derive from amounts. The user enters only the deductible (acquisition-debt) interest on `homeMortgageInterestPaid`; the field help already states "only interest on acquisition debt is deductible." Treated as intake guidance (pass-through), consistent with the analogous charitable/casualty user-precomputed inputs. No compute or blocker planned. **★ SQA conformance scenario authored 2026-07-12: `sc_00252` (mixed-use tracing — deduct only the improvement fraction of the proceeds)** — a gap-finder for a full commercial product; our own app's allocation stays user-entered.
...
- **Combined 50%-of-AGI overall ceiling + ordering rule** when a return has BOTH large cash and large capital-gain-property gifts in the same year — the buckets interact (30% capital-gain is further limited so total ≤ 50%). ~~Implemented as independent per-bucket limits; the interaction is deferred in our app.~~ ✅ **RESOLVED 2026-07-12** — `buildScheduleA` now applies the IRC §170(b)/Pub. 526 combined ordering (cash 60% first; basis + capital-gain property share the remaining 50%-AGI base; capital-gain further capped at 30%). Unit test + live e2e green (sc_00253: property $40k → $20k allowed, $20k carryover). No new fields.
...
- **§1033 involuntary-conversion gain deferral:** ✅ **RESOLVED 2026-07-12.** Auto-compute on the capital gain/loss form (V119: 6 nullable columns): realized = proceeds − basis; recognized = min(realized, proceeds not reinvested within the period); recognized gain routes to Schedule D line 11/4. Unit + live e2e green (sc_00117 → $50k recognized on partial reinvestment; $0 on full). UI entry via the existing line-11 "casualty/involuntary-conversion" field; the auto-compute fields are API-seedable.

## Schedule E rental — deferred items (2026-07-12)

Gap-closure Phase 4 implemented Schedule E Part I net rental/royalty income + §280A (history 2026-07-12); Phase 5
added the §465 at-risk + §469 passive-loss machinery. Deferred:
- ~~**$0-net rental-only Schedule 1 Part I pruning:**~~ ✅ **FIXED 2026-09-16 (V260) — and the diagnosis below was WRONG.** The carryforward was not merely *hidden* on $0-net returns: it was never PERSISTED on any return (no column, no mapper reference, no importer), so every §280A filer forfeited the disallowed depreciation permanently. `add_inc_rental_depreciation_carryforward` + `importedPriorYearRentalDepreciationCarryover` complete the bridge; the carry-in is spent against remaining §280A headroom and the rest rolls forward. `Section280ADepreciationCarryforwardTest`. Original note: when a return's ONLY additional income is a rental that nets to $0 (e.g. a §280A vacation home with all deductions absorbed), the output serializer prunes the empty Schedule 1 Part I, which also hides `rentalDepreciationCarryforward`. The carryforward surfaces whenever any non-zero additional income (another property, other Schedule 1 line) is present. Low impact; revisit if a Schedule E preview form needs the carryforward on a $0-net return.
- ~~**Multi-return dual-write trigger:** `pf_rental_income`/`pf_rental_property` carry a `person_id` column but no V66/V67-style trigger yet; the mapper queries by uid+owner_role (single-return path works). Add a trigger when rental is integrated into the MFS/dependent-own multi-return scoping.~~ **✅ DONE 2026-07-14.** Rental wired into MFS scoping: (1) `MfsFormScoper.normalizeRentalIncomeSpouse` routes the spouse's `spouseProperties` → the filer slot's `taxpayerProperties` on the `mfs_spouse` leg (rental is the owner_role + role-suffixed-child-list twin of Form 8911 #34 — the generic `-spouse`→`-taxpayer` rename would otherwise keep `spouseProperties`, which `computeRentalScheduleE` ignores, so her rental vanished from her own leg); (2) **V130** adds the missing V66-style `person_id` BEFORE-trigger on `pf_rental_income` (`pf_rental_property` is a child by `parent_id`, no `person_id`). Scoper unit test + MFS e2e (`mfs-spouse-rental-income.spec.ts`: head rental $3k on his leg, spouse rental $8k on hers). Full suite 1511 green. (K-1 statement MFS attribution by recipient-TIN remains separately deferred below.)
- ~~**Dedicated Schedule E preview form**~~ ✅ **STALE — it exists** (verified 2026-09-16). `ScheduleE` output carries lines 26/32/41 plus a per-property fields map, and `form-tax-return-schedule-e.component.ts` renders it; built with the C6d Schedule E work.
- ~~**Passive-activity loss limits (Form 8582) + at-risk (Form 6198):** ordinary rentals may currently show a full loss on line 5 — the $25k allowance / MAGI phaseout / suspension is Phase 5 (sc_00073–00078).~~ **STALE — the loss *math* is IMPLEMENTED (verified 2026-07-13).** `computeRentalScheduleE` (TaxReturnComputeService.java:15628) runs the full pipeline in IRS order: §280A personal-residence cap → §465 at-risk cap (excess → `atRiskCarryforward`) → passive/nonpassive split (RE professional or material participation = nonpassive/full loss) → §469 (passive losses offset passive income, then up to the $25,000 active-participation special allowance phased out 50% over MAGI $100k–$150k, excess suspended → `passiveLossCarryforward`, full disposition releases all suspended per §469(g)). Constants correct ($25,000 / $100,000 / 0.50). Locked by 9 unit tests (`passiveRentalLossFullySuspendedAboveMagi150k`, `passiveRentalAllowancePhasesOutInBand`, `passiveLossOffsetsPassiveIncomeExcessSuspended`, `atRiskLimitCapsLossBeforePassiveRules`, `scheduleE280APersonalResidenceLimitsLossToZeroWithDepreciationCarryover`, …, TaxReturnComputeServiceTest.java:20052+). ~~**What's genuinely still deferred:** the standalone printable **Form 8582 / Form 6198 worksheets** (the net reaches line 5 and the carryforwards are on the result, but the fill-in forms aren't generated) — NOT the loss computation itself.~~ **✅ BUILT 2026-07-13 — Form 8582 + Form 6198 Tax-Return preview forms now exist** (see history.md 2026-07-13). `Form8582Output` (summary lines 1a–11) + `Form6198Output` (Part I / simplified Part II / Part IV) are populated by `computeRentalScheduleE`, persisted (V128, `out_form_8582`/`out_form_6198`), and rendered by pure-HTML previews (`form-tax-return-8582`/`-6198`) with Save-as-PDF, gated into the sidebar on presence. Only the per-activity worksheet tables (8582 Parts IV–IX; 6198 detailed Part III) render blank — this app limits on the aggregate. 2 unit + 2 e2e assertions (sc_00074 §469 allowance, sc_00077 §465 at-risk).
...
- **Other K-1 boxes:** ⚠️ **RENTAL ROUTED 2026-09-16.** Boxes 2 and 3 (net rental real estate / other net rental) now join the §469 passive pool via `sumPassiveK1RentalNet` — per se passive under §469(c)(2), so NOT gated on material participation, and no $25,000 allowance (§469(i)(6)(C)). They were captured and read by nothing, so a K-1 rental contributed nothing at all. `K1RentalBoxRoutingTest`. **STILL PARKED:** guaranteed payments (4a/4b — raise a Schedule SE question, since box 1 does not feed SE tax today either), §179 (box 12), and the other income boxes.
...
saved and produces **no depreciation at all**, with no flag — the filer gets the deduction only if they
also type a figure into the rental form's own `depreciationAmount` box. Taxpayer harm, ~$10,000/yr on a
$275,000 building. Reachable: the asset form sits unconditionally in the Incomes sidebar.

Full scope, blast radius and phasing: **[scope-rental-depreciation-assets.md](scope-rental-depreciation-assets.md)**.
Short version — the MACRS arithmetic for 27.5/39-year real property already exists and the Schedule C
`matched.isEmpty() ? typed : computed` rule settles the double-count question, so Phase 1 is wiring with
