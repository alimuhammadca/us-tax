...
> - **Form 8829 home-office carryforward** — the disallowed excess (home-office deduction beyond the gross-income limit) is surfaced via the `HOME_OFFICE_LIMITED_BY_GROSS_INCOME` advisory but is NOT stored/carried into a future year, and no prior-year home-office carryforward is read into current-year expenses. **NOTE (updated 2026-07-27):** the app is no longer app-wide no-carryforward — three carryforwards now bridge across years (Form 172 NOL, Form 8990 §163(j), and Schedule A charitable — each persisted + auto-imported on the primary path). Form 8829 home-office, §461(l) disallowed loss, and §465 at-risk suspended amounts remain **single-year** (current-year math IRS-correct; only the cross-year plumbing is out — a candidate to bridge the same way if wanted).
...
> - **§199A UBIA recovery-window drop-off** — UBIA of qualified property is threaded from the business's depreciable assets (2026-07-22, fixes the prior UBIA=0), but an asset is NOT dropped from UBIA once it passes the LATER of its recovery period or 10 years (§199A(b)(6)(A)). Slight over-inclusion of UBIA for old assets; a precise fix needs the per-asset placed-in-service-window check.
