...
1. **Income-attribution / dependent-statements gap (the main one — see analysis below).** A dependent's income is pulled from the household statement pool by SSN match, but the dependent tab historically had no first-class statement entry. Statements carry an `owner_role` column (taxpayer/spouse/dependent) exactly like personal forms, and the Statements section IS rendered on a dependent tab with `1099-int`/`1099-div` auto-offered — but the FULL applicable set is not yet curated for dependents, and there is no `dependent_id` on statement rows (multi-dependent UI disambiguation relies purely on the `recipientTIN`/SSN filter). This is the "give dependents their own statements, like we did for the spouse" work.
