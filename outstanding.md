...
- **Parent's own preferential income + 28%/§1250 Schedule D Tax Worksheet.** The parent's *own* qualified dividends / net capital gain are not captured by any input field, so line 10 stays bracket-based (it cancels in line 11 only when the parent has no preferential income; when the parent does, the kiddie tax is slightly overstated). The 28%-rate/§1250 Schedule D Tax Worksheet path (collectibles / unrecaptured §1250 — rare for a child) also remains on the QDCG 0/15/20 approximation. The manual `childFinalTaxLine18` override covers both.
