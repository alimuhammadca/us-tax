...
- **Targeted FE tests for the multi-return/optimizer UI — REAL gap, deferred (LOW).** Only 1 of ~7 multi-return/optimizer Angular components has a `*.component.spec.ts` (`form-filing-status-taxpayer`); the optimizer comparison panel, MFS/dependent-own toggle panels, EIC comparison panel, and `filing-status-spouse` have none. The multi-return SERVICE is well covered (`multi-return.service.spec.ts`, 19 tests) and the optimizer panel is fully `data-testid`-instrumented for e2e, so the logic is pinned; the component-render/interaction layer is not.
