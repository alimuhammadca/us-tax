...
2. **EIC block is by-omission, not an explicit §32 disqualification.** EIC never computes on the scoped map because the opt-in form isn't seeded — functionally correct (no EIC) but no user-facing "you're a dependent, EIC disallowed" reason.
3. **§152 self-claim edges not fully modeled** — a dependent claiming their own dependents, or the interaction with the parent's claim, is deferred.
