...
- **Increment 2b (next) — service wrapper + Schedule wiring.** `computeForm8889(hsaForm, 1099-SA entries, personSsn, dob, uid)` resolving the form inputs + summing line 14a from the person's HSA-flagged 1099-SA (read-only) + deriving catch-up from DOB (born ≤ 1970-12-31); compute taxpayer + spouse in `prepare()`; wire line 13 → Schedule 1 line 13, line 16/20 → Schedule 1 line 8f, line 17b → Schedule 2 line 17c, line 21 → Schedule 2 line 17d by injecting the computed per-person values into the existing input maps **only when the manual field is absent** (manual stays the override; a mismatch advisory optional). Care needed: null input maps (HSA-only filers), MFS spouse-shadowing, and the Schedule 2 17c/17d single return-level map (sum taxpayer+spouse).
- **Increment 2c — Angular HSA form** (`hsa-taxpayer`/`hsa-spouse`) + sidebar.
