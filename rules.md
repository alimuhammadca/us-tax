...
- **After opening a personal form, wait for its `ngOnInit` load GET to RESOLVE before setting any field.** `clickSidebarItem` activates the sidebar link but returns before the form's async `ngOnInit` finishes; that `ngOnInit` awaits `loadModel()` → `GET /api/personal/{formId}`, which sets `this.model`. If a radio/field is set before that GET resolves, the load callback overwrites `this.model` and silently discards the input — the PUT still returns 200 with the stale value, so a *derived* output (e.g. a Form 1040 line-12 checkbox) comes back wrong at compute, not at save. A visibility guard is NOT sufficient: `selectRadioOption` polls `isChecked()`, which reads the *native DOM* checked state, and the late GET resets it out from under the poll — so it only fails under full-regression load. Arm `page.waitForResponse(r => r.url().includes('/api/personal/<formId>') && r.request().method() === 'GET')` *before* `clickSidebarItem`, then `await` it before interacting (idiom: line4abc / line1e / w2-required specs). Root-caused on `personal-per-person-forms.spec.ts` §157 (`line12cChecked`/`line12aChecked`) 2026-07-19; same class as the 2026-07-06 `youWereDualStatusAlien` visibility-guard fix, which treated the symptom not the GET race.
