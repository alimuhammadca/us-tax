...
amounts; box-3 write-ins are inside the full line-16 `getTax()`).

Now ALSO implemented (2026-08-04, "fix all gaps"): **Form 2555 "AMT capital gain excess" Part III
modification** — for a Form 2555 filer, reduce Part III line 13 (then line 14) by `max(0, SchDTW line 10 −
Form 6251 line 6)`; threaded via `populateAmtPartIIIFields(form2555ExcessLine6Reference)` ONLY on the AMT
FEITW line-4 leg (null elsewhere — the standard Part III already caps via line 16 = min(line12,line15)).
**"Who Must File" attachment** — Form 6251 is emitted (a $0-AMT record) when line 6 = 0 but the return
claims a GBC / Form 8801/8834/8911 (`claimsForm6251TriggeringCredit`) or lines 2c-3 are negative and TMT
without them > line 10. No tax impact; matches the paper attachment rule. Pins
`amtFeitwForm2555CapitalGainExcessReducesPreferentialPool`,
`amtForm6251AttachedWhenClaimingPriorMinTaxCreditEvenWithNoAmt`.
...
**Rental passive/non-passive classification — real-estate-professional status is PER-OWNER, never for royalties** (fixed 2026-08-03, Form 8960 NIIT audit). In `computeRentalScheduleE` the §469 passive/non-passive classifier must use the OWNER's `isRealEstateProfessional` flag (track `taxpayerPropertyCount`: properties [0,count) are the taxpayer's, the rest the spouse's), NOT a household-wide OR over both spouses. §469(c)(7) RE-professional status is an individual determination: it makes only THAT person's own RENTAL real estate non-passive, never reaches the other spouse's property, and NEVER applies to ROYALTY properties (royalties are portfolio net investment income). The prior household-OR-applied-to-all-properties swept the other spouse's rentals AND royalty income out of the passive pool → under-stated NIIT (Form 8960 line 4c) and over-freed the other spouse's rental losses on the 1040. `materialParticipation` (per property, label "activity is nonpassive") remains the direct non-passive signal for both rentals and active-business royalties. Do NOT reintroduce a household-wide `reProfessional`. Pins: `TaxReturnComputeServiceTest#reProfessionalDoesNotSweepOtherSpouseRentalNonpassive` + `#reProfessionalDoesNotExcludeRoyaltyFromNiit` + `#realEstateProfessionalLossIsNonpassiveAndFullyDeductible`. **Form 8960 verified correct (do NOT "fix"):** Part III lines 12–17, the $200k/$250k/$125k thresholds, the min(NII, MAGI−threshold) lesser-of + 3.8%, the Schedule 2 flow, the MFS FEIE guard, the K-1 passive split (SE excluded), line-9a auto §163(d), the $3k cap-loss limit. **Line-13 MAGI adds back ONLY the §911(a)(1) foreign EARNED-INCOME exclusion (Form 2555 line 42), NEVER the §911(a)(2) foreign HOUSING exclusion** — the 2025 Form 8960 "Line 13—MAGI Worksheet" step 2(a) pulls Form 2555 line 42, not line 45/line 36 (verified vs `i8960.pdf` 2026-08-05). This was mis-implemented once (housing wrongly added → over-taxed expats, fixed 2026-07-30) and has since been re-flagged as a "CRITICAL bug" by TWO separate adversarial audits (2026-08-03, 2026-08-05) that assumed the worksheet uses line 45. It does NOT. Adding the housing exclusion OVER-taxes every expat with a housing exclusion by 3.8% of it. Do not "fix" it. (Contrast §36B PTC MAGI, which DOES add §911(a)(2).) **KNOWN DEFERRED:** line-13 MAGI adds GROSS §911 FEIE not net of §911(d)(6) allocable deductions (no Form 2555 line-44 figure); §1411(c)(2)(B) trading-business income not swept into NII; line-5b/self-rental rely on overrides. See [[feedback_principled_diagnosis_over_test_tweaking]].

**Schedule SE (self-employment tax, §1401/1402) — audited 2026-08-04, verified correct (do NOT re-question these).** A 3-lens adversarial audit found NO bugs. Confirmed-correct so future work doesn't re-litigate: net earnings = (Sch C net-for-SE + Sch F + K-1 box-14A **code A only**) combined then × **0.9235** ONCE, guarded `signum()>0` so a net loss yields $0 SE tax and no spurious ½-SE deduction; **SS wage base $176,100** (2025) reduced by the person's own W-2 **box 3 (SS wages) + box 7 (SS tips)** + Form 4137 line 10 + Form 8919 line 10 (line 8d) before the 12.4% OASDI portion — NOT box 1 or box 5; Medicare 2.9% on the full net earnings (no cap); $400 floor; ½-SE deduction taken from the **rounded whole-dollar line 12** (deriving from raw cents understates line 13 by $1 and inflates AGI — SQA sc_00211/00219); per-person (each spouse's own Schedule SE + $176,100 cap). Special income all correct: church-employee 5a→**5b = 5a×0.9235** as a SEPARATE line-6 track (not folded into line 3/4a); optional-method 2025 constants **$7,240** (=4×$1,810 quarter-of-coverage) max / **$7,840** net-eligibility / **$10,860** farm-gross / **72.189%** nonfarm; clergy Form 4361 zeroes only the ministerial base, Form 4029 skips the whole person; statutory-employee (box 13) net EXCLUDED from the SE base but kept as a QBI activity; multi-business netting is per-person before the 0.9235 (farm merged into SE only AFTER the SE call, so no double-count). Interactions correct: Form 8959 Part II line 8 = Schedule SE line 6 net earnings (already ×0.9235), reduced-threshold coordination, its 0.9% is NOT double-counted with the Schedule SE 2.9% and is NOT in the §164(f) ½-SE deduction; §199A QBI base subtracts ½-SE + SE-health + SE-retirement (§1.199A-3(b)(1)(vi)); §461(l) disallowed EBL goes to Schedule 1 line 8p (never re-added to the SE base); Schedule 2 line 4 (SE) + line 11 (Form 8959, which SUPERSEDES the Part-I wage estimate, not adds). **Caveat:** `docs/IRS-Forms/Schedule SE.pdf` is a stale 2024 revision — verify SE constants against the 2025 form / ReferenceData, not that PDF. See [[feedback_principled_diagnosis_over_test_tweaking]].

**Form 1116 Foreign Tax Credit (§904) — over-credit guards** (fixed 2026-08-04, adversarial audit). (1) **§904(j) simplified exception is gated on the $300/$600 threshold** in `computeForm1116`: the election (claim the FTC without Form 1116, waiving the §904(a) ratio limitation) is used ONLY when creditable foreign taxes ≤ $300 (single/MFS) / $600 (MFJ); above it the taxpayer is INELIGIBLE and falls through to the full Form 1116. Do NOT re-introduce the unconditional `min(foreignTax, US tax)` simplified path (it escaped §904 at any amount). (2) **Line 19 fraction is capped at 1.0** (`line17.divide(line18,4,HALF_UP).min(ONE)`) — "if line 17 > line 18, enter 1". (3) **Total FTC (line 33/35) is capped at total US tax** (`line35.min(usTax)`) = min(line 20, line 32). (4) **A country's line-7 net may be a LOSS** (`gross.subtract(totalDeds)`, no per-country floor) so intra-basket losses offset gains; the CATEGORY total (line 17) is floored at 0 (`totalNetForeignTi.max(ZERO)`). **Verified correct (do NOT "fix"):** per-basket separation (§904(d), one Form 1116 per category, independent limitation, no cross-basket pooling), HTKO reclassification, line-20 base = Form 1040 line 16 + Schedule 2 line 1z additions (AMT is Sch 2 line 2 → excluded), line 24 = min(line 21, line 14), AMT FTC (§59(a): AMTI denominator + TMT, numerator capped at 1.0), Form 6251 line-8→11 wiring, per-basket vintage-tagged carryover still §904-limited, Schedule 3 line-1 summation. **KNOWN DEFERRED (need apportionment bases / feature work):** line-3g pro-rata standard/itemized deduction apportionment is hardcoded 0 → SYSTEMATIC over-credit (needs gross-income-all-sources base); no §904(b)(2)(B) QD/cap-gain rate adjustment (over-credit on large foreign QDCG); line 18 omits the 2025 Sch 1-A line-37 senior add-back (narrow); silent 3-country truncation (needs additional Forms 1116); no Form 2555 FEIE double-count guard; §904(c) carryforward lost in gap years/baskets (UNDER-credit); 1-year carryback not implemented. Pins: `TaxReturnComputeServiceTest#form1116SimplifiedExceptionOver300FallsThroughNoUncappedCredit` + `#form1116SimplifiedExceptionUnder300IsAllowed` + `#form1116IntraBasketCountryLossOffsetsGain`. See [[feedback_principled_diagnosis_over_test_tweaking]].

**Schedule A — Form 8396 recompute must preserve ALL line-17 components; OBBBA 2025 SALT verified** (fixed 2026-08-04, adversarial audit). When the mortgage-interest credit (Form 8396) reduces the Schedule A interest deduction, `applyForm8396ReductionToScheduleA` recomputes line 17 as `originalTotalItemized − actualInterestReduction` (only the interest line changes). Do NOT re-sum a hand-listed subset of components — the prior code summed only medical + capped-SALT + interest + charity + disaster-loss, silently DROPPING line 6 (foreign taxes), line 15 (casualty), and every line-16 item (gambling, impairment, estate tax on IRD, claim-of-right, Form 4684 §B) → over-stated taxable income (over-tax). Subtracting the delta preserves all components and survives future line-16 additions. Pin: `TaxReturnComputeServiceTest#form8396ScheduleAReductionPreservesNonInterestItemizedLines`. **Verified correct (do NOT "fix"):** OBBBA 2025 SALT cap $40,000/$20,000 MFS + phasedown = base cap reduced by 30% of MAGI over $500,000/$250,000 MFS, floored at $10,000/$5,000 MFS (`saltLimitForStatusAndMagi`; the phasedown is 30%, NOT $1-for-$1 — a stale comment said otherwise); MAGI = AGI + §911 add-back; income-vs-sales-tax election; medical 7.5% floor; $750k/$1M acquisition-debt limit + Pub 936 ratio; points amortization; §163(d) investment interest; casualty §165(h) federally-declared-disaster gate (guided Form 4684 path); gambling ≤ winnings (2025 = 100%, OBBBA 90% haircut deferred to 2026); §1341; itemize-vs-standard §63 election + MFS both-must-itemize; AMT SALT add-back uses the CAPPED line-7 amount (not uncapped 5d); §111 SALT-cap-aware state-refund taxability; §275(a)(4) FTC-vs-deduction foreign-tax suppression. **KNOWN DEFERRED (input-trust / scope):** §931/§933 excluded from the phasedown MAGI (only §911 added back); home-equity interest defaults to fully deductible and escapes the combined $750k cap, points not ratio-limited on jumbo loans (all input-trust over-deduction); a legacy pre-computed casualty scalar bypasses the disaster gate (advisory-only); MFS cross-leg itemize attestation not auto-derived. See [[feedback_principled_diagnosis_over_test_tweaking]] and [[feedback_input_side_statutory_reduction_audit_lens]].

**EIC (§32) — earned income excludes box-11 NQDC; US-residency applies to ALL filers** (fixed 2026-08-04, adversarial audit; 2025 table verified against Rev. Proc. 2024-40 §2.06 text). (1) `computeLine27aEIC` must subtract W-2 box 11 nonqualified deferred comp (`sumW2NonqualifiedPlansBySsn`) from the earned-income wage base — box 11 is deferred comp (§32(c)(2)(B)(ii)), NOT earned income. It cannot reuse `sumW2WagesAfterCarveOuts` because the EIC path must KEEP statutory-employee box 1 (that helper drops it); so box-11 and inmate-wage carve-outs are applied separately in the EIC method. (2) The `mainHomeInUsMoreThanHalfYear` gate (US principal abode > half year) applies to ALL EIC filers — §32(c)(1)(A)(ii)(I) for childless AND Pub 596 Rule 9 for the with-child path (the child must have lived with the taxpayer in the US > half year). Do NOT re-scope it to `numChildren == 0`. Null-tolerant (only an explicit "no" disqualifies). **Verified correct (against Rev. Proc. 2024-40 §2.06 — do NOT "fix"):** earned-income amounts $8,490/$12,730/$17,880/$17,880, max credits $649/$4,328/$7,152/$8,046, non-MFJ phaseout $10,620/$23,350 & MFJ $17,730/$30,470, completed phaseouts $19,104/$50,434/$57,310/$61,555 & MFJ $26,214/$57,554/$64,430/$68,675, statutory rates 7.65/34/40/45 + 7.65/15.98/21.06/21.06, investment-income limit $11,950 (strict >); the earned-vs-AGI dual lookup (min only at/above phaseout start, Pub 596 Wksht A); MFS §32(d)(2) exception; childless 25–64 age gate; SSN-not-ITIN; dedicated EIC qualifying-child count; dependent-own self-claim disallow; ½-SE reduction; combat-pay election. **KNOWN DEFERRED:** §32(c)(1)(B) qualifying-child-of-another bar checked only on the childless path (field conflates it with the §(ii)(III) dependent bar); Form 4852 substitute wages omitted (under-refund); Jan-1 age day-before rule; newborn/died full-year residency (intake-set months=12); relationship-string coverage. Pins: `TaxReturnComputeServiceTest#eicEarnedIncomeExcludesBox11NonqualifiedDeferredComp` + `#eicWithChildRequiresUsResidencyLikeChildlessPath`. See [[feedback_principled_diagnosis_over_test_tweaking]] and [[feedback_prefer_irs_docs_over_web]].

**Form 8863 Education Credits (§25A) — verified vs 2025 IRS instructions** (fixed 2026-08-04, adversarial audit; the actual Instructions for Form 8863 were pdftotext'd — the Credit Limit Worksheet, AOTC-vs-LLC table, and Part III routing come from primary IRS text). (1) **Schedule R (line 6d) is computed BEFORE Form 8863** because the 8863 Credit Limit Worksheet line 5 subtracts Schedule 3 "lines 1, 2, 6d, and 6l" (verbatim). Do NOT move `computeScheduleR`/`applyScheduleRToSchedule3` back after `computeForm8863` — line 6d would read 0 and over-state the education-credit tax limit. Schedule R's own limit (line 21) subtracts only lines 1/2/6l, never education (line 3), so there is no circular dependency; it must still precede Form 5695 (§25C CLW) and Schedule 8812 (CLW-A). (2) **An AOTC-disqualified student falls back to the LLC.** Form 8863 Part III lines 23–26 (4-prior-years / half-time / first-4-years / felony-drug) each route "Stop! Go to line 31 for this student" — the four eligibility disqualifiers add the student's adjusted expenses to `totalLlcExpenses`, they do NOT `continue` to $0 (2025 instructions Example 1 + AOTC-vs-LLC table; §25A(c) imposes none of those gates on the LLC). Line 31 shares the Adjusted Qualified Education Expenses Worksheet with line 27, so the same expense figure is IRS-consistent. (3) **The student-TIN gate bars BOTH credits** (the AOTC-vs-LLC table's "TIN needed by filing due date" row applies to LLC too) — its `continue` correctly drops the student with no LLC fallback, and it runs BEFORE the disqualifier gates so a TIN-less student never reaches the fallback. Do NOT re-add the false "LLC has no student-TIN due-date rule" comment. (4) **The §25A(g)(3) dependent bar keys on BOTH** `claimedAsDependentOnAnotherReturn` (8863 field) AND the return-level `dependentOwnReturn` filing-status flag — a dependent filing their own return cannot claim an education credit even if the 8863 field is null. (5) **The Form 8862 Part IV AOTC recert gate joins by SSN/TIN, not positional index** (upgraded 2026-08-04, user-authorized field-add). The gate builds a normalized-TIN → eligibility map from the parallel `Form8862.aotcStudentTins` / `aotcStudentEligible` lists and matches each Form 8863 education student by `studentTinLine21` — order-independent, so a mis-ORDERED or differently-sized recert list gates the correct student. No matching recert entry (or a student with no TIN) fails CLOSED. When TINs are absent (older returns saved before the field existed) it falls back to the positional index, which ALSO fails closed on an out-of-bounds index. The student TIN is captured on the Form 8862 Part IV intake two ways: **auto-filled** from the education-credits student when picked from the dropdown (`existingAotcStudentTinByName`), and via a **manual SSN/ITIN input** shown on the "Someone else" (free-text name) path so a student not on the education-credits list still feeds the join instead of falling to the fail-closed fallback. Persisted via `out_form_8862.aotc_student_tins_json` (V232), mirroring `aotc_student_eligible_json`. Do NOT revert the gate to a bare positional index. An 8862-blocked student is still dropped with no LLC fallback (deliberate, rare recert path). **Not silent:** when a student is blocked *because their SSN matched no recert entry* (no TIN on file / positional overflow) — as opposed to a matched entry marked ineligible — the gate emits a NON-BLOCKING advisory flag `AOTC_STUDENT_NOT_MATCHED_TO_FORM_8862_RECERT` naming the student, so a missing-SSN drop is visible and fixable rather than a silent under-credit. A genuinely-ineligible match does not flag. Pin: `TaxReturnComputeServiceTest#form8863_form8862Gate_unmatchedStudentBlockedAndFlaggedNotSilent`. Pin: `TaxReturnComputeServiceTest#form8863_form8862Gate_tinJoinGatesCorrectStudentWhenMisordered` (+ `#form8863_form8862Gate_shorterRecertList_failsClosedForUnlistedStudent` for the positional fallback). Pins: `TaxReturnComputeServiceTest#form8863_creditLimitWorksheet_subtractsScheduleRLine6d` + `#form8863_aotcDisqualifiedStudent_fallsBackToLlc` + `#form8863_dependentOwnReturn_barsEducationCredit` + `#form8863_form8862Gate_shorterRecertList_failsClosedForUnlistedStudent`. See [[feedback_principled_diagnosis_over_test_tweaking]] and [[feedback_prefer_irs_docs_over_web]].

**§199A QBI (Form 8995/8995-A, line 13a) — SE-deduction allocation weights by SE income, not the QBI base** (fixed 2026-08-04, adversarial audit; both engines otherwise verified clean vs the 2025 forms). In `buildScheduleCQbi`, the ½SE / SE-health / SE-retirement deductions (Reg §1.199A-3(b)(1)(vi)) are apportioned across a person's SE businesses by `ScheduleCQbiComponent.seBase` — the business's SE INCOME — NOT by `netProfit`, which is the QBI base and includes the §1245/§1250 QBI-only Form 4797 recapture (not SE income, §1402(a)(3)) and statutory-employee income (no SE tax). `seBase` defaults to `netProfit` (so QJV / Schedule F components are byte-identical) and is set to `statutory ? 0 : netProfit(pre-QBI-recapture)` at the recapture injection site. The QBI itself is still `netProfit − allocated`. Do NOT revert the allocation weight to `c.netProfit`. Aggregate QBI is unchanged either way (below-threshold returns unaffected); the fix corrects the per-activity split that matters above the threshold when wage/UBIA caps bind. Pin: `TaxReturnComputeServiceTest#qbiScheduleC_seDeductionAllocationExcludesQbiOnlyRecapture`. **Verified correct (do NOT "fix"):** below-threshold netting + QBI/REIT-PTP dual negative→carryforward tracks + taxable-income limit (net cap gain = min(Sch D 15,16) both-positive + qualified dividends); above-threshold W-2/UBIA greater-of (50% vs 25%+2.5%), phase-in reduction (excess × phase-in%, larger income → larger reduction), SSTB applicable-% before loss netting, threshold routing (strict >, taxable-income-before-QBI). **KNOWN DEFERRED (LOW/conservative):** safe-harbor rental QBI recomputed in §469 isolation (allowed/suspended split can diverge from pooled Schedule E — usually UNDER-deduction; deliberate spouse-separation design); no elective aggregation (Schedule B; non-aggregated default is correct absent an election). See [[feedback_principled_diagnosis_over_test_tweaking]].

**§199A QBI — safe-harbor rental (pooled §469) + elective aggregation (Reg §1.199A-4)** (built 2026-08-04, closing the two deferred audit items). (1) **Safe-harbor rental QBI = the MARGINAL contribution to the POOLED Schedule E** (`rentalScheduleE.netIncome` minus the pooled netIncome recomputed with the safe-harbor rental(s) removed) — NOT an isolated `computeRentalScheduleE` that ignores the passive-K1/farm offset and mis-sizes the shared $25k allowance. Byte-identical for a single-rental pool; both-spouses-elect splits by isolated-net magnitude. Do NOT revert to the isolated recompute. Pin `qbiSafeHarborRental_usesPooled469AllowedNetNotIsolatedRecompute`. (2) **Elective aggregation** — `compute8995AQbiDeductionComponent` groups activities sharing a non-null aggregation label, summing QBI + W-2 + UBIA before the wage/UBIA limit; SSTBs are never aggregated (§1.199A-4(b)(1)(iii)) and stay singletons. Each activity carries an aggregation KEY (K-1 entity EIN via `k1AggregationKey`, Schedule C business name via `ScheduleCQbiComponent.aggregationKey` ← `BizIntermediate.businessName`); `computeLine13a` translates key→group LABEL via the QBI-deduction form's `aggregationGroups` (list of {businessKey, groupLabel}), persisted as `pf_qbi_deduction.aggregation_groups_json` (V233). The election lives ENTIRELY on the QBI-deduction personal form, keyed by the business's existing name / entity EIN — **do NOT add an aggregation field to any K-1 statement** (statement/tax-return forms are immutable). The UI auto-derives the key (reads the K-1 entity name+EIN from the statement) so matching is exact, not typed. `normalizeQbiBusinessKey` (lowercase, alphanumeric-only) normalizes both sides. Pins `qbiAggregation_combinesWagesAcrossGroupedBusinessesAboveThreshold` + `qbiAggregation_twoK1BusinessesSameGroupRaiseDeductionEndToEnd`. **Deliberately excluded from aggregation (correct scope, NOT gaps — do not "fix"):** (a) **SSTB businesses** — §1.199A-4(b)(1)(iii) prohibits aggregating a specified service trade or business; the compute forces any SSTB activity to a singleton regardless of its assigned label. (b) **§199A safe-harbor rentals** — they enter QBI as `qbiActivityNoSe(..., null, null, ...)` with no W-2/UBIA attached, so aggregating them cannot change the wage/UBIA limit (and above-threshold UBIA for safe-harbor rentals is itself a separate documented follow-up); they carry a null aggregation key and stay singletons. Farm (Schedule F) activities likewise carry no aggregation key today. See [[feedback_confirm_before_field_add_delete]].

**Form 8962 Premium Tax Credit (§36B) — audit fixes** (2026-08-04, verified vs the actual 2025 Form 8962 + instructions). (1) **Part IV allocation percentages are DECIMAL fractions** (0.50 = 50%, per the IRS boxes + the intake); `allocationFraction` clamps to [0,1] and does NOT divide by 100. Do NOT re-introduce the `/100` — it scaled every shared-policy allocation 100× too small (under-credit AND under-repayment). (2) **Hawaii FPL increment = $6,190/person** (`FORM8962_FPL_HAWAII_STEP`; Table 1-3: 1=$17,310, 2=$23,500, 3=$29,690) — not $6,230. (3) **Alternative calculation for year of marriage**: when elected AND line 24 > line 25, force line 26 = 0 (skip 27-29) — do not post the full-year net PTC to Schedule 3 line 9. (4) The **Pub 974 SE-health↔PTC solver is gated on `!ptcClaimableAsDependent`** — a §36B(c)(1)(D)-barred dependent has no PTC, so no circular solve (else a phantom credit lowers the §162(l) deduction → over-tax). (5) `computeForm8962` receives **`isMfsReturn ? null : form2555Spouse`** — the other spouse's §911 exclusion must not inflate an MFS leg's household MAGI (matches the NIIT/13b guards). **Verified correct:** Table 5 caps + ≥400% no-limitation + MFJ doubling + Single-ONLY column (MFS uses the doubled column — the `getRepaymentCap` "Single or MFS" comment was stale); the 2025 applicable-figure table with **no 400% cliff** (8.5% cap over 400%, IRA-extended through 2025); MAGI §103/§911/nontaxable-SS add-backs; monthly PTC (D=max(0,B−contribution), E=min(A,D)); MFS net-PTC disallowance while retaining the excess-APTC repayment; refundable net PTC → Schedule 3 line 9, repayment → Schedule 2 line 1a. **MFS Allocation-Situation-2 substitute-SLCSP (BUILT 2026-08-04):** the optional per-shared-policy field `substituteMonthlySlcspAmount` (on the premium-tax-credit personal form; persisted `pf_ptc_shared_policy.substitute_monthly_slcsp_amount`, V234) replaces the allocated 1095-A column B with the filer's OWN coverage family's monthly SLCSP for the allocated months when > 0 (col A/APTC still allocated by %); null → allocate col B by the percentage. Pin `form8962SharedPolicySubstituteSlcspReplacesAllocatedColumnB`. NOTE: the UI stores the allocation percentages AS DECIMALS (it converts percent↔decimal on save/load, `percentToDecimalAllocation`), so the backend `allocationFraction` correctly treats the stored value as the final [0,1] fraction. Pins `form8962SharedPolicyAllocationUsesDecimalFractionNotDividedBy100` + `form8962HawaiiFederalPovertyLineIncrementIs6190`. See [[feedback_prefer_irs_docs_over_web]].

**Social Security benefits worksheet (lines 6a/6b, §86 / Pub 915) — MFS spouse-exclusion guard** (fixed 2026-08-04, adversarial audit; core verified vs actual Pub 915 Worksheet 1). In `computeSocialSecurityBenefits`, the spouse-side Form 2555 (§911) AND §931/§933 possession income exclusions in Worksheet 1 line 5 carry `isMfsReturn ? null : …` — the non-filing spouse's exclusions must NOT inflate an MFS filer's provisional income (matches the three sibling MAGI add-back guards at ~:3075 Pub 590-A, ~:3422 SALT, ~:23975 Sch 1 8d). Do NOT drop these guards (the `possession-residence-exclusion-spouse` form is un-prefixed + read unconditionally, so it IS reachable — an earlier comment falsely claimed the upstream scoper nulled it). Pin `mfsSocialSecurityWorksheetExcludesSpousePossessionExclusion`. **Verified correct:** two-tier 50%/85% + $25k/$32k base + $34k/$44k second tier; MFS-lived-with-spouse $0 base (`min(0.85·w1, 0.85·w7)`); 85% cap + zero-floors; line 6b self-excluded from WS line 3; §86(b)(2)(B) tax-exempt-interest + §137 adoption (excluded-benefits) add-backs; Pub 915 Schedule-1 subset (11-20,23,25, excl. student-loan interest); Form 8815 carve-back; IRA-coordination single-subtraction; §86(e) lump-sum election (opt-in, never worse than normal, combined box-5, per-year recompute); RRB-1099 tier-1 SSEB → line 6a; box-4 repayment netting; line 6b → line 9 once. **KNOWN DEFERRED (negligible/scope):** pre-1994 lump-sum uses the modern two-tier worksheet not Pub 915 Worksheet 3 (~32-yr retroactive SSA payment ≈ impossible); §86(e) election read from the taxpayer SS form only on joint returns (defensible return-level design). See [[feedback_mfs_spouse_prefixed_form_scoper_unprefix]].

**Form 8959 Additional Medicare Tax — line 19 includes W-2 box 12 codes B & N** (fixed 2026-08-04, adversarial audit; Parts I/II/III + flow otherwise verified clean vs the 2025 form). Part V line 19 (Medicare tax withheld) = W-2 box 6 PLUS box 12 code B (uncollected Medicare tax on tips) + code N (uncollected Medicare on group-term life > $50k, former employees) — `sumW2Box12ByCodes(w2Entries, {B,N}, ssn)`, SSN-scoped like box 6 (both spouses on MFJ). Do NOT drop the box-12 term (omitting it under-credits Form 1040 line 25c). Line 22 = whole-dollar (rounded) line 19 − line 21 (line-by-line convention). **Verified correct (do NOT "fix"):** thresholds $200k/$250k/$125k by status; MFJ two-spouse aggregation vs the single threshold; Part II line 8 = Schedule SE §1402 net earnings (single 0.9235, SE-loss→0); the wage↔SE shared-threshold reduction (line 10 = Part I wages, line 11 = threshold − wages, floored); Part III RRTA own full threshold (no wage reduction, "no equivalent rule for RRTA") + no double-count into Part I; line 18 = 7+13+17 → Schedule 2 line 11 once via the G3 supersede; Part V line 20 = line 1 (NOT line 4) × 1.45% regular Medicare; line 24 → 1040 line 25c once (box 6 never on 25a). Follow-up (2026-08-04, all three deferred items built): (a) lines 7/13/17 each `roundMoney`-wrapped (line 18 = sum of whole-dollar lines, IRS line-by-line); (b) Part III line 14 on MFJ sums ALL household RRTA (`sumW2RrtaCompensationForSsn(w2Entries, null)`, matching line 1 — a blank/mismatched-SSN railroad W-2 is no longer dropped; pin `form8959Line14MfjSumsAllRrtaIncludingBlankSsnW2`); (c) the redundant Part-I-only `computeAdditionalMedicareTax` estimate is REMOVED — Schedule 2 line 11 is set solely from Form 8959 line 18 (do NOT re-add a separate estimate). Pin `form8959Line19IncludesBox12CodeBAndNUncollectedMedicare`. See [[feedback_prefer_irs_docs_over_web]].

**Form 8962 line 10 — the annual calculation is DIRECTIVE, not optional** (built 2026-08-30, QA sc_00142). Line 24 reads "Enter the amount from **line 11, column (e)**, OR add lines 12 through 23, column (e)" — which one is settled by line 10, and the Instructions for Form 8962 (2025), "Line 10", *Full-year coverage with no changes on Form 1095-A, Part III, column A or B*, phrase it as a command: "Check 'Yes' and continue to line 11 if all of the following apply … **Otherwise**, check 'No' and continue to lines 12 through 23." The three conditions are (a) enrolled in the qualified health plan **all 12 months**, (b) **column A** (enrollment premium) the same every month, (c) **column B** (SLCSP) the same every month. **Column C (APTC) is deliberately NOT in that list** — a mid-year APTC change does not force the monthly path, and line 25 is the same annual sum either way, so ONLY line 24 turns on the election. Do NOT "simplify" the engine back to always summing the monthly rows: the paths differ because line 8b rounds the monthly contribution to a whole dollar, so twelve monthly rows can subtract more contribution than line 8a does once (sc_00142: line 8a 1,148 → line 8b 96 → 12 × 96 = 1,152, a **$4 under-credit** — 4,248 instead of the correct 4,252, propagating to net PTC and refund). **Line 9 takes precedence**: a Part IV shared-policy allocation or the Part V alternative calculation for year of marriage routes to those parts and never uses line 11 — `useAltCalc || !allocationByPolicy.isEmpty()` forces the monthly path. `allMonthlyValuesEqual` treats null as zero so a blank month cannot masquerade as uniform with a populated one, and an empty row list is not uniform. Line 11 is computed on BOTH paths (the monthly rows are its only source, and having both available makes the election auditable) — a filed PDF must print only the elected path, keyed off `line10UseAnnualCalculation`, never both. Persisted `out_form_8962.line10_use_annual_calculation` (V243). Pins: `Sc00142SqaScenarioTest#line10ElectsTheAnnualCalculationForFullYearUniformCoverage` + `#line10FallsBackToMonthlyWhenSlcspVariesMidYear`. **Naming trap (cosmetic, do NOT trust the Java names):** `Form8962.line5FederalPovertyLine` holds form line **4**; `line6HouseholdIncomeAsPctOfFpl` holds form line **5** (form line 6 is "Reserved for future use"); `line10EmployerCoverageAffordability` is NOT form line 10 — it is the employer-coverage flag, kept under that name only for persistence compatibility. The PDF semantic map (`f8962_field_map_semantic.csv`) uses the correct form numbering, and these model fields never reach it. See [[feedback_prefer_irs_docs_over_web]].


**§72(t) additional tax is charged on the INCLUDIBLE amount, never on box 2a** (fixed 2026-09-15, QA sc_00344). In `computeIraDistributionsForPerson` the code-1 (10%) and code-S (25%) bases are accumulated per 1099-R entry from `taxableAmountAmount`, but the line-4c exclusions - `totalIraRolloverAmount`, the capped QCD, `totalHfdAmount` - and any Form 8606 basis recovery are applied later and at FORM level to line 4b. A ceiling now caps the combined base at the person's final `taxableAmount` (= line 4b) before it is stored on the per-person record. Do NOT remove it: without it an ordinary VALID 60-day indirect rollover was taxed 10% on the whole gross, because the payer cannot know money will be rolled over and therefore stamps box 7 code 1 with box 2a = gross - and since the blocking `FORM5329_REQUIRED_FOR_EARLY_DISTRIBUTION` flag keys on a positive penalty, the filer could not file without accepting the charge. Authorities, all three saying the same thing: i5329 line 1 "Enter the amount of early distributions **includible in income**"; Pub. 590-B "The additional tax on early distributions is 10% of the amount of the early distribution **that you must include in your gross income**"; Pub. 590-B "The 10% additional tax applies to **the part of the distribution that you have to include in gross income**"; plus §408(d)(3)(A)(i) excluding a valid rollover from income. **Capped, not recomputed per entry, on purpose** - the exclusions are entered as form-level totals, so which 1099-R a rollover came from is not knowable; the overflow comes off the 10% base first and only then the 25% base, mirroring how the line-2 exception amount is applied just above. The result is set to **NULL, not ZERO**, when fully excluded (canonical null/zero semantic - §72(t) does not apply at all, and null is what stops Form 5329 being demanded). **Do NOT infer that the app polices §408(d)(3)(B)** (one rollover per 12 months): it cannot - the limit aggregates every IRA at every institution, so the taxpayer determines it and simply does not report the failed rollover as a rollover. H&R Block asks the question and ignores the answer (proved by A/B pair in `us-tax-hrb`); we do not ask. Pins: `Sc00344SqaScenarioTest#aValidRolloverOwesNoEarlyDistributionTaxAndDoesNotBlock` + `#aPartialRolloverIsPenalisedOnTheUnrolledBalance`. See [[feedback_two_divergent_actuals_trees]], [[canonical-null-zero-semantic]].


**Form 1116's two ratios are BOTH rounded to four decimal places** (fixed 2026-09-15, QA sc_00345). Line 3f (gross foreign ÷ gross all sources, apportioning the standard deduction) and line 19 (the §904(a) limitation fraction, line 17 ÷ line 18) must be figured the same way. Line 3f used to `divide(..., 10, HALF_UP)` while line 19 used `divide(..., 4, HALF_UP)`, giving 3,239 on sc_00345 where four-at-both gives 3,237 and full-precision-at-both gives 3,238. **Be accurate about the status of the old behaviour: it was defensible, not invalid.** "At least four decimal places" is a FLOOR, and line 3f is not printed on the form, so a filer carrying 3f at full precision and line 19 at the printed four decimals produces a return that foots. Four is chosen because it is the only worked example the IRS gives and because it matches H&R Block in both Actuals trees — a deliberate convention alignment, not an arithmetic repair. **Line 19 is the half that is genuinely constrained:** the fraction is rendered onto the filed form (`line19_divide_line_17_by_line_18`), so line 21 MUST be computed from the same four decimals that print, or an examiner multiplying line 20 by line 19 gets a different credit than the form states. Authority, with the IRS's own worked example — Instructions for Form 1116, "Line 3f": "Divide line 3d by line 3e and **round off the result to at least four decimal places** (for example, if your result is 0.8756782, round off to 0.8757, not to 0.876 or 0.88). **Enter the result, but don't enter more than '1.'**" — hence the `.min(BigDecimal.ONE)` on 3f as well. The ratio is rounded FIRST and then multiplied by line 3c, so 15,750 × 0.1429 = 2,250.675 → **2,251**, not the exact 2,250. Do NOT restore the ten-place divide "for accuracy": it silently over-credits, and it made us disagree with H&R Block in both Actuals trees. Cross-checked by a second scenario in a different basket — sc_00337's general basket became H&R Block's 142,392 (apportioned 7,608) with no other change. **Two further Form 1116 facts worth not re-deriving:** line 19 divides line **17** by line 18 (not line 15), and **line 18 is not simply Form 1040 line 15** — the form says "Form 1040 line 11b minus line 14, **plus Schedule 1-A (Form 1040), line 37**", so the 2025 tips / overtime / car-loan-interest / enhanced-senior deductions are added BACK into the §904 denominator. Pins: `Sc00345SqaScenarioTest#bothLimitationRatiosAreRoundedToFourDecimalPlaces` + `#theStandardDeductionIsApportionedAgainstForeignSourceIncome`. See [[feedback_a_red_test_may_be_indicting_the_last_fix]], [[feedback_prefer_irs_docs_over_web]].


**Form 1116 Part I line 3 renders as a complete block, and line 3f is PERSISTED** (built 2026-09-15). The form apportions not-definitely-related deductions across seven sub-lines, each with column A/B/C boxes — 3a deduction in, 3b other, 3c sum, 3d gross foreign, 3e gross all sources, 3f = 3d/3e, 3g = 3c × 3f. The engine formerly kept only the 3g result (`pro_rata_deductions`) and discarded 3a/3e/3f as compute-loop locals, and the renderer wrote that result onto **line 3a** — the box for the deduction going IN — leaving 3c–3g blank. V254 adds `apportionable_deduction_line3a`, `gross_income_all_sources_line3e` and `apportionment_ratio_line3f` to `out_form_1116_country`. **Do NOT "simplify" by re-deriving 3f in the renderer:** line 3g is computed from the ratio ROUNDED to four decimals, so a full-precision 3f would print a fraction that does not reproduce the printed 3g (15,750 × 0.142857 = 2,250, but 3g shows 2,251 from 0.1429) and the filed page would not foot line by line. Store what the arithmetic actually used — the same rule as the line 19 fraction, which is likewise printed. 3c and 3d need no columns: 3d **is** `gross_foreign_income` (the same figure as line 1a) and 3c = 3a + 3b with 3b unmodelled. Both UI repos (`us-tax-ui`, `us-tax-return-forms`) carry the mapping and must be changed together. Pin: `Sc00345SqaScenarioTest#theLine3ApportionmentBlockIsCompleteAndFootsAsPrinted` re-does 3c × 3f off the stored values and requires it to reproduce 3g. See [[feedback_rounding_claims_need_the_printed_form_test]].


**Form 1116 Part II — `taxesPaid` is a BOOLEAN and drives the (j)/(k) checkboxes, never a money column** (fixed 2026-09-15). `Form1116Country.taxesPaid` records paid (cash) vs accrued basis. The Angular type had it as `number | string | null` and the renderer pushed it through the amount formatter into column (p), the foreign-currency "other taxes" box, while Part II's "Credit is claimed for taxes (**you must check one**)" pair went unrendered — a mandatory box blank on every filed form. Both checkboxes were already in the semantic map as `part2_credit_claimed_for_taxes_paid_checkbox` / `part2_credit_claimed_for_taxes_accrued_checkbox`; no asset regeneration is needed. It is a **form-level** election (compute derives it from the category's `creditBasis` and stamps every country with it), so read the first country. **Do NOT regenerate the f1116 semantic assets to "add" fields:** `generate-semantic-f1116.js` derives names mechanically from the AcroForm path and would overwrite the hand-curated names (`category_passive_income`, `line3g_multiply_3c_by_3f_country_a`, …) with `page1_0_part2_0_active_header_elements_0_c1_3_0`-style junk. **Columns (m)–(p) (foreign currency) are deliberately left BLANK** and the USD figure sits in the residual **(t)**: the intake collects one unclassified `foreignTaxesPaidUsd` per country, and column (u) is defined as "add cols. (q) through (t)" — USD only — so the row total and line 8 are right regardless. Splitting (q) dividends / (r) rents and royalties / (s) interest needs per-country breakdown fields on the intake form; the import-time `importedDivForeignTax` / `importedIntForeignTax` split is return-level and cannot be attributed to a country, so do not infer from it. Pins: `Sc00345SqaScenarioTest#theCreditBasisElectionReachesTheCountryRows` + `#theForeignTaxIsCarriedInUsDollarsAndTotalsTheRow`.


**Form 1116 Part II breakdown — when the filer gives the split, the split is AUTHORITATIVE** (built 2026-09-15, V255/V256). Eight optional per-country intake fields carry the withholding-type split: `foreignTaxWithheld{Dividends,RentsRoyalties,Interest}Usd` + `otherForeignTaxesPaidUsd` (columns (q)-(t)) and the same four suffixed `ForeignCurrency` (columns (m)-(p)). **When ANY U.S.-dollar box is present the country's total is DERIVED from the four**, replacing the legacy single `foreignTaxesPaidUsd`, and that governs the credit as well as the render — because the form defines column (u) as "add cols. (q) through (t)", so any other total prints a row whose parts do not add up. A disagreeing legacy figure raises the non-blocking `FORM_1116_COUNTRY_TAX_BREAKDOWN_DISAGREES_WITH_TOTAL` naming both numbers; it is never silently discarded. **With no breakdown the behaviour is byte-identical to before** (legacy total, printed in the residual (t)) — pinned by `Sc00345SqaScenarioTest#withoutABreakdownTheLegacyTotalIsUntouched`, which is what guarantees no existing return moves. **The foreign-currency columns are disclosure only and must NEVER reach the credit** ("Report all amounts in U.S. dollars except where specified in Part II"); they stay NULL when unanswered so the boxes print blank, not "0.00". **FIVE surfaces, and missing one fails SILENTLY:** the YAML, the Angular model (interface + `emptyCountryEntry` + markup + `normalizeOnLoad` — save spreads the entry so it needs nothing), the intake entity `PfFtcCountry`, and BOTH directions of `ForeignTaxCreditMapper` (`PayloadCoercion.decimal` on save, `putIfNotNull` on load). Miss the mapper and the API accepts the value, the mapper drops it, and the entry vanishes on reload with no error. `Form1116Part2FieldWiringTest` pins all of it. See [[feedback_personal_resource_allowlist]], [[feedback_grep_mapper_for_seed_field_names]].


**Form 2555 — the housing cap trims EXPENSES, and the FEIE is net of the housing exclusion** (both fixed 2026-09-15, QA sc_00347). Two ordering rules that the form states plainly and the engine had wrong. **(1) Line 29b caps the EXPENSES, not the exclusion:** line 30 = "Enter the smaller of line 28 or line 29b", then line 33 = "Subtract line 32 from line 30". Compute `min(housingExpenses, cap) − baseHousing`, never `(housingExpenses − baseHousing)` capped afterwards — the latter silently stops subtracting the base once the cap binds (60,000 of expenses → 39,000 instead of 18,200, a 20,800 over-exclusion). Diagnostic: **a housing exclusion can never equal its own expense limit**; an exclusion landing exactly on 39,000 (or on a high-cost-locality figure like Geneva's 102,600) is the symptom, not a coincidence. **(2) Line 41 = line 27 − line 36:** the FEIE is limited to foreign earned income NET of the housing exclusion, and line 42 = min(line 40 prorated maximum, line 41), floored at zero. Without it the two exclusions can exceed the income they exclude and drive total income NEGATIVE (100,000 of wages + a 9,200 housing exclusion → a 109,200 exclusion). **Therefore the housing exclusion MUST be computed before the FEIE** in `computeForm2555Exclusions` — do not reorder them back. The high-cost-locality limits (Notice 2025-16) are line-29b figures and prorate by qualifying days exactly like the base. Pins: `Sc00347SqaScenarioTest#theHousingCapLimitsExpensesBeforeTheBaseIsSubtracted` + `#theTwoExclusionsTogetherCannotExceedTheForeignIncome`, plus the two corrected `TaxReturnComputeServiceTest#form2555HousingExclusion*` locality tests. See [[feedback_a_red_test_may_be_indicting_the_last_fix]].


**§170(f)(11) charitable substantiation is TWO gates, and the second is useless without the first** (built 2026-09-15). **(a) `FORM_8283_DETAIL_REQUIRED`** — over $500 of `charitableNonCashContributions` with no `donations` rows on the `form-8283` intake. **(b) `CHARITABLE_QUALIFIED_APPRAISAL_REQUIRED`** — a Section B donation row (FMV > $5,000 and not `isPubliclyTradedSecurity`) whose new `hasQualifiedAppraisal` is not TRUE (V257, `pf_form_8283_donation`). **Why both:** the deduction that reaches Schedule A is a single AGGREGATE on the deductions form, but the per-item values that decide whether §170(f)(11)(C) applies exist only in the separate Form 8283 intake — so a filer who claims 9,000 and never opens Form 8283 is invisible to (b). Gate (a) is what makes (b) reachable; do not remove it thinking it is redundant. **Both are blocking but deliberately OVERRIDEABLE** (absent from `NonOverrideableFlags`), matching the §170(f)(12) `CHARITABLE_VEHICLE_1098C_REQUIRED` sibling: the paperwork is the filer's own and may exist unrecorded, so an override is the filer asserting they hold it. **Do NOT zero the deduction** — that would punish the filer who overrides precisely because they do hold an appraisal; the point is that the return cannot pass in silence. `hasQualifiedAppraisal` is nullable and NULL (unanswered) counts as "not held" — the filer is asked, never assumed. The intake shows the question only for Section B rows (`needsAppraisal(d)`), since asking every donor would be noise. Pins: `CharitableQualifiedAppraisalGateTest` (5) + `Sc00300SqaScenarioTest#aNoncashGiftOver5000WithNoAppraisalBlocksButKeepsTheDeduction`. See [[feedback_verify_entry_not_outcome]] — the first draft of the unit test seeded the surface the gate read and passed while the real path stayed wide open.


**§465 at-risk gates PASSIVE K-1 losses, and the field is NOT on the K-1** (built 2026-09-15, V258). `someK1InvestmentNotAtRisk` + `k1AmountAtRisk` live on the §469 companion intake form `passive-activity-carryforward-taxpayer` / `-spouse`, never on the K-1 statement. Two reasons that agree: a K-1 is a STATEMENT and no field may be added to one, and the partnership **does not report the partner's at-risk basis on the K-1 at all** — it turns on the partner's own facts (recourse vs nonrecourse debt, personal guarantees, stop-loss arrangements). The same form already carries the K-1 prior-year suspended passive loss for exactly this reason, so it is the established home for K-1 facts the statement cannot hold. **§465 runs BEFORE §469** — the loss is capped at the at-risk amount and the excess suspended *before* it reaches the passive pool; skipping that releases the excess a year early once passive income appears (over-deduction). The suspended amount joins Schedule C and Schedule F in the Form 6198 carryforward, so it is DEFERRED, not lost. **Aggregate and OPT-IN:** null/unanswered means fully at risk and no limit, matching the Schedule C / F gate — never limit a filer who did not answer. **Seeding trap:** the engine reads `part3Line1OrdinaryBusinessIncomeLoss` (slot→semantic naming), not `ordinaryBusinessIncome`; a test seeded with the wrong name produces no loss at all and the gate silently "passes". Pins: `PassiveK1AtRiskGateTest` (4). See [[project_k1_slot_semantic_rework]], [[feedback_verify_entry_not_outcome]].


**§6654(i)(2) farmer gate — GROSS on both sides, and EITHER year qualifies** (completed 2026-09-16, V259). The ⅔ test compares gross farm income to TOTAL GROSS income, and Form 1040 line 9 is not total gross income: it carries NET for the farm (Schedule F line 34), for Schedule C (line 3) AND for Schedule E (line 5). All three are added back — the farm from `grossFarmForF2210`, Schedule C from `taxpayerGrossNonfarm + spouseGrossNonfarm`, Schedule E by summing each property's `rents + royalties` off `rentalScheduleE.properties`. Do NOT drop any of the three: each omission understates the denominator and admits a filer the statute measures at 90% (gross farm 100,000 vs a 200,000-receipt / 190,000-expense business is 33%, but 91% if the business is read at net). A LOSS makes the net negative and subtracting it correctly RAISES gross income — a deduction never shrinks gross. **§6654(i)(2)(A) — either year:** the test may be met on the current OR the preceding year, so the gate is `currentYearFarmerFisher || priorYearFarmerFisher`. The preceding year's figures are filer-supplied (`priorYearGrossFarmIncome` / `priorYearTotalGrossIncome` on the prior-year intake form) because they cannot be derived — last year's gross farm income is not on this return, and the stored `priorYearAgi` is NET and cannot answer a GROSS ratio. Both optional; blank = current-year-only, unchanged. Pins: `Form2210FGrossIncomeGateTest` (5). See [[feedback_input_side_statutory_reduction_audit_lens]].


**Pub. 570 has TWO different proration fractions — do not use one for the other** (SE half added 2026-09-16). **(a) The general fraction** — gross income subject to U.S. tax ÷ gross income from all sources including excluded territory income — prorates the standard deduction, itemized total and line 12e (`territoryAllocationFraction`, 2026-09-10). **(b) The self-employment fraction** — SE income subject to U.S. tax ÷ total SE income including excluded — prorates ONLY the §1402 half-SE-tax deduction on Schedule 1 line 15 (`territorySelfEmploymentFraction`). A filer whose excluded territory income is all INVESTMENT income prorates their standard deduction while line 15 stays whole; using (a) on line 15 gives a different, wrong answer that a test seeded only with excluded business income cannot distinguish — pin the investment-only case. Pub. 570: "if you … exclude ALL of your self-employment income … you cannot take the deduction on Schedule 1 (Form 1040), line 15" and "If only part … is excluded, the part of the deduction that is based on the nonexcluded income is allowed." **Both sides of (b) are GROSS on purpose**: the possession form captures `businessGrossIncomeExcluded` / `farmGrossIncomeExcluded`, so the numerator uses Schedule C receipts + Schedule F gross. A gross denominator against a net numerator is the distortion that made the §6654(i)(2) farmer gate over-permissive. Computed BEFORE the adjustments are summed, so Schedule 1 line 26 and AGI carry the allowed figure. **Verified NOT prorated:** the enhanced senior deduction (Pub. 570 says claim the full amount, though its MAGI test counts excluded income), QBI (definitely related to business income), and the **IRA deduction** — "Do not take excluded income into account"; possession exclusions are correctly absent from the Pub. 590-A MAGI add-backs, which list Form 2555 only. Pins: `TerritorySelfEmploymentDeductionTest` (5).


**Schedule 1-A under a §931/§933 exclusion — each of the four parts is governed DIFFERENTLY** (line 30 fixed 2026-09-16). Never prorate `line38Total` wholesale. **Line 30 car loan interest** — apportioned by the GENERAL Pub. 570 fraction ("you generally must apportion the deduction between the income included in your U.S. tax return and all other income"), AND its MAGI counts excluded income; the MAGI half was already right, only the apportionment was missing. **Line 37 enhanced senior** — MAGI counts excluded income but the allowed amount is claimed in FULL; never multiply it by the fraction. **Lines 13/21 tips and overtime** — SOURCE-restricted, not apportioned: allowed "only with respect to the qualified tips or qualified overtime compensation that you did not exclude from your U.S. gross income". That needs a territory-source split of the tips/overtime the intake does not capture, so they are deliberately untouched — do NOT approximate them with the general fraction. **Testing trap:** adding excluded territory income legitimately SHRINKS the senior deduction through the 6%-over-$75,000 phaseout, because its MAGI test counts that income — so a plain before/after comparison cannot tell proration from phaseout and will fail for the wrong reason. Use a SAME-MAGI control instead (excluded territory interest vs equal U.S. interest produce identical Schedule 1-A MAGI, since the excluded amount is added back), leaving the territory fraction as the only difference. Pins: `TerritorySelfEmploymentDeductionTest#theCarLoanInterestDeductionIsProratedByTheGeneralFraction` + `#theEnhancedSeniorDeductionIsNotProrated`.


**Pub. 570 territory proration happens at the INPUT for any Schedule A line with a floor or a cap** (2026-09-16). "Enter on Schedule A (Form 1040) only the allowable portion of EACH deduction" — and the worked example prorates medical on line 1 and real estate taxes on line 5b, letting the floor and the cap act on the already-reduced figure. **This is not presentational.** For a plain multiplication, prorating the input or the total gives the same answer; with a floor or cap between them it does not, and it errs in BOTH directions: medical 11,000 at AGI 96,000 and fraction 0.8 is 1,600 the right way and 3,040 the wrong way (over-deduction), while SALT of 60,000 against the 40,000 OBBBA cap is 40,000 the right way and 32,000 the wrong way (under-deduction). `buildScheduleA` therefore prorates `medicalDentalExpensesPaid` and all four SALT components at read time, and `computeLine12` prorates only the REMAINDER — `itemized − line4 − line5e` — adding those two lines back unchanged so nothing is prorated twice. **Do not restore a whole-total proration**; it silently reintroduces both errors. Still open and deliberately untouched: **charitable contributions**, which carry their own AGI-percentage ceilings and carryovers and so have the same order sensitivity — rebuilding them belongs with the charitable machinery, not with this change. Pins: `TerritoryPerLineScheduleATest` (5), including a no-exclusion control for each direction.


**Charitable gifts are prorated BEFORE the §170(b) ceilings, and prior-year carryovers never are** (2026-09-16). Pub. 570's example prorates the cash gift and enters the reduced figure on line 11, so the AGI percentage ceilings act on it. Prorating the total instead puts the ceiling first, and because whatever the ceiling disallows becomes a §170(d)(1) carryover the error lands in TWO years: an 80,000 gift at AGI 96,000 with fraction 0.8 is 57,600 allowed / 6,400 carried the right way, and 46,080 allowed / 22,400 carried the wrong way — 11,520 of under-deduction now plus a 16,000 carryover overstatement that flows into the next return through the carryforward bridge. At a 60,000 gift the wrong order even INVENTS a 2,400 carryover from a gift that was fully deductible. The six CURRENT-year buckets are prorated immediately before `computeCharitableDeduction` — the single entry point into the ceiling engine, so none of the ~15 individual inputs or the bucket machinery needs touching — and `computeLine12`'s remainder proration subtracts `getTotalCharitableContributions()` alongside medical and SALT so nothing is doubled. **Never prorate `priorCharityDetail`:** a vintage arising in an earlier territory year was already reduced when created, and reducing it again compounds every year it survives. Pin the CARRYOVER, not just the deduction — it is the half that leaves the year. Pins: `TerritoryPerLineScheduleATest#whenTheCeilingBindsBothTheDeductionAndTheCarryoverAreCorrect` + `#withoutAnExclusionTheCeilingBehavesNormally`.


**§280A(c)(5) rental depreciation carries forward — and it must be PERSISTED to do so** (V260, 2026-09-16). On a personal-residence rental (vacation home or below-market let) §280A caps deductions at gross rental income in three tiers — mortgage interest/taxes, operating expenses, then depreciation — so depreciation is what gets squeezed out, and the disallowed amount carries to the succeeding year with NO expiry (unlike the 5-year §170(d)(1) charitable carryover). The figure was computed and set on `Schedule1AdditionalIncome` but had no column, no mapper reference and no importer, so it was discarded on save and forfeited permanently on EVERY return. `add_inc_rental_depreciation_carryforward` on `out_schedule_1` + `importedPriorYearRentalDepreciationCarryover` (mirroring `importedPriorYearRentalAtRisk`) complete the bridge; `computeRentalScheduleE` spends the carry-in against each §280A property's headroom AFTER current-year depreciation and rolls the remainder forward. **It can never create a loss** — the gross-rental-income cap is the point of the subsection. Aggregate, matching how it is stored: one figure out, one figure back, applied across §280A properties in order. **Rental seeding field names** (a repeat of the K-1 slot trap): the engine reads `mortgageInterestAndPropertyTaxes`, `otherOperatingExpenses` and `depreciationAmount` — NOT `mortgageInterest` / `repairs` / `depreciation`. Seeded with the wrong names no §280A squeeze occurs at all and the carryforward is null, so a loose assertion passes against an inert fixture. Pins: `Section280ADepreciationCarryforwardTest` (4). See [[feedback_verify_entry_not_outcome]], [[project_carryforward_bridges]].


**K-1 boxes 2/3 (rental) are PER SE passive — never gate them on material participation** (routed 2026-09-16). `sumPassiveK1RentalNet` sums `part3Line2NetRentalRealEstateIncomeLoss` + `part3Line3OtherNetRentalIncomeLoss` across 1065 and 1120-S entries and adds the result to `passiveK1Net`. **Do NOT copy box 1's `materiallyParticipatedInActivity` gate onto them.** §469(c)(2) makes a rental activity passive however much the partner participates; gating would let a partner who answered "yes" put a rental loss straight on Schedule 1 line 5 with no §469 limit. **No $25,000 special allowance either:** that requires ACTIVE participation and §469(i)(6)(C) denies it for a limited partnership interest, and the K-1 does not disclose general-vs-limited — so these feed the "other passive" pool, not the rental-real-estate pool. 1041 has no equivalent box (a trust reports rental on its own Schedule E and passes the net through box 5/6, already routed). Before this the boxes were captured and read by NOTHING: income escaped tax, losses were forfeited. Pins: `K1RentalBoxRoutingTest` (4) — including that a rental loss is suspended against wages but DOES absorb other passive income, which is what distinguishes "reached the §469 pool" from "dropped on the floor".


**Form 4684 Section A — the Schedule D figure is LINE 15, never line 16** (fixed 2026-09-16). Line 15: "If line 13 is more than line 14, enter the difference here **and on Schedule D**. Do not complete the rest of this section." Line 16 = "Add lines 13 and 15. Subtract the result from line 14" — the residual LOSS, which line 17 floors at 10% of AGI and line 18 routes to Schedule A. You never reach line 16 when there is a gain. The engine read line 16 and added a positive value to Schedule D line 11 as a long-term gain, **taxing a casualty loss as a capital gain** while the real gain on line 15 escaped Schedule D entirely — and "positive line 16" is the ORDINARY shape, since the form only asks for it when there is a net loss. **The field name fossilises the misreading:** `sectiona_line16_add_lines_13_and_15_subtract_line14` inverts the form's "subtract THE RESULT FROM line 14". The name does not change what the filer entered — do not let it argue you back into reading line 16. **§165(h)(5)(B) on this path is the FILER's job**, applied when they compute line 14 (Worksheet 1-1: non-disaster losses deductible to the extent of casualty gains, remaining gain reduces deductible federal casualty losses); we only route. **Still open:** `partaLine18SubtractLine17FromLine16` is persisted and read by nothing, so a filed Form 4684's Section A deduction never reaches Schedule A — unfixed because it would double-count against the guided deductions-form path. Pins: `TaxReturnComputeServiceTest#phase2D_form4684_sectionALine15GainFlowsToScheduleDLine11` (+ the aggregation and negative-line-16 cases). See [[feedback_a_red_test_may_be_indicting_the_last_fix]].


**Form 4684 Section A line 18 → Schedule A, and a filed Form 4684 SUPERSEDES the guided casualty inputs** (routed 2026-09-16). Line 18 already carries the $100-per-event floor, the §165(h)(5)(B) casualty-gain netting (line 14 / Worksheet 1-1) and the 10%-of-AGI floor, so it is the finished Schedule A figure. **The two paths compute the same deduction — never add them.** `computeForm4684SectionALine18Loss` (mirroring the Section B helper, same MFS attribution by `identifyingNumber`) is derived in `computeLine12` and passed to `buildScheduleA`, which REPLACES the guided `personalCasualtyAndTheftLoss` when a positive line 18 exists. Same precedence the dedicated Form 4952 takes over the simple investment-interest fields: the form carries per-property detail, the casualty-gain offset and the $500 qualified-disaster rules the guided inputs cannot express. A disagreement raises the non-blocking `SCHEDULE_A_CASUALTY_FORM4684_SUPERSEDES_GUIDED_ENTRY` naming both figures. Before this, line 18 was persisted and read by nothing, so a filer who entered a real Form 4684 got NO casualty deduction unless they duplicated it through the guided inputs. Pins: `Form4684SectionALine18RoutingTest` (4) — including that with both populated the result is the form's figure and strictly less than their sum.


**Two more carryforward bridges (V261) — and the sweep rule that found them.** `rentalPassiveLossCarryforward` (§469 regular suspended rental passive loss, `out_schedule_1.add_inc_rental_passive_loss_carryforward`) and `nextYearInvestmentInterestCarryforward` (§163(d)(2), Form 4952 line 7, `out_schedule_a.next_year_investment_interest_carryforward`) were each computed, set on an output model, and discarded — no column, no mapper, no importer. The §469 one is the sharper case: **its AMT shadow twin has had a bridge since V200**, so the shadow survived the year while the track it shadows did not. **USER ENTRY WINS in both** — each has a manual path (`priorYearSuspendedPassiveLoss` per property, `priorYearInvestmentInterestCarryover` on the deductions form), and applying the import on top of a typed figure would DOUBLE the carryforward; the guard is `!anyPropertyPriorSuspendedEntered` and `!hasPositiveAmount(priorInvInterestCarryover)` respectively. **THE SWEEP RULE, worth reusing: a carryforward cannot be recomputed.** Every other output figure is rebuilt from this year's inputs on load, so failing to persist it loses nothing — which is why Form 8960 and Schedule H (outputs not persisted at all) were false positives, and why "statement field unread by compute" is not a defect either (statements feed intake forms; 1099-G box 1 is read as `unemploymentRepaymentAdjustment`). Filter to carryforward-shaped fields that compute SETS and nothing persists: 983 → 135 → 3 → 2 real. Pins: `CarryforwardBridgesV261Test` (3). See [[project_carryforward_bridges]].


**★ A NEW CARRYFORWARD MUST BE REGISTERED IN THREE ALLOW-LISTS, and nothing fails when it is not.** Found by the two-year e2e for V260/V261, which failed 3-of-4 on first run against work already committed and reported green. (a) `Schedule1OutputMapper.loadByTaxReturnId` and (b) `ScheduleAOutputMapper.loadByTaxReturnId` are NARROW loaders — they do **not** call `toModel`, they copy a hand-written field list, so a field is invisible to every bridge until copied there; the Schedule 1 one additionally had an early-return returning null unless one of seven *named* fields was set (guard **deleted** — every caller reads one field and null-checks it, so an all-null object already equalled null). (c) `hasAnySchedule1Input`, the gate deciding whether Schedule 1 exists at all, carries escape hatches so a $0 line still persists carryforwards — it listed the §469 regular/AMT/§465 rental ones and **not** §280A depreciation. **(c) is the sharp one: §280A caps deductions AT gross rents, so line 5 nets to exactly $0 whenever the cap binds — precisely and only when a carryforward exists.** V260's column, mapper and importer could never once have fired: dead on arrival, not lossy. **No unit test can see any of this** — the compute half returns the right number (`Section280ADepreciationCarryforwardTest` asserts 6,000 and is right); the loss is downstream at persist/re-read. That is the argument for the two-year e2e. Pins: `carryforward-bridges-v260-v261.spec.ts` (4). See [[project_carryforward_bridges]].


**A carryforward can be destroyed by its CONTAINER going missing — check that shape, not just the happy path.** The §280A bug survived because no test used a shape where the containing line was empty; re-running existing bridge tests would never have found it. Applying the lens deliberately found a second instance: **§163(d)(2) investment interest was lost whenever the filer took the STANDARD deduction**, because `usesScheduleA` dropped Schedule A and took the carryforward with it. Form 4952 line 7 is "subtract line 6 from line 3" — the net-investment-income excess, computed without reference to itemising — and §163(d)(2) carries it forward indefinitely, so its existence never depended on Schedule A. Fixed by retaining Schedule A when a positive carryforward exists (a third instance of the existing `usedForStandardDeductionIncrease` precedent). **Safe because nothing downstream branches on `scheduleA != null`** — all 14 sites, Form 6251 AMTI included, key off `getUsedForItemizedDeduction()`, which is false; verify that before retaining any output object, since a wrong answer there corrupts AMT silently. The allowable portion is still lost, correctly — allowed by §163(d)(1), simply not claimed. Structural audit of all 8 loaders is clean, and GBC/Form 5405 are immune to the Schedule 1 gate by a different mechanism (they create their own `AdditionalIncome`). **Charitable standard-deduction case: RESOLVED 2026-09-16, and my note here was WRONG** — Reg. §1.170A-10(a)(2) says the carryover provisions apply "even though the taxpayer elects … to take the standard deduction in that year", i.e. the carryover IS created, the SAME shape as §163(d), not the opposite. The consumption rule I was half-remembering is §1.170A-10(b)(2) and governs a different year (see below). Pins: `carryforward-bridges-v260-v261.spec.ts` (5).


**The charitable carryover has TWO standard-deduction rules that pull opposite ways — never reason about them as one.** Reg. **§1.170A-10(a)(2)**, CONTRIBUTION year: the carryover **is created** "even though the taxpayer elects … to take the standard deduction in that year" — identical in shape to §163(d). Reg. **§1.170A-10(b)(2)**, CARRYOVER year: the carryover is **consumed** — "treated as paid (but not allowable as a deduction)" up to the ceiling headroom — so it is spent without yielding any deduction and only the remainder survives. Both were lost to the same cause as §163(d): Schedule A dropped whenever it was not the chosen method. **Extending the `usesScheduleA` retention to `getCharitableCarryforwardToNextYear` fixed BOTH** — the consumption arithmetic underneath was already correct, producing the reg's own numbers ($12,000 deemed paid, $6,000 surviving); it was being computed and thrown away, so no new absorption logic was needed. **Pub. 526 lists both rules under "additional rules for carryovers" and then explicitly declines to state them** ("Because of their complexity … they aren't discussed in this publication") — the publication a developer reaches for omits the answer, which is likely how the gap survived; go to the CFR. Test design is load-bearing: AGI **$20,000** so the 60% cash ceiling ($12,000) sits BELOW the $15,750 standard deduction — the only way to have a real §170(b) excess AND take the standard deduction. The pre-existing charitable tests force `deductionElection: 'ITEMIZED'`, which is exactly why they never saw it. Pins: `charitable-carryover-bridge.spec.ts` (+2).


**§911(d)(6) — foreign tax on EXCLUDED income is not creditable, and we were granting the double benefit.** sc_00348: with a Form 2555 exclusion and a Form 1116 credit on the same return, Form 1116 line 12 was never computed, so the engine excluded $130,000 of salary AND credited the whole $46,000 of foreign tax paid on all $230,000 — reproducing H&R Block's un-reduced output line for line (FTC 21,576 not 20,000, total tax 0 not 1,576, refund 2,000 not 424). Reduction = foreign taxes on foreign earned income × excluded ÷ total foreign earned income; applied to the **general category only** (passive tax sits on dividends/interest §911 never excluded), surfaced via `FOREIGN_EARNED_EXCLUSION_TAX_REDUCTION_APPLIED` because line 12 has no output-model field. **★ Do not read the §931/§933 territory advisory ten lines below as precedent for leaving this alone — the difference is the DENOMINATOR.** Pub. 570's territory formula divides by "total income subject to the territory's tax", which a U.S. return does not carry, so naming it for the filer is right; the §911 denominator is total foreign earned income = Form 2555 line 27, already on the return, so it is exact. Pins: `Sc00348SqaScenarioTest` (8), incl. a no-exclusion control. **Still open:** Form 1116 line 1a should carry "wages not excluded on Form 2555" (100,000) and carries the filer's gross entry (230,000) instead; harmless here because line 19 caps the fraction at 1, but it inflates the fraction generally — re-cutting a user entry needs sign-off.


**Form 1116 line 1a takes "wages NOT EXCLUDED on Form 2555" — and it must move independently of the line-3f ratio.** sc_00348 follow-up: we carried the filer's gross foreign income (230,000) rather than the non-excluded 100,000, matching H&R Block. Harmless when line 19's fraction is capped at 1, but it inflates the foreign share of taxable income generally and over-states the limitation wherever the fraction would otherwise be below 1. **★ Do NOT reduce `gross` itself:** lines 3d/3e explicitly INCLUDE the excluded income, so the standard-deduction apportionment must keep dividing the full 230,000 by 230,000. The obvious one-line fix would have cut the apportioned deduction from 15,750 to 6,848 and line 15 from 84,250 to 93,152 — wrong, and plausible enough to pass review; `theDeductionApportionmentIsUnmoved` pins the ratio at 1 so it cannot be done silently later. **★ The reduction needs a double-reduction guard:** `grossForeignIncome` is the filer's own entry and nothing records whether it is gross or already net, so the cut applies only when the entered total EXCEEDS the non-excluded foreign earned income; an already-net entry is left alone (`anAlreadyNetGrossEntryIsLeftAlone`). General category only; pro-rata across countries. Pins: `Sc00348SqaScenarioTest` (11).


**The §904 limitation is figured from the FOUR-PLACE printed fraction — confirmed twice now.** sc_00349: 20,267 × 0.2308 = 4,677.62 → **4,678**; at full precision 20,267 × 0.23076586 = 4,676.93 → 4,677. Line 19 is rendered onto the filed form, so only 4,678 foots — the convention settled in sc_00345, now independently confirmed, with H&R Block agreeing. **A scenario's Expected column is not an oracle:** sc_00349's §904(c) carryover of 7,323 is wrong (12,000 − 4,678 = **7,322**); the document states the credit as 4,678 but carries 4,677 through that one subtraction, and its Computation notes say 15,590/410 where its own table says 15,589/411. us-tax-be and H&R Block both produce 7,322. **★ High-tax kickout changes the basket:** 12,000 on 30,000 is a 40% effective rate, above the top US individual rate, so Reg. §1.904-4(c) reclassifies the income passive → general (`applyPassiveHighTaxKickout`). No graded value moves, but the §904(c) carryover is tagged to the GENERAL basket, which decides what can absorb it next year — so a scenario headed "passive" can legitimately produce a general-category Form 1116. **Open, needs sign-off:** the instructions want HTKO PRESENTED on two forms ("HTKO" on line i, negative in the passive HTKO column, positive in the general); we emit only the general form. Credit/limitation/carryover are unaffected — the passive form nets to zero — so this is filed-presentation only. Pins: `Sc00349SqaScenarioTest` (7), incl. a below-threshold control that stays passive.


**Schedule 8812 Part II-B is numbered 21-26, and sc_00351's document labels are each one too high.** Real 2025 form: 21 withheld social security/Medicare (W-2 boxes 4 and 6), 22 Schedule 1 line 15 + Schedule 2 lines 5/6/13, 23 add 21 and 22, 24 EIC (Form 1040 line 27a) + excess social security (Schedule 3 line 11), 25 subtract 24 from 23 floored at zero, 26 larger of line 20 or line 25; the ACTC itself is **line 27 in Part II-C**. Our output model already uses the real numbering, so its values match the SQA tester's readings rather than the doc's labels — the HRB tree reconciled by meaning and recorded 29 passes, and both trees agree once the offset is applied. **★ A ZERO ON LINE 24 PROVES NOTHING WITHOUT A CONTROL.** Line 24 subtracts the EIC, so an allowed EIC wipes out the alternative method entirely. My first probe showed line 24 = 0 and every graded value matching, and the zero was there *for the wrong reason* — the EIC had never been computed, because it needs THREE seeding surfaces (household dependents, its own `eicQualifyingChildren` list, and the `claimsEIC` gate). Dropping the interest under the $11,950 §32(i) ceiling yields an EIC of $2,036, floors line 25, and returns the ACTC to method 1's $300 — that is what makes the zero evidence. Employee FICA is 7.65% against the formula's 15%, so Part II-B only overtakes below roughly $5,100 of earned income. Pins: `Sc00351SqaScenarioTest` (6), incl. the control. See [[feedback_eic_child_three_seeding_surfaces]], [[feedback_verify_entry_not_outcome]].


**A matching bottom line can hide a different credit entirely — grade the sub-form.** sc_00353: Form 8863 Part III lines 23-26 (the four AOTC eligibility questions) FAIL CLOSED, so a student who fails the at-least-half-time test is not AOTC-eligible and the engine routes the expenses to the **Lifetime Learning Credit** instead — 20% × $4,000 = $800, capped at the same $328 of tax. **Every Form 1040 value is then identical**: same tax, same credit, same refund, different credit claimed. My first probe of this scenario matched all thirteen Form 1040 rows for exactly that wrong reason, and only the empty Form 8863 internals gave it away. The fallback itself is correct (an AOTC-ineligible student may still take the LLC). **Also a real check: the tax on $3,250 is 328, not 325** — the Tax Table's 3,250-3,300 row taxes the $3,275 midpoint at 10% = 327.50 → 328, where the rate schedule gives a flat $325; a return quietly using the schedule below $100,000 is wrong by three dollars and still looks plausible. The §25A(i) line-7 bar (under 24, full-time student, earned income < half of support, parent alive, not MFJ) denies the refundable 40% and shifts the whole credit to the nonrefundable Part II, where the excess is LOST — it does not carry forward. Pins: `Sc00353SqaScenarioTest` (6), incl. a bar-lifted control (refund 2,900) and the LLC-fallback control. See [[feedback_verify_entry_not_outcome]].


**Credit ordering is proved by a TRIPLE of values, not by any one.** sc_00354: FTC $150 → education $500 → saver's limited to the remaining $438 → CTC last with nothing left. The signature of correct order is {saver's 438, nonrefundable CTC 0, ACTC 1,700}; applying the CTC before the saver's credit would instead give {0, 1,088, 1,112} and a refund of $2,112 rather than $2,700. All three move together, so assert them together. The $562 of saver's credit above the remaining tax is **permanently lost** — §25B has no carryforward — while the CTC it displaced is recovered as refundable ACTC. Non-circular because Form 8880's Credit Limit Worksheet subtracts only Schedule 3 lines 1-3 while Schedule 8812's subtracts lines 1-4, putting the CTC last. **The scenario also omits an EIC the filer qualifies for** (HOH, one child, $35,000 earned, investment income far under the $11,950 §32(i) ceiling): $2,462, for a refund of $5,162 not $2,700 — the SQA tester caught it and us-tax-be agrees to the dollar, while HRB's $2,700 is a run where the EIC was never claimed rather than a disagreement. It moves only the bottom line: the EIC is refundable and touches neither Schedule 3 nor the 8812 Credit Limit Worksheet. **Both headline figures are TABLE lookups:** tax $1,088 (10,850-10,900 row) and EIC $2,462 (35,000-35,050 row, where the formula at exactly $35,000 gives $2,466). Seeding traps hit here: the 1099-DIV gates are `hasUploadedAtLeastOne1099DivStatement`/`confirmAllReceived1099DivUploaded`, and the IRA deduction is `iraDeductionLine20` — **not** `traditionalIraContributionAmount`, which drives the Form 5329 excess excise instead; with both wrong the return computed cleanly and entirely incorrectly. Pins: `Sc00354SqaScenarioTest` (5).


**Schedule SE nonfarm optional method (§1402(l)) — the elected figure REPLACES the regular one and raises the tax on purpose.** sc_00355: min(⅔ × gross, the $7,240 maximum preprinted on line 14) becomes net earnings directly — **not** multiplied by 92.35% again, because it enters at line 4b as net earnings, which is why it replaces rather than adds to the regular $6,800 × 0.9235 = $6,280. The form itself carries the mechanics: "Skip line 2 if you use the nonfarm optional method in Part II", and the Part II heading states both ceilings (net nonfarm profit under **$7,840** AND under **72.189%** of gross nonfarm income) plus the 2-of-3 prior-year gate and the five-times-lifetime caution. **The election COSTS money — $1,108 vs $961 — in exchange for a fourth quarter of coverage, so assert the DIRECTION**; an engine that silently optimised to the lower figure would be wrong. The income-tax side stays on the ACTUAL Schedule C net profit; only the half-SE-tax adjustment reflects the election. All gates fall back to the regular method rather than producing an invalid base. **★ The two Actuals trees disagree here and our engine reproduces BOTH**: the SQA tester recorded 1,108 (election made), the HRB automation 961 with line 17 at zero — and 961/6,280/481 is exactly the no-election result, so the divergence is a missing election, not a reading of §1402(l). Seeding trap: Schedule C expenses must use real `SCHEDULE_C_EXPENSE_KEYS` (`supplies`, `carAndTruckExpenses`, …); a catch-all `otherExpenses` on the business is ignored, net becomes gross, and the engine then *correctly* refuses the election for an unrelated reason. Pins: `Sc00355SqaScenarioTest` (8).


**Church-employee income (§1402(j)) is its own Schedule SE track, and the $108.28 floor is its only floor.** sc_00356: a lay employee of a church that filed Form 8274 has no FICA withheld, so line 5b = wages × 92.35% carries SE tax with **no $400 net-earnings minimum** — the $108.28 figure exists because 108.28 × 0.9235 ≈ $100. The same wages remain ordinary income on line 1a, so they carry income tax AND SE tax. **Lines 10 and 11 are each rounded to WHOLE DOLLARS before being added** (3,206 + 750 = 3,956) — the same convention settled in sc_00210, where carrying full precision left the printed form unable to reproduce its own line 11; the two rows the SQA tester marked FAIL are this convention, not a divergence, and the Expected column's unrounded 3,206.39/749.88 sum to the identical 3,956. **Design choice, not a defect:** church-employee income must be DECLARED on the SE-options form — a W-2 with box 1 and blank boxes 3-6 yields no SE tax and no advisory, because that shape also belongs to statutory employees and certain territory W-2s, so inferring from it would be wrong more often than right (H&R Block asks the same question). Pins: `Sc00356SqaScenarioTest` (7), incl. BOTH sides of the floor ($100 → no Schedule SE, $109 → $101 net earnings and $16 tax) — a one-sided floor test would pass against an engine that never computed the track at all.


**§108(a)(1)(B) insolvency exclusion: we compute the worksheet, but Form 982 does not exist.** sc_00360: insolvency = max(0, liabilities − asset FMV) per §108(d)(3); the exclusion is capped at that amount (§108(a)(3)) and applied to the COD remaining after any QPRI exclusion, which takes precedence by default under §108(a)(2)(C). Derived from `codTotalLiabilitiesBeforeDischarge` / `codFmvAssetsBeforeDischarge` on `other-incomes-taxpayer` rather than asking the filer for a pre-computed exclusion — Pub. 4681's worksheet is retained rather than filed, so skipping it is legitimate, but deriving it removes the mis-subtraction failure mode. The exclusion is **opt-in**: without `codExcludeUnderInsolvency` the whole cancellation is taxable under §61(a)(11). **★ REAL GAP: no Form 982 anywhere** — no output model, mapper, PDF asset, UI form, or outstanding.md entry; the string "982" does not appear in `main/`. It is a FILED attachment whose instructions say to file it with the return to report the exclusion, so the return claims the exclusion unsubstantiated, and a FULLY excluded discharge leaves no trace on the return at all. In scope by the project's own attachment rule; needs sign-off to build. Seeding trap: the form id is `other-incomeS-taxpayer` (**plural**) — the singular drops the COD silently and the return computes cleanly on wages alone. Pins: `Sc00360SqaScenarioTest` (6), incl. floor / cap / opt-in controls.


**Form 982 (V262) — the filed form the §108 exclusion never had.** COD excluded under §108 must be reported on Form 982, a FILED attachment whose instructions say to file it with the return; the app computed the exclusion and netted it out of Schedule 1 line 8c while producing no form at all. Part I is now built from the SAME `CodExclusionBreakdown` that drives line 8c, so form and return cannot disagree. **★ The gate is the EXCLUDED AMOUNT, not line 8c** — a fully-excluded discharge nets 8c to zero and Schedule 1 disappears, which is precisely when Form 982 is the only evidence the debt was forgiven; an election that excludes nothing produces no form. **`computeOtherIncomes` returns null on `!hasAnySchedule1Input`, which silently ate the form** until the gate was taught to return a Form-982-only result — the same "computed, set, then dropped" shape as the V260/V261 bridge failures, hit one day after being written down as a known hazard. **Asset pipeline, in order:** `scripts/generate-semantic-982.js` (27/27 curated IRS-line names → `pdfs/f982_semantic_labels.pdf` + `_field_map_semantic.csv`, failing loudly on an unmapped field) then `scripts/generate-f982-elements.py`, which unlike every earlier `*_elements.json` generator extracts the layout DIRECTLY from the IRS PDF with PyMuPDF rather than from an external renderer's `pdf_elements_v2.json` — **flip y, since PyMuPDF is top-left origin and the CSV/preview are bottom-left**; verify by cross-checking a label against its field rect, not by eye. Part II (§108(b) attribute reduction) is specified in `lines/982.md` §5, columns present and null. Pins: `Sc00360SqaScenarioTest` (9), `form982-cod-exclusion-preview.spec.ts` (3).


**§108(b) attribute reduction (Form 982 Part II) — built, and the credit rate runs the opposite way to intuition.** Excluding COD is a deferral: the excluded amount must reduce tax attributes in the §108(b)(2) order — NOL, general business credit, minimum tax credit, net capital loss, basis, passive activity carryovers, foreign tax credit. **★ "Reduce that carryover by 33⅓ cents FOR EACH DOLLAR EXCLUDED" means $30,000 of exclusion costs $10,000 of credit — $1 of credit absorbs $3 of exclusion.** I originally wrote the inverse ($30,000 consuming $90,000 of credit), which is wrong by a factor of nine; `applySection108Reduction` carries the direction in its javadoc for that reason. **★ §108(b)(4)(A) makes this a carryforward problem, not a current-year one** — reductions happen after the year's tax, so nothing on the discharge-year return moves and only the outgoing carryforward shrinks; five of the fourteen bridges hold exactly these attributes. **Only a two-year test can prove it**: reducing a value that is then persisted unreduced passes every unit test and is worth nothing (the V260/V261 failure shape). Line 8 skipped — no MTC carryforward is persisted; line 10a (§1017 property basis) reported as an advisory rather than guessed. **The AMT passive twin is reduced by the SAME exclusion, not a second bite** — the regular and AMT suspended losses measure one activity and must not drift. Seeding trap: an NOL leaves a carryforward only when it exceeds the §172 80% limit, so a too-small seed makes a working reduction look broken. Pins: `Section108AttributeReductionTest` (6), `form982-cod-exclusion-preview.spec.ts` (4).


**Form 982 is now complete except two documented caps.** §108(a)(2) is a **strict precedence**: title 11 → QPRI → insolvency → farm → QRPBI, each taking what the one above left; **a title 11 discharge DISPLACES the others entirely** (they "shall not apply") rather than layering under them, and is excluded in full with no cap of its own. **★ §108(b)(5) REORDERS Part II rather than adding a line** — without the election the NOL is reduced first and basis last; with it, depreciable-property basis absorbs the exclusion ahead of the NOL, which is why anyone makes it. Test the CONTRAST (NOL intact vs reduced), not the presence of line 5. **Line 3 is tri-state** — the form prints a Yes/No pair, so unanswered must leave both boxes blank; a printed "No" is an answer the filer never gave. **★ The semantic-name map must be built from the FINISHED model**: building it inside `buildForm982` froze it before the §108(b) reduction ran, so Part II and the header were computed, persisted and never rendered — `populateForm982Fields` now runs last. Still deferred: line 8 (no MTC attribute), line 10a (§1017 general basis, advisory), and the §108(g)(3) farm / §108(c)(2) QRPBI caps, which need figures a 1040 does not carry. UI note: form labels are single-quoted TS literals — **"your spouse's" terminates the string**, and escaping through a Python-writes-TS generator is two layers of quoting; use possessive-free phrasing. Pins: `Form982ExclusionOrderingTest` (7), `form982-cod-exclusion-preview.spec.ts` (5).


**★ §108(b)(1) decides which COD exclusions reach Form 982 Part II, and it is not all of them.** The attribute cascade applies to "the amount excluded from gross income under subparagraph (A), (B), or (C) of subsection (a)(1)" — title 11, insolvency, qualified farm. **QRPBI (D) goes to line 4 alone (§108(c)(1)) and QPRI (E) to line 10b alone (§108(h)(1));** whatever a basis cannot absorb is **included in income**, never spilled into the NOL. The instructions say it three times: line 1d ("applied to reduce the basis of depreciable real property on line 4"), line 10b ("**that part of line 2 that is attributable to** the exclusion of qualified principal residence indebtedness"), and line 2 (reductions run over "lines 5 through 13 **excluding line 10b**"). **Within the cascade, the farm basis step is lines 11a-c and comes AFTER the capital loss** (line 1c: "in the order listed on lines 6 through 9. Any remaining amount... lines 11a through 13"), not ahead of the NOL. **The §108(b)(5) election is gated to boxes 1a-1c.** ★ Feeding the cascade line 2 instead of its own portion passed sc_00361 20-for-20 — a filer with no attributes cannot expose it — so **pin an NOL control on any attribute-ordering rule**, never the graded case alone. Still open: line 10b's "**if you continue to own the residence after discharge**" gate has no intake field. Pins: `Sc00361SqaScenarioTest` (6), `Form982ExclusionOrderingTest` (9).


**Form 982 line 10b is conditional on still owning the home (V264, `codRetainedResidenceAfterDischarge`).** "If box 1e is checked **and you continue to own the residence after discharge**, enter the smaller of..." — and line 2's instruction repeats that the reduction is "required only if you continue to own the residence". ★ **Tri-state, and unanswered is NOT "no"**: reduce-by-default understates a foreclosed filer's basis, skip-by-default overstates a retaining filer's, and **neither error surfaces until the home is sold years later** — so an unanswered question yields a blank line 10b plus the blocking `FORM_982_QPRI_RESIDENCE_RETENTION_UNANSWERED`. Blocking but deliberately NOT in `NonOverrideableFlags.CODES`: missing information is not a return the app *knows* to be wrong, so §17 absolutism does not apply. **Losing the home does not make the debt taxable** — line 2 is untouched in all three states; only the basis reduction is gated. The field went on the **intake** form (`other-incomes-taxpayer` + spouse, in `PERSONAL_FORMS`), never the `1099-c` statement or the `form-tax-return-982` preview — line 10b already exists on the IRS form and the question only decides whether it is populated.


**★ §165(d) is enforced for the CASUAL gambler and unguarded for the PROFESSIONAL one.** Schedule A line 16 is capped at the line 8b winnings (`SCHEDULE_A_GAMBLING_LOSSES_EXCEED_WINNINGS`); a Schedule C wagering business is not capped at all, and a losing year produces a net business loss the statute forbids. For TY2025 the limit covers **more than the bets**: the TCJA sentence in force for years beginning after 2017 and before 2026 makes "losses from wagering transactions" include "any deduction otherwise allowable … incurred in carrying on any wagering transaction" — the sentence that overruled *Mayo* — so travel, tournament fees and data services are capped too, and **the worst a wagering Schedule C can show is zero**. (Pub. L. 119-21 adds a 90% haircut from TY2026 only.) **The blocker is intake, not arithmetic**: nothing marks a Schedule C as a wagering business, so closing it needs a field and sign-off. **★ A profitable gambler passes every graded row whether or not the cap exists — pin the LOSING year.** Pins: `Sc00363SqaScenarioTest` (4, one a characterization test naming the correct answer).

**★ A scenario doc can convict itself — check its internal consistency before trusting any single line.** sc_00363 stated taxable income 136,522 while its own tax note computed 24% of 33,173 (= 136,523 − 103,350), and its stated tax was unreachable from 136,522. I had previously dismissed that $1 as "half-SE rounding" **without deriving it**; naming a plausible mechanism is not checking one. When two figures in a doc disagree, derive both — the one that reproduces the doc's *other* numbers is the right one.


**★ §165(d) is now enforced on BOTH gambling paths (V265).** Casual: Schedule A line 16 capped at the line 8b winnings. Professional: `isWageringBusiness` **per activity** on `business-income-taxpayer` (a filer may run a poker business *and* a consulting business; only the wagering one is capped), applied to the activity's **total deductions** — because the TCJA sentence in force for TY2018-2025 makes "losses from wagering transactions" include "any deduction otherwise allowable … incurred in carrying on any wagering transaction" (the sentence that overruled *Mayo*), so travel, tournament fees, depreciation and home office are capped alongside the bets. **The excess is lost — §165(d) grants no carryforward.** ★ **The rate is a TAX-YEAR SWITCH**: Pub. L. 119-21 §70114 allows only **90%** of the losses from TY2026, *and* keeps the gains ceiling, so from 2026 a **profitable** gambler is haircut too (the sc_00363 filer nets 232,000, not 200,000). ★ **A cap that manufactures exact zeros needs a `hasAnySchedule1Input` clause** — a capped wagering business lands on exactly $0 and the gate discards the whole additional-income block, so Schedule C line 31 had nowhere to go; that gate now has four such clauses and each was added after something real was dropped. **Year-dependent behaviour cannot be unit-tested here**: setting `currentContext` makes every prior-year importer reachable and `importedPriorYearQbiLossCarryforward` hits Panache, so pin the rate by reflection and prove the arithmetic in an e2e that can send `X-Tax-Year`. Pins: `Sc00363SqaScenarioTest` (6), `professional-gambler-section165d.spec.ts` (3).


**Schedule A lines 11-13 carry the DEDUCTIBLE charitable amounts, not the gross gifts.** Line 14 is literally `line14_add_lines_11_through_13` — a pure sum, with **no ceiling step on the form**; the §170(b) AGI limit is applied in the Pub. 526 worksheet beforehand. A scenario expecting the gross gift on line 12 *and* a capped line 14 is self-contradictory. **§170(b)(1)(E) conservation easements** get a **50%**-of-contribution-base ceiling (the `fiftyPool` remainder — 50% of AGI less every category already allowed) against the **30%** that caps ordinary appreciated capital-gain property, and **§170(b)(1)(E)(ii) gives the conservation slice 15 succeeding years** where §170(d)(1) gives ordinary categories 5 — the two windows coexist inside one vintage, so past year 5 a vintage keeps **only** its conservation slice (`ageCharityVintages`, static and package-private, so the boundaries are testable without seeding sixteen years). ★ **The graded rows cannot tell a 50% ceiling from any rule that happens to yield the same number — pin the 30% contrast.** Pins: `Sc00364SqaScenarioTest` (6).

**★ An absence claim loses to a measurement that cites its line.** On sc_00364 the two Actuals trees disagreed on one row: the SQA tester recorded the 50%-of-AGI ceiling as "not printed anywhere", while the us-tax-hrb tree read it off the Charitable Donations Worksheet Part IV line 20 and noted the row had been upgraded *from* arithmetic *to* a direct reading. The tester was reading the **filed** return; that worksheet is **retained**. When trees disagree, ask which row is a measurement — and check whether the artifact is filed or retained before calling it missing. [[feedback_absent_artifact_is_not_a_zero]] [[feedback_two_divergent_actuals_trees]]


**★ Form 4952 line 4a is GROSS investment income and INCLUDES qualified dividends** — line 4b exists precisely to take them back out, and 4c = 4a − 4b. A scenario (or a reader) that calls the 4c remainder "line 4a" is mislabelling; likewise "line 4" is **line 4h** (= 4c + 4f + 4g). **The §163(d)(4)(B) election on line 4g must COST what it buys**: it raises the §163(d)(1) ceiling *and* the elected amount must be **removed from preferential income** in the line-16 worksheets, or the same dollars are double-benefited — a graded scenario cannot see this, so **pin the no-election control** (deduction falls to net investment income, the excess carries forward on line 7, and the standard deduction may then win). **An amount on line 4g FORCES the Schedule D Tax Worksheet**, not the QDCG worksheet — they agree only when there is no 28%-rate or unrecaptured §1250 gain. Pins: `Sc00368SqaScenarioTest` (5).

**★ Three scenarios running (sc_00363/364/368), the SCENARIO DOCUMENT was wrong about which line it meant, never the software.** Schedule 1 "line 25" was line 26; Schedule A line 12 was expected to hold a gross gift that line 14's pure sum makes impossible; Form 4952 "4a" was 4c and "line 4" was 4h. **Resolve every line-number claim against the IRS AcroForm field map in `C:\us-tax\pdfs\*_field_map*.csv`, never the printed label or the doc's wording** — and check the doc's internal consistency, because a mislabelled line usually contradicts another figure in the same doc.


**A red e2e run is not automatically a defect — read the FAILURE TYPE first.** `TypeError: Failed to fetch` and `expect(response.ok()).toBeTruthy()` are HTTP/network failures; a real tax defect shows up as a *value* mismatch. In the 2026-09-18 full regression all three failures were network-level, scattered across tests #420/#865/#1337, and all three passed on a targeted re-run with no code change. `retries` is unset GLOBALLY in `playwright.config.ts` (Playwright defaults to **0**), so for most specs a single momentary blip prints identically to a genuine failure — **re-run the failing specs before diagnosing**. ★ CORRECTION (2026-09-19): a few specs OPT IN per-file via `test.describe.configure({ retries: 1 })` — `mfs-spouse-alt-fuel-credit` and `medicaid-waiver` do. I previously stated the no-retries rule as blanket, which is wrong; check the spec before concluding "it only ran once". And note a retry failing is NOT proof of a real defect: on 2026-09-19 the alt-fuel spec failed twice in-suite (incl. an HTTP 500) and then passed in isolation. ★ And check your instruments: I twice reached a confident wrong conclusion here — once from a `netstat | head -3` truncated past the line I wanted (concluding a live dev server was dead), once from comparing a container ID against a baseline I had recorded *before* a restart. Both were one command away from being checked.


**Form 8960 lines 4a-4c are a RECONCILIATION, not a single net figure.** 4a carries the **gross** Schedule 1 line 5 amount (rental/royalty/partnership/S-corp/trust/trade-or-business); 4b is the "Adjustment for net income or loss derived in the ordinary course of a **non-section 1411** trade or business" and backs out the materially-participated (non-passive) portion; 4c combines them. Reporting the net on 4a and leaving 4b blank gives the same NII and the same NIIT — **every graded row passes either way** — but it never shows the IRS the income arriving and being removed, which is the adjustment being claimed. ★ **§1411(a)(1) taxes the SMALLER of NII and the MAGI excess** — a counterfactual that multiplies 3.8% by NII alone overstates the tax whenever the excess is the binding side (sc_00369's doc claimed 3,230; the real figure is 2,850). Run the counterfactual, don't quote it. Pins: `Sc00369SqaScenarioTest` (4).


**★ Form 8960's adjustment lines are SIGNED, and guarding them on "positive" hides them.** Lines 5b, 5c, 6, 7 and 10 are adjustments that are **negative in their most common real use** — a §1411(c)(4) partnership-sale adjustment, a §121 residence exclusion. Guarding the output field on `hasPositiveAmount` let the figure reach line 5d and the tax while the line printed **blank**: the form does not foot and the exclusion is invisible. Use `hasNonZeroAmount` for any line that can legitimately be negative. sc_00370 graded 27-for-27 the whole time it was broken. **★ "Combine lines 5a through 5c" means SIGNED ADDITION, not subtraction** — the line 5b instruction is explicit ("Enter the amount of gains **as a negative number** and losses as a positive number"), so subtracting line 5b inverts the sign for any filer who follows the instructions. When a field's sign convention comes from the IRS instructions rather than the printed label, **say so in the UI label** — ours carried the IRS wording with no hint, leaving the filer to guess. Pins: `Sc00370SqaScenarioTest` (5), incl. `line5bIsCombinedNotSubtracted` and a 5a+5b+5c = 5d footing assertion.


**Form 8960 Part II: 9d = 9a+9b+9c, line 10 is "Additional modifications", line 11 is the total, and line 12 subtracts line 11 from line 8.** A reference calling the deductions total "line 10" is off by one. **Line 9a auto-derives from Form 4952 line 8** — the same investment interest allowed on Schedule A, not asked for twice — while **line 9b is a taxpayer entry on purpose**: Reg. §1.1411-4(f)(3)(iii) permits "any reasonable method" for allocating state/local income tax to NII, so picking one substitutes our judgement for the filer's. Expect commercial software to differ here by its own method (HRB uses the unrounded ratable share) — **that is a method difference, not a defect**, and the right test pins both columns. ★ **Form 4952 needs line 4a seeded or the whole chain is dead**: with no gross investment income line 4h is 0, so line 8 = min(line 3, 0) = 0 and nothing reaches Schedule A — which looks exactly like "the feature is not implemented". That misreading sat in `_repro_progress.md` as an UNRESOLVED gap. Pins: `Sc00371SqaScenarioTest` (5).


**★ The OBBBA enhanced senior deduction ($6,000, 65+) is SEPARATE from the age-65 standard-deduction add-on ($2,000).** Both apply to the same filer: the add-on sits inside Form 1040 line 12, the senior deduction on Schedule 1-A → line 13b, phasing out at 6% of MAGI above $75,000 ($150,000 MFJ). Assert them separately (line 12, line 13b, line 14) so they can never be conflated. A 2025 reference that omits it understates taxable income relief by $6,000 — sc_00373's doc did exactly that, and BOTH Actuals trees caught it. **Form 1040 line 5a stays BLANK for a fully taxable pension** — "enter the total … on line 5b; don't make an entry on line 5a" — so a null there is the right answer, not a missing one. ★ **And a tester comment is not automatically right**: after six scenarios where the document mislabelled a line, sc_00373's worksheet-renumbering comment described a *different* worksheet (its own software's display, with a "line C"), while the doc matched the official Social Security Benefits Worksheet exactly. Check the instructions, not the pattern. Pins: `Sc00373SqaScenarioTest` (4).


**RRB retirement is two statements under two regimes and they must not be commingled.** The BLUE RRB-1099 box 5 **net SSEB** is Tier 1 → Form 1040 line 6a and the §86 worksheet (read the SSEB-specific subfield, not a generic box 5 — NSSEB routes to the pension lines instead); the GREEN RRB-1099-R is Tier 2/NSSEB/supplemental → line 5 as a §72 pension, fully taxable when box 3 shows no unrecovered cost. ★ **Tier 2 enters the §86 worksheet at line 3 as OTHER income** — it raises provisional income, and so changes how much Tier 1 is taxed, without ever being a §86 benefit itself. Pin that with a Tier-2-removed control: provisional income drops below the base amount and taxable Tier 1 goes to zero. **The OBBBA senior-deduction omission in the scenario docs is confined to sc_00373 and sc_00374** — verified by scanning every 65+ scenario: sc_00337 computes its phase-out correctly and sc_00380/381 are dedicated to it. Those two are the low-MAGI retiree cases where the deduction is worth its full $6,000, i.e. the generator misses it exactly where it bites. Pins: `Sc00374SqaScenarioTest` (5).


**Form 2210 Part I has no line 8a or 8b.** Line 4 is current-year tax, **line 5** is 90% of it, **line 6** is withholding, **line 8** is "Maximum required annual payment based on prior year's tax" (100%, or **110%** when prior-year AGI exceeds $150,000 / $75,000 MFS), and line 9 takes the smaller of 5 and 8. The penalty is **line 19** → Form 1040 line 38; line 27 is Schedule AI. ★ **Form 1040 line 37 INCLUDES the penalty** — "Add the penalty to any tax due and enter the total on line 37" — so a reference showing tax-due on 37 *and* a penalty on 38 as separate end figures is self-inconsistent. **The §6654 penalty is computable, not approximate**: four equal installments of the underpayment accruing at the year's underpayment rate (7% for 2025) over 365/304/212/90 days to the April filing date. sc_00375's "≈$71, illustrative" was wrong; hand calculation, us-tax-be and H&R Block all give **63**. ★ **Name Form 2210 output fields semantically** (`ninetyPctCurrentYear`, `priorYearSafeHarbor`, `requiredAnnualPayment`) rather than by line number — that is why the document's invented line numbers never reached our output, while seven scenarios in this run carried renumbering errors. Pins: `Sc00375SqaScenarioTest` (5).

**★ An input table that cannot produce its own expected output is a scenario defect.** sc_00375 listed only W-2 boxes 1 and 2 but expected $450 of Additional Medicare Tax, which Form 8959 computes from **box 5** Medicare wages; "box 5 = box 1" appeared only in a parenthesis on the output side. One tester seeded exactly what was written and got zero — correctly. When two Actuals trees diverge, check whether the *inputs* differ before concluding anything about the software.


**★ §6654(e)(3)(B) (retired after 62 / disabled) is a REQUESTED waiver, not an automatic one.** The statute reads "No addition to tax shall be imposed … if the Secretary **determines**" — the filer checks Form 2210 **Part II box A** ("your **entire** penalty"; box B is partial) and attaches a statement. Never infer it from a date of birth: that claims relief the filer did not ask for, on facts (reasonable cause, not willful neglect) software cannot know. Key it off an election, and set `computationMethod=WAIVED` so a zero penalty records *why* it is zero rather than being indistinguishable from "no penalty arose". ★ **Pin the no-election control** — every graded row in a waiver scenario is equally consistent with an engine that found no penalty at all. **A waiver forgives the CHARGE, not the liability**: the tax is unchanged and line 37 drops only by the penalty. ★ **Age 62 earns the waiver, NOT a deduction** — the age-65 standard-deduction addition and the OBBBA senior deduction both require 65, so a 62-or-63-year-old gets neither; assert both are zero. Pins: `Sc00376SqaScenarioTest` (3).

**Expect commercial packages to differ on FEATURE COVERAGE, not just arithmetic.** On sc_00376 both packages computed the identical $165 penalty and only one could waive it — H&R Block asks the retired-after-62 question and then offers no box to claim the relief. "The trees disagree" there means one product lacks a feature, not that anyone computed wrongly. Record it as a product gap and keep both columns.


**★ Schedule 1-A uses BOTH rounding directions, and which one depends on the Part.** Parts II and III (tips, overtime) **FLOOR** the $1,000 bucket count — line 11: "decrease the result to the next lower whole number … decrease 1.5 to 1, and decrease 0.05 to 0". Part IV (car loan interest) **CEILINGS** it, $200 per $1,000 over $100,000. "The Schedule 1-A phaseout rounds down" is not a blanket rule; `computeTipsOvertimePhaseout` (FLOOR) and `computeCarLoanPhaseout` (CEILING) sit adjacent in the code for exactly this reason. ★ **A round-number MAGI hides the difference** — sc_00377's 170,000 gives exactly 20 buckets, so a doc that said "or fraction" (a ceiling) still produced the right answer; test a fractional excess (170,999) to tell them apart. **The phaseout prints POSITIVE on line 12 and is subtracted at line 13** — not as a signed negative. **The tips deduction stacks ON the standard deduction and never leaves AGI**, so the tip income still drives the MAGI that phases its own deduction out. Pins: `Sc00377SqaScenarioTest` (4).

**★ Schedule 1-A Parts II-IV need FOUR gates and fail silently without them**: `hadAdditionalDeductions` (parent opt-in), `confirmAllRelevantStatementsUploaded` (statement gate), and `taxpayerReceivedQualifiedTips` / `taxpayerTippedOccupationConfirmed` — the last two **PREFIXED**, with the unprefixed spellings silently ignored. Miss any and there is no Schedule 1-A at all: a clean return, a zero deduction, and no flag. Part **V** (the OBBBA senior deduction) is an **entitlement** and needs none of them, which is why a 65+ scenario computes it with no additional-deductions form.


**★ Never pass `--reporter` on the e2e CLI — it REPLACES the config's reporter list.** `playwright.config.ts` registers `[['list'], ['json', { outputFile: 'test-results-report.json' }]]`, and its comment records why: a 2026-09-13 failure whose assertion existed only in terminal scrollback could never be investigated. Passing `--reporter=list` silently drops the JSON reporter, so a 3.6-hour run's failure detail is lost exactly when it is most expensive to re-obtain. I did this on 2026-09-19. If a reporter must be added, append to the config rather than override on the command line.


**★ A green regression is not a clean run — read the backend log too.** On 2026-09-20 a suite that went **1,590 passed / 0 failed** was simultaneously emitting **123 HTTP 500s**, all from `/api/tax-return/optimize/joint-vs-separate`. No test was red because the UI swallows that call, so the only evidence was in the log. After a regression, count distinct exception types and failing endpoints in the backend log, not just the Playwright tally.

**★ Throw the exception type that carries the right STATUS — a validation precondition is a 4xx, never a 500.** `enableMfs` threw `IllegalStateException` for "no spouse identification on file", which no mapper claims, so it rendered as *Internal Server Error* with an error id for an ordinary input state. Use `jakarta.ws.rs.BadRequestException` (mapped to 400 + `ErrorResponse`) for anything the filer can fix, and keep `IllegalStateException` for invariants something else guarantees — the "no head person" check beside it is the correct use, since `AppUserBootstrap` creates that row. ★ **The cost of getting this wrong is diagnostic, not cosmetic:** 123 bogus 500s buried the genuine ones and cost a full triage cycle on 2026-09-19. Noise is worth fixing even when nothing is red.

**★ A UI that fires a request it EXPECTS to fail is a defect in two places.** `computeAllAndOptimize` calls the optimizer after every compute with a comment saying the backend "throws otherwise", then swallows it. Swallowing made it invisible to users and to tests, and left it to rot in the log. When you see a `try { … } catch {}` whose comment explains that the call normally fails, treat it as a bug report.


**★ Schedule 1-A's rounding direction is per-PART, now confirmed on two scenarios.** Lines 11 (tips) and 19 (overtime) both "decrease the result to the next **lower** whole number" — a FLOOR; line 28 (car loan interest) "increase the result to the next **higher** whole number" — a CEILING, at $200 per $1,000 over $100,000. The semantic field map encodes it too: `line19_overtime_phaseout_whole_thousands` vs `line28_car_loan_interest_phaseout_whole_thousands_round_up`. ★ **A round-number MAGI cannot tell a floor from a ceiling** — sc_00377 (20 buckets) and sc_00378 (15 buckets) both carried a wrong ⌈⌉ rule in their docs and both still produced the right answer. When checking a bucketed phaseout, always test a FRACTIONAL excess; for sc_00378 that is 165,999 (→11,000) against 166,000 (→10,900). Pins: `Sc00377SqaScenarioTest`, `Sc00378SqaScenarioTest`.

**★ Overtime has no occupation gate and that is CORRECT — don't "fix" the asymmetry.** Tips need `taxpayerReceivedQualifiedTips` **and** `taxpayerTippedOccupationConfirmed`; overtime needs only `taxpayerReceivedQualifiedOvertime`. §224 keys tips to a customarily-tipped **occupation**, which is a fact separate from the amount; §225 keys overtime to whether the pay is **FLSA §7 mandated premium**, which the amount itself already expresses. Adding a parallel gate would invent a field nobody needs. (Same lesson as 1040 lines 27c/28 — paired controls on one form are not automatically symmetrical.)
